    ...
    <script type="module">
      ...
      window.updateReceivePreview = function () {
        ...
        if (tokensPerEth > 0n) {
          ...
          document.getElementById('receive-amount').textContent = receive.toLocaleString(
            undefined,
...
<html lang="en">
  ...
  <body>
    ...
    <div class="card">
      ...
      <div class="input-suffix">
        <input
          ...
          id="cash-out-amount"
          placeholder="0"
          oninput="updateCashOutPreview()"
        />
        <span>tokens</span>
      </div>
      <button class="btn btn-secondary" onclick="setMax()" style="margin-bottom: 0.75rem;">
        Max
      </button>
      <div class="receive-preview">
    ...
    <div class="card warning">
      <p>
        Cash outs burn your tokens permanently. You'll receive your proportional share of the
        treasury surplus.
      </p>
    </div>
    ...
    <script type="module">
      ...
      async function loadTokenBalance() {
        ...
        document.getElementById('token-balance').textContent =
          parseInt(formatEther(tokenBalance)).toLocaleString() + ' tokens'
      ...
      window.cashOut = async function () {
        ...
        try {
          ...
          document.getElementById('tx-status').innerHTML =
            `Transaction sent... <a href="${getTxUrl(CHAIN_ID, hash)}" target="_blank">View</a>`
          await publicClient.waitForTransactionReceipt({ hash })
  ...
  <body>
    ...
    <div class="card hidden" id="selected-tier">
      ...
      <div class="stat-row">
        ...
        ><span class="stat-value tier-price" id="tier-price"></span>
      </div>
      <div class="stat-row">
    ...
    <script type="module">
      ...
      function renderTiers() {
        document.getElementById('tiers-container').innerHTML = TIERS.map(
          (tier) => `
  ...
  <body>
    ...
    <div id="payouts-tab" class="tab-content active">
      <div class="card">
        ...
        <button class="btn btn-secondary" onclick="setMaxPayout()" style="margin-bottom: 1rem;">
          Max Available
        </button>
        <div class="splits-preview">
          <strong>Split Recipients:</strong>
          <div id="splits-list" style="margin-top: 0.5rem; color: var(--text-muted);">
            Loading...
          </div>
        </div>
    ...
    <div id="allowance-tab" class="tab-content">
      <div class="card">
        ...
        <p style="color: var(--text-muted); font-size: 0.875rem; margin-bottom: 1rem;">
          Withdraw from surplus (discretionary spending).
        </p>
        <div class="stat-row">
    ...
    <div id="reserved-tab" class="tab-content">
      <div class="card">
        ...
        <p style="color: var(--text-muted); font-size: 0.875rem; margin-bottom: 1rem;">
          Mint and distribute accumulated reserved tokens.
        </p>
        <div class="stat-row">
...
<html lang="en">
  ...
  <body>
    ...
    <div class="card" id="claim-section">
      <div class="info">
        Credits are stored in the Juicebox protocol. Claiming converts them to ERC-20 tokens in your
        wallet.
      </div>
    ...
    <div class="card warning-card hidden" id="no-token-warning">
      <p>
        This project hasn't deployed an ERC-20 token yet. Credits cannot be claimed until the
        project owner deploys one.
      </p>
    </div>
    ...
    <script type="module">
      ...
      async function loadBalances() {
        ...
        creditBalance = await publicClient.readContract({
          address: tokens,
          abi: TOKENS_ABI,
          functionName: 'creditBalanceOf',
          args: [address, PROJECT_ID],
        })
        ...
        document.getElementById('credit-balance').textContent = parseInt(
          formatEther(creditBalance),
        ).toLocaleString()
        document.getElementById('token-balance').textContent = parseInt(
          formatEther(tokenBalance),
        ).toLocaleString()
        document.getElementById('total-balance').textContent = parseInt(
      ...
      window.setMaxClaim = () =>
        (document.getElementById('claim-amount').value = parseInt(formatEther(creditBalance)))
...
<html lang="en">
  ...
  <body>
    ...
    <div class="card">
      ...
      <div class="stat-row">
        <span class="stat-label">Duration</span><span class="stat-value" id="duration">-</span>
      </div>
      <div class="stat-row">
        <span class="stat-label">Issuance (tokens/ETH)</span
      ...
      <div class="stat-row">
        <span class="stat-label">Data Hook</span><span class="stat-value" id="data-hook">-</span>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="stat-row">
        ...
        <span class="stat-value"
          ><a id="owner-link" href="#" target="_blank"><span id="owner-address">-</span></a></span
        >
      </div>
    </div>
    ...
    <script type="module">
      ...
      async function loadProjectData() {
        ...
        try {
          ...
          document.getElementById('token-supply').textContent = parseInt(
          ...
          document.getElementById('weight').textContent = parseInt(
            formatEther(ruleset.weight),
          ).toLocaleString()
          ...
          document.getElementById('data-hook').textContent =
            metadata.dataHook === zeroAddress ? 'None' : truncateAddress(metadata.dataHook)
