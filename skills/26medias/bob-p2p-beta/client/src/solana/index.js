...
class SolanaClient {
  ...
  loadKeypair(privateKey) {
    ...
    if (typeof privateKey === 'string') {
      ...
      try {
        ...
        throw new Error(
          'Invalid private key format. Must be: array [123, 45, ...], base58 string, or mnemonic phrase',
        )
      }
    }
  ...
  async sendPayment(recipient, amount) {
    try {
      ...
      const sourceTokenAccount = await getAssociatedTokenAddress(this.tokenMint, this.publicKey)

      const destTokenAccount = await getAssociatedTokenAddress(this.tokenMint, recipientPubkey)
  ...
  async getBalance() {
    try {
      const tokenAccount = await getAssociatedTokenAddress(this.tokenMint, this.publicKey)
