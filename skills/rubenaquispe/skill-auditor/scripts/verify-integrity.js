...
function formatVerifyReport(results, skillName, sourceUrl) {
  ...
  const total =
    ...
    results.remoteOnly.length
  const tampered =
    results.modified.length > 0 || results.localOnly.length > 0 || results.remoteOnly.length > 0
  ...
  if (!tampered) {
    ...
    lines.push(
      `Files: ${total} checked | ${results.matched.length} match, ${results.modified.length} modified, ${results.localOnly.length} added, ${results.remoteOnly.length} missing`,
    )
    lines.push('')
...
async function main() {
  ...
  if (!fs.existsSync(skillDir)) {
    console.error(`Not found: ${skillDir}`)
    process.exit(2)
  }
