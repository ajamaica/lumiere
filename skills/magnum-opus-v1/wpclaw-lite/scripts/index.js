...
if (!WPCLAW_STORE_URL || !WPCLAW_STORE_SECRET) {
  throw new Error(
    'Configuration Error: WPCLAW_STORE_URL and WPCLAW_STORE_SECRET environment variables are required.',
  )
}
...
async function callWoo(endpoint, payload) {
  try {
    ...
    const signature = crypto.createHmac('sha256', WPCLAW_STORE_SECRET).update(body).digest('hex')
