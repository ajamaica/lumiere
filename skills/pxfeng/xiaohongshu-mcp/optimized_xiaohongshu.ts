...
export async function login() {
  ...
  try {
    ...
    try {
      await page
        .getByText('发布笔记', { exact: true })
        .or(page.locator('.publish-btn'))
        .first()
        .waitFor({ state: 'visible', timeout: 300000 }) // 5 minutes wait
    } catch (e) {
...
export async function upload(args: {
  ...
  try {
    ...
    if (validatedTitle.length > 20) {
      console.error(
        `Warning: Title exceeds 20 characters (${validatedTitle.length}), truncating...`,
      )
    ...
    const contentInput = page
      .locator(
        'textarea[placeholder*="填写正文"], div[contenteditable="true"], div[role="textbox"]',
      )
      .first()
