...
import { calculateDistance, formatDate, geocodeLocation, parseEventId } from './utils'
...
async function handleHostGuests(slug: string): Promise<any> {
  ...
  if (!cookies) {
    return {
      error: 'Not authenticated',
      message:
        "You need to set up Luma cookies to view guest lists. Run 'luma configure' for instructions.",
    }
  }
