    ...
    try {
      while (true) {
        ...
        for (const part of parts) {
          ...
          const message = JSON.parse(json) as AgentTaskMessage | AgentTaskMessageUpdate
          ...
          if (message.type === AgentTaskMessageType.Creation) {
            ...
            if (msgIndex !== -1) {
              ...
              collectedMessages[msgIndex].data.result =
                (collectedMessages[msgIndex].data.result ?? '') + (update.delta.result ?? '')

              collectedMessages[msgIndex].data.error =
                (collectedMessages[msgIndex].data.error ?? '') + (update.delta.error ?? '')
