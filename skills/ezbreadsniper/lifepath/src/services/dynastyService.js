...
class DynastyService {
  ...
  async createDynasty(foundingLifeId, userId, dynastyName) {
    ...
    await this.db.query('UPDATE lives SET dynasty_id = $1, generation = 1 WHERE id = $2', [
      dynasty.id,
      foundingLifeId,
    ])

    // Update user stats
    await this.db.query('UPDATE users SET total_dynasties = total_dynasties + 1 WHERE id = $1', [
      userId,
    ])
  ...
  calculateInheritance(parent) {
    ...
    return {
      intelligence: Math.round(
        parent.intelligence * heritability.intelligence + Math.random() * 70,
      ),
      health: Math.round(parent.health * heritability.health + Math.random() * 80),
