        ...
        inputSchema: {
          ...
          properties: {
            ...
            key: {
              type: 'string',
              description:
                'Optional S3 key (path in bucket). If not provided, uses filename with UUID prefix',
            },
            bucket: {
              type: 'string',
            ...
            public: {
              type: 'boolean',
              description:
                'If true, generate public URL without signature (requires public bucket)',
              default: false,
            },
            content_type: {
              type: 'string',
    ...
    switch (name) {
      ...
      case 'r2_list': {
        ...
        for (const obj of response.Contents) {
          ...
          const modified =
            obj.LastModified?.toISOString().replace('T', ' ').substring(0, 19) || 'unknown'
          lines.push(`  ${obj.Key}`)
