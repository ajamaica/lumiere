  ...
  <head>
    ...
    <style>
      ...
      .timeline-line {
        ...
        bottom: 0;
        width: 2px;
        background: linear-gradient(180deg, var(--jb-yellow) 0%, var(--border-color) 100%);
      }
      ...
      .ruleset-card.current {
        ...
        background: linear-gradient(90deg, var(--success-bg) 0%, var(--bg-secondary) 20%);
      }
      .ruleset-card.upcoming {
        border-left-color: var(--jb-yellow);
        background: linear-gradient(90deg, rgba(255, 204, 0, 0.1) 0%, var(--bg-secondary) 20%);
      }
      .ruleset-card.past {
        opacity: 0.7;
      }
      ...
      .ruleset-card.current .timeline-dot {
        background: var(--success);
      }
      .ruleset-card.upcoming .timeline-dot {
  ...
  <body>
    <div class="container">
      ...
      <div id="timelineContainer">
        <div class="loading">Enter a project ID to load timeline</div>
      </div>
    </div>
    ...
    <script type="module">
      ...
      const CONTROLLER_ABI = [
        ...
        },
        {
          ...
          outputs: [
            ...
            { name: 'approvalStatus', type: 'uint256' },
          ],
        },
        {
          name: 'allRulesetsOf',
      ...
      async function loadTimeline() {
        ...
        container.innerHTML =
          '<div class="loading"><div class="spinner"></div>Loading ruleset history...</div>'
        ...
        try {
          ...
          document.getElementById('projectIdDisplay').textContent =
            `ID: ${projectId} on ${chainConfig.name}`
          document.getElementById('projectHeader').classList.add('visible')

          // Load current ruleset
          const [currentRuleset, currentMetadata] = await controller.read.currentRulesetOf([
            BigInt(projectId),
          ])
          ...
          let upcomingRuleset = null,
            queuedRuleset = null
          try {
            const [upcoming, upcomingMeta] = await controller.read.upcomingRulesetOf([
              BigInt(projectId),
            ])
          ...
          try {
            const [queued, queuedMeta, approvalStatus] =
              await controller.read.latestQueuedRulesetOf([BigInt(projectId)])
            const queuedId = queued.id.toString()
            if (
              queuedId !== currentRulesetId &&
              (!upcomingRuleset || queuedId !== upcomingRuleset.ruleset.id.toString())
            ) {
              queuedRuleset = {
                ruleset: queued,
                metadata: queuedMeta,
          ...
          const historicalRulesets = await controller.read.allRulesetsOf([
            BigInt(projectId),
            0n,
            50n,
          ])
      ...
      function toggleDetails(index) {
        ...
        btn.textContent = details.classList.contains('visible')
          ? 'Hide details'
          : 'Show more details'
      }
      ...
      function compareRulesets(index1, index2) {
        const rs1 = allRulesets[index1],
          rs2 = allRulesets[index2]
        const fields = [
          { key: 'weight', label: 'Weight', format: formatWeight },
          {
            key: 'weightCutPercent',
            label: 'Weight Cut %',
            format: (v) => (Number(v) / 10000000).toFixed(2) + '%',
          },
          {
            key: 'duration',
            label: 'Duration',
            format: (v) => (Number(v) === 0 ? 'Indefinite' : formatDuration(Number(v))),
          },
          {
            key: 'reservedRate',
            label: 'Reserved Rate',
        ...
        fields.forEach((field) => {
          ...
          const f1 = field.format(val1),
            f2 = field.format(val2)
