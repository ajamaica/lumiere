...
import { computeRsi, slopeLast, sma } from './indicators/rsi.js'
...
function narrativeFromSign(x) {
  if (x === null || x === undefined || !Number.isFinite(Number(x)) || Number(x) === 0)
    return 'NEUTRAL'
...
async function main() {
  ...
  while (true) {
    ...
    try {
      const chainlinkPromise =
        polymarketWsPrice !== null
          ? Promise.resolve({
              price: polymarketWsPrice,
              updatedAt: polymarketWsTick?.updatedAt ?? null,
      ...
      const close1mAgo =
        klines1m.length >= 2 ? (klines1m[klines1m.length - 2]?.close ?? null) : null
      const close3mAgo =
        klines1m.length >= 4 ? (klines1m[klines1m.length - 4]?.close ?? null) : null
      ...
      const predictNarrative =
        pLong !== null && pShort !== null && Number.isFinite(pLong) && Number.isFinite(pShort)
          ? pLong > pShort
            ? 'LONG'
            : pShort > pLong
              ? 'SHORT'
      ...
      const signal =
        rec.action === 'ENTER' ? (rec.side === 'UP' ? 'BUY UP' : 'BUY DOWN') : 'NO TRADE'
      ...
      const spread =
        spreadUp !== null && spreadDown !== null
          ? Math.max(spreadUp, spreadDown)
          : (spreadUp ?? spreadDown)
      ...
      const marketStartMs =
        poly.ok && poly.market?.eventStartTime
        ...
        if (slug && !dumpedMarkets.has(slug)) {
          ...
          try {
            ...
            fs.writeFileSync(
              ...
              JSON.stringify(poly.market, null, 2),
              'utf8',
            )
          } catch {
            // ignore
      ...
      const binanceSpotBaseLine = colorPriceLine({
        label: 'BTC (Binance)',
        price: spotPrice,
      ...
      const lines = [
        ...
        settlementLeftMin !== null
          ? kv('Time left:', `${polyTimeLeftColor}${fmtTimeLeft(settlementLeftMin)}${ANSI.reset}`)
          : null,
        priceToBeat !== null
          ? kv('PRICE TO BEAT: ', `$${formatNumber(priceToBeat, 0)}`)
          : kv('PRICE TO BEAT: ', `${ANSI.gray}-${ANSI.reset}`),
        currentPriceLine,
        '',
        sepLine(),
        '',
        ...
        kv(
          'ET | Session:',
          `${ANSI.white}${fmtEtTime(new Date())}${ANSI.reset} | ${ANSI.white}${getBtcSession(new Date())}${ANSI.reset}`,
        ),
        '',
        sepLine(),
