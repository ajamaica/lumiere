#!/usr/bin/env node
import { getListingDetails, searchListings, summarizeListing } from '../src/index.js'
...
function printListing(listing, index) {
  ...
  console.log(
    `   Price: ${listing.priceDisplay}${listing.location ? ` | Location: ${listing.location}` : ''}`,
  )
...
async function fetchAndPrintDetails(target) {
  ...
  if (details.images.length > 0) {
    console.log(
      `Images: ${details.images.slice(0, 3).join(', ')}${details.images.length > 3 ? ' ...' : ''}`,
    )
  }
  ...
  for (const facet of facets.slice(0, 8)) {
    ...
    const sample =
      facet.categories
        ...
        ?.join(' | ') ||
      facet.attributeGroup
        ?.slice(0, 3)
        ?.map((a) => a.attributeValueLabel)
        ?.join(' | ') ||
      ''
    console.log(
      `  ${label} → ${param}${sample ? ` (e.g., ${sample}${facet.categories?.length > 3 || facet.attributeGroup?.length > 3 ? ' ...' : ''})` : ''}`,
    )
  }
  console.log('')
}
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Showing ${result.listings.length} of ${result.total} results for "${args.query}".\n`,
    )
    result.listings.forEach((listing, idx) => printListing(listing, idx))

    if (args.detailsTarget) {
      const target =
        args.detailsTarget === 'first'
          ? result.listings[0]?.vipUrl || result.listings[0]?.id
          : args.detailsTarget
      if (!target) {
