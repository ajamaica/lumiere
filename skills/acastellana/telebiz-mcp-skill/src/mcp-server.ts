...
const TOOLS: Tool[] = [
  ...
  // Message Tools
  {
    name: 'sendMessage',
    description:
      'Send a text message to a chat. Supports Telegram markdown: **bold**, __italic__, ~~strike~~, `code`, ||spoiler||, [text](url).',
    inputSchema: {
      type: 'object',
      properties: {
        chatId: {
        ...
        username: {
          type: 'string',
          description:
            'Username (without @) for private chats. Required if chat not previously opened.',
        },
        text: {
          type: 'string',
  ...
  },
  {
    name: 'createFolder',
    description:
      'Create a new chat folder. Must provide either includedChatIds or chat type filters.',
    inputSchema: {
      type: 'object',
      properties: {
        title: {
