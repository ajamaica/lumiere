...
async function findList(bring, listIdentifier) {
  ...
  if (!listIdentifier) {
    ...
    if (defaultList) {
      const found = lists.find(
        (l) => l.listUuid === defaultList || l.name.toLowerCase() === defaultList.toLowerCase(),
      )
      if (found) {
  ...
  const found = lists.find(
    (l) => l.listUuid === listIdentifier || l.name.toLowerCase() === listIdentifier.toLowerCase(),
  )
        ...
        JSON.stringify(
          {
            lists: lists.map((l) => ({
              ...
              uuid: l.listUuid,
              isDefault:
                process.env.BRING_DEFAULT_LIST &&
...
function main() {
  ...
  program.command('lists').description('show available shopping lists').action(listsAction)
