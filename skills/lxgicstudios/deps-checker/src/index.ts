...
export async function runDepcheck(
  dir: string,
): Promise<{ unused: string[]; unusedDev: string[]; missing: Record<string, string[]> }> {
...
export async function analyzeResults(
  depcheckResult: { unused: string[]; unusedDev: string[]; missing: Record<string, string[]> },
  outdated: string[],
  pkgJson: any,
): Promise<string> {
  if (!process.env.OPENAI_API_KEY) {
    throw new Error(
      'Missing OPENAI_API_KEY environment variable. Set it with: export OPENAI_API_KEY=sk-...',
    )
  }
    ...
    messages: [
      {
        role: 'system',
        content:
          "You're a dependency audit expert. Analyze the project dependencies and give actionable advice. Be concise, use bullet points. Mention specific packages by name. Flag any security concerns you know about.",
      },
      {
        role: 'user',
