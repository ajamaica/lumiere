...
2. **Default-deny Firewall**
   ...
   - Commands:
     ...
     sudo ufw enable
     ```
     _Note: Ensure you have console access or a fallback before enabling if SSH is not yet allowed on another interface, though we configure Tailscale next._
...
4. **Install Tailscale**
   ...
   - Commands:
     ...
     sudo tailscale up
     ```
   - _Wait for user to authenticate the Tailscale link._
