...
```typescript
const result = await engine.validate('ls && rm -rf /', metadata)
...
```typescript
const result = await engine.validate('http://169.254.169.254/latest/meta-data/', metadata)
// result.severity = CRITICAL
// result.action = block_notify
```
...
```typescript
const result = await engine.validate('../../../etc/passwd', metadata)
// result.severity = HIGH
// result.action = block
```
...
```typescript
const result = await engine.validate('API_KEY=sk-abc123def456ghi789', metadata)
// result.severity = CRITICAL
// result.action = block_notify
```
