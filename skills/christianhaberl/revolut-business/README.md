...
> ⚠️ **Important:** `REVOLUT_ISS_DOMAIN` must be the domain part of your redirect URI (without `https://`). This is used as the `iss` (issuer) claim in the JWT. Revolut will reject any other value with: _"The 'iss' (issuer) claim must be your domain"_
