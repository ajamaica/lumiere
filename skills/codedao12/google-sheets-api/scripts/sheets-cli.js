...
const READ_ONLY_COMMANDS = new Set(['read', 'batchGet', 'info', 'getFormat'])
...
function resolveCredentials() {
  const inlineJson =
    process.env.GOOGLE_SHEETS_CREDENTIALS_JSON || process.env.GOOGLE_SERVICE_ACCOUNT_JSON
  if (inlineJson) {
    ...
    switch (command) {
      ...
      case 'format': {
        ...
        if (!spreadsheetId || !range || !formatRaw)
          throw new Error('Usage: format <spreadsheetId> <range> <formatJsonOr@file>')
      ...
      case 'borders': {
        ...
        if (!spreadsheetId || !range)
          throw new Error('Usage: borders <spreadsheetId> <range> [styleJsonOr@file]')
      ...
      case 'copyFormat': {
        ...
        if (!spreadsheetId || !sourceRange || !destRange)
          throw new Error('Usage: copyFormat <spreadsheetId> <sourceRange> <destRange>')
