...
- UX: fix `status --summary` to actually include the one-line summary _and_ the detailed status output.
...
- UX: `charge status --json` now prints _only_ JSON (subset of `charge_state`) for piping/parsing.
...
- UX: `status --json` now prints _only_ JSON (no extra human text), making it safe to pipe/parse.
