...
async function main() {
  try {
    switch (command) {
      ...
      case 'token-balance':
        if (!args[0] || !args[1])
          throw new Error('Usage: bnb.js token-balance <token_address> <wallet_address>')
        await getTokenBalance(args[0], args[1])
        break

      case 'send':
        const sendKey = getKey(args)
        if (!args[0] || !args[1] || !sendKey)
          throw new Error('Usage: bnb.js send <to> <amount> --key <private_key>')
        await sendBNB(args[0], args[1], sendKey)
        break

      case 'send-token':
        const tokenKey = getKey(args)
        if (!args[0] || !args[1] || !args[2] || !tokenKey)
          throw new Error('Usage: bnb.js send-token <token> <to> <amount> --key <private_key>')
