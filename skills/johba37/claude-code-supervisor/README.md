...
`watchdog.sh` is a pure bash script (no LLM, no Claude Code dependency) that runs on a timer and checks: _is the tmux session still there? Is Claude Code still running inside it?_ If something is dead and nobody reported it, it notifies.
