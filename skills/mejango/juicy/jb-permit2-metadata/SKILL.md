...
const JB721_HOOK_ID =
  '0x' + ethers.utils.keccak256(ethers.utils.toUtf8Bytes('JB721TiersHook')).slice(2, 10) // First 4 bytes
```
...
function encode721HookData(allowOverspending: boolean, tierIds: number[]): Hex {
  return encodeAbiParameters(
...
function build721HookMetadata(allowOverspending: boolean, tierIds: number[]): Hex {
  ...
  const hookId =
    '0x' + ethers.utils.keccak256(ethers.utils.toUtf8Bytes('JB721TiersHook')).slice(2, 10)
...
function buildCombinedMetadata(
  ...
  return createMetadata([permit2Id, hookId], [permit2Data, hookData]) as Hex
}
