...
function validateWithSecuritySkill(
  command: string,
  ...args: string[]
): { allowed: boolean; reason?: string } {
  try {
    if (!existsSync(CONFIG.securitySkillPath)) {
      logSecurityEvent('WARNING', 'Security skill not found - validation skipped', {
        path: CONFIG.securitySkillPath,
      })
    ...
    logSecurityEvent('WARNING', 'Security validation failed - allowing by default', {
      command,
      error: error.message,
    })
        ...
        for (const field of textFields) {
          if (args[field] && typeof args[field] === 'string') {
            ...
            if (!validation.allowed) {
              logSecurityEvent(
                'THREAT',
                `Prompt injection detected in ${field} - ${validation.reason}`,
                {
                  tool,
                  field,
...
export async function after_tool_call(
  context: ToolCallContext & { result?: any; error?: any },
): Promise<HookResult> {
