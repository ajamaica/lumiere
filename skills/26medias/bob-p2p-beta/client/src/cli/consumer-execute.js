...
async function main() {
  ...
  try {
    ...
    if (!apiId) {
      ...
      console.log(
        'Usage: npm run execute <api-id> -- --config config.json --provider http://provider-url --provider-wallet ABC --body \'{"key":"value"}\'',
      )
      process.exit(1)
    }
    ...
    if (args['queue-code'] && args['transaction']) {
      ...
      if (completedJob.localFilePath) {
        ...
        console.log(
          `Download: npm run download ${completedJob.jobId} -- --config config.json --url ${completedJob.resultUrl} --output result.file`,
        )
      }
    } else {
      // Full workflow
