...
import { getAuthorization, logout, startOAuthFlow } from '../lib/auth.js'
import {
  getCurrentAccountId,
  getTokens,
  isAuthenticated,
...
export function createAuthCommands(): Command {
  ...
  auth
    ...
    .action(async () => {
      try {
        if (isAuthenticated()) {
          console.log(
            chalk.yellow('Already authenticated. Use "basecamp auth logout" to logout first.'),
          )
          return
        }
  ...
  auth
    ...
    .action(async () => {
      ...
      try {
        ...
        console.log(
          chalk.dim(
            `User: ${authorization.identity.first_name} ${authorization.identity.last_name}`,
          ),
        )
  ...
  auth
    ...
    .action((options) => {
      ...
      console.log(
        chalk.yellow(
          'You must set it via environment variable before running "basecamp auth login":',
        ),
      )
      console.log(chalk.cyan('\n  export BASECAMP_CLIENT_SECRET="your-client-secret"\n'))
      console.log(
        chalk.dim('Add this to your shell profile (~/.bashrc, ~/.zshrc, etc.) for persistence.'),
      )
    })
  ...
  const accounts = new Command('accounts')
    ...
    .action(async () => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to list accounts:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
...
export function createAccountCommand(): Command {
  ...
  account
    ...
    .action(async (id: string) => {
      try {
        ...
        console.error(
          chalk.red('Failed to set account:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
  ...
  account
    ...
    .action(async () => {
      ...
      if (!currentAccountId) {
        console.log(
          chalk.yellow('No account selected. Run "basecamp account set <id>" to select one.'),
        )
        return
      }
