...
async function main() {
  ...
  const rpcUrl =
    input.rpcUrl || process.env.STARKNET_RPC_URL || 'https://rpc.starknet.lava.build:443'
  ...
  for (const c of callsInput) {
    if (c && c.contractAddress && c.method) {
      ...
      throw new Error(
        'Each call must be either {contractAddress, method, args} or {contractAddress, entrypoint, calldata}',
      )
    }
  }
