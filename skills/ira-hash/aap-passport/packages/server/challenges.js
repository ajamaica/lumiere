  ...
  nlp_extract: {
    generate: (nonce) => {
      ...
      const distractorPool =
        category === 'animals' ? 'fruits' : category === 'fruits' ? 'colors' : 'animals'
...
Response format: {"salt": "${salt}", "items": ["item1", "item2", "item3"]}`,
        ...
        validate: (solution) => {
          try {
            ...
            const items = (obj.items || obj.animals || obj.fruits || obj.colors || [])
              .map((s) => s.toLowerCase())
              .sort()
...
Response format: {"salt": "${salt}", "output": "result"}`,
        ...
        validate: (solution) => {
          try {
            ...
            return (
              output === String(expected) || output.toLowerCase() === String(expected).toLowerCase()
            )
          } catch {
            return false
          }
...
export function generate(nonce, type) {
  ...
  const selectedType =
    type && types.includes(type) ? type : types[Math.floor(Math.random() * types.length)]
...
export function validateBatch(validators, solutions) {
  ...
  for (let i = 0; i < validators.length; i++) {
    ...
    const valid =
      solution && validators[i](typeof solution === 'string' ? solution : JSON.stringify(solution))
