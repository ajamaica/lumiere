...
// Remove entries
await sdk.removeAddressBookEntry([{ chain: 'Ethereum', address: '0x...' }])
```
...
vault.on('qrCodeReady', (data) => {
  /* show QR for device pairing */
})
vault.on('deviceJoined', (data) => {
  /* co-signer connected */
})
vault.on('allDevicesReady', (data) => {
  /* threshold met, signing can proceed */
})
