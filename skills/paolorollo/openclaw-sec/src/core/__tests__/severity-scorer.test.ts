import { Finding, SecurityPattern, Severity } from '../../types'
...
describe('SeverityScorer', () => {
  ...
  describe('calculateSeverity', () => {
    ...
    it('should handle single finding', () => {
      const scorer = new SeverityScorer()
      const findings: Finding[] = [createMockFinding('prompt_injection', Severity.HIGH)]
    ...
    it('should correctly order severity levels (CRITICAL > HIGH > MEDIUM > LOW > SAFE)', () => {
      ...
      testCases.forEach(({ findings, expected }) => {
        const mockFindings = findings.map((sev, idx) => createMockFinding(`module_${idx}`, sev))
  ...
  describe('edge cases', () => {
    ...
    it('should handle large number of findings', () => {
      const scorer = new SeverityScorer()
      const findings: Finding[] = Array(1000)
        .fill(null)
        .map((_, idx) => createMockFinding(`module_${idx % 6}`, Severity.LOW))
