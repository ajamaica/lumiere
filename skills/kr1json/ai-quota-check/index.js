...
async function generateDashboard(options = {}) {
  ...
  const currentModel = currentModelArg
    ? currentModelArg.split('=')[1]
    : process.env.OPENCLAW_MODEL || 'N/A'
  ...
  if (quotas.copilot) {
    ...
    if (quotas.copilot.error) {
      ...
      if (p.unlimited) {
        ...
        console.log(
          `- **Premium Interactions:** ${getEmoji(p.percent)} ${p.remaining}/${p.total} (${p.percent}%)`,
        )
  ...
  if (quotas.codex) {
    ...
    if (quotas.codex.error) {
      ...
      if (quotas.codex.primary) {
        ...
        console.log(
          `- **Daily:** ${getEmoji(p.remaining)} ${p.remaining}% left (resets ${formatResetTime(p.resetTime)})`,
        )
      ...
      if (quotas.codex.secondary) {
        ...
        console.log(
          `- **Weekly:** ${getEmoji(s.remaining)} ${s.remaining}% left (resets ${formatResetTime(s.resetTime)})`,
        )
      }
      console.log('')
    }
  ...
  console.log(
    '- **Claude (Opus/Sonnet via Antigravity):** High Risk. Hidden weekly cap. Excessive use → 3-7 day lockout.',
  )
