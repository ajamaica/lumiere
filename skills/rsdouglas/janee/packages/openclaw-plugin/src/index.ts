...
async function connect(api: any): Promise<Client> {
  ...
  const client = new Client({ name: 'openclaw-janee', version: '0.1.0' }, { capabilities: {} })
...
export default function (api: any) {
  api.registerTool({
    ...
    description:
      'List available API services managed by Janee. Shows which services have stored credentials.',
  ...
  api.registerTool({
    name: 'janee_execute',
    description:
      'Execute an API request through Janee with stored credentials. Use janee_list_services first to see available services. All requests are logged for audit.',
    parameters: Type.Object({
      service: Type.String({
        description: "Service name from janee_list_services (e.g., 'stripe', 'github', 'bybit')",
      }),
      ...
      reason: Type.Optional(
        Type.String({
          description:
            'Reason for this request (for audit logs, may be required for sensitive operations)',
        }),
      ),
    }),
  ...
  api.registerTool({
    ...
    description:
      'Reload Janee configuration from disk. Use after adding or modifying services in ~/.janee/config.yaml to pick up changes without restarting.',
    parameters: Type.Object({}),
    async execute() {
      return await callWithReconnect(api, async (client) => {
