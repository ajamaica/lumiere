...
export function collectFiles(dir: string, max = 20): string[] {
  ...
  const walk = (d: string) => {
    ...
    for (const entry of readdirSync(d)) {
      ...
      else if (['.ts', '.js', '.tsx', '.jsx', '.py', '.go'].includes(extname(full)))
        files.push(full)
    }
  }
  walk(dir)
...
export async function generateQuestions(
  ...
  const code = files
    .map((f) => `// ${f}\n${readFileSync(f, 'utf-8').slice(0, 600)}`)
    .join('\n\n')
    .slice(0, 10000)
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You generate technical interview questions based on a codebase. Level: ${level}. Generate ${count} questions.
