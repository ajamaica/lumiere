...
import type { AuthConfig, CapabilityConfig, ServiceConfig } from '../config-yaml'
...
export async function addCommand(
  ...
  try {
    ...
    const createCap =
      !createCapAnswer ||
      createCapAnswer.toLowerCase() === 'y' ||
      createCapAnswer.toLowerCase() === 'yes'
    ...
    if (createCap) {
      ...
      const autoApprove =
        !autoApproveInput ||
        autoApproveInput.toLowerCase() === 'y' ||
        autoApproveInput.toLowerCase() === 'yes'

      const requiresReasonInput = await rl.question('Requires reason? (y/N): ')
      const requiresReason =
        requiresReasonInput.toLowerCase() === 'y' || requiresReasonInput.toLowerCase() === 'yes'
