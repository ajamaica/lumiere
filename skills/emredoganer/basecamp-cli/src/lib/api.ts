...
export async function listTodoLists(projectId: number): Promise<BasecampTodoList[]> {
  ...
  const response = await client
    .get(`buckets/${projectId}/todosets/${todosetId}/todolists.json`)
...
export async function listTodos(
  ...
  const response = await client
    .get(`buckets/${projectId}/todolists/${todolistId}/todos.json${params}`)
    .json<BasecampTodo[]>()
  return response
}
...
export async function listMessages(projectId: number): Promise<BasecampMessage[]> {
  ...
  const response = await client
    .get(`buckets/${projectId}/message_boards/${messageboardId}/messages.json`)
...
export async function getCampfireLines(
  ...
  const client = await createClient()
  const response = await client
    .get(`buckets/${projectId}/chats/${campfireId}/lines.json`)
