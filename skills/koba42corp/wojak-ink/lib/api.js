...
class WojakAPI {
  ...
  async fetchListings(forceRefresh = false) {
    ...
    if (
      !forceRefresh &&
      this.cache.listings &&
      now - this.cache.listingsTimestamp < this.cacheDuration
    ) {
    ...
    try {
      const response = await fetch(
        `${DEXIE_API}/offers?type=nft&collection=${COLLECTION_ID}&status=0&page_size=100`,
      )
      if (!response.ok) throw new Error(`HTTP ${response.status}`)
  ...
  parseOffer(offer) {
    try {
      ...
      const nftOffered = offer.offered?.find(
        (item) => item.type === 'nft' || item.asset_id === COLLECTION_ID,
      )
      ...
      const priceXch =
        requested.type === 'xch' || !requested.asset_id ? requested.amount / 1e12 : null
