...
class LifeService {
  ...
  async createLife(userId, mode, country, year, gender) {
    ...
    const result = await this.db.query(query, [userId, mode, country, year, gender, seed])
  ...
  shouldDie(age, health) {
    ...
    if (age < 10)
      deathChance = 0.02 // Child mortality
    else if (age < 30)
      deathChance = 0.01 // Young adult
    else if (age < 50)
      deathChance = 0.03 // Middle age
    else if (age < 70)
      deathChance = 0.08 // Senior
    else if (age < 90)
      deathChance = 0.2 // Elderly
    else deathChance = 0.4 // Very old
