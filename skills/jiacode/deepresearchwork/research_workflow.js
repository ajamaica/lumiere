...
class DeepResearchFramework {
  ...
  async deepDive(researchPlan) {
    ...
    for (const source of prioritizedSources.slice(0, 5)) {
  ...
  async reportGeneration(researchPlan) {
    ...
    const remainingQuestions = this.identifyRemainingQuestions(
      researchPlan.researchQuestions,
      researchPlan.phase3.verifiedClaims,
    )
