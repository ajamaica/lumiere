...
import type { SmokeCheck, SmokeTestOptions, SuiteResult, TestResult } from './types.js'
import { findAllByRole, getInteractiveElements } from './utils/snapshot.js'

const DEFAULT_CHECKS: SmokeCheck[] = ['pageLoad', 'consoleErrors', 'navigation', 'images']
...
async function checkForms(qa: QABot, start: number): Promise<TestResult> {
  try {
    ...
    return {
      ...
      consoleEvents: [],
      steps: [
        { name: `${formElements} form elements properly labeled`, status: 'pass', duration: 0 },
      ],
    }
  } catch (err) {
    return {
...
async function checkAccessibility(qa: QABot, start: number): Promise<TestResult> {
  try {
    ...
    const headings = [findAllByRole(snapshot, 'heading')].flat()
