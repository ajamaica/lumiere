...
import { type DiscordVoiceConfig, parseConfig } from './src/config.js'
import { type CoreConfig, loadCoreAgentDeps } from './src/core-bridge.js'
...
const discordVoicePlugin = {
  ...
  register(api: PluginApi) {
    ...
    const mainConfig = api.config as {
      ...
      if (cfg.autoJoinChannel) {
        try {
          ...
          if (channel && channel.isVoiceBased()) {
            ...
            api.logger.warn(
              `[discord-voice] Auto-join channel ${cfg.autoJoinChannel} is not a voice channel`,
            )
          }
        } catch (error) {
          api.logger.error(
    ...
    discordClient.login(discordToken).catch((err) => {
      api.logger.error(
        `[discord-voice] Failed to login: ${err instanceof Error ? err.message : String(err)}`,
      )
    })
    ...
    async function handleTranscript(
      ...
      try {
        ...
        api.logger.error(
          `[discord-voice] Agent chat error: ${error instanceof Error ? error.message : String(error)}`,
        )
