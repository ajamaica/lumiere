...
import { binaryReverseMint, bitagent } from '@bitagent/sdk'
...
async function getAuthenticatedClient() {
  ...
  if (!nonceResponse.ok) {
    throw new Error(
      `Failed to fetch nonce: ${nonceResponse.status} ${nonceResponse.statusText} - ${await nonceResponse.text()}`,
    )
  }
...
async function getCreatorByToken(tokenAddress: string, fallbackAddress?: string): Promise<string> {
  ...
  throw new Error(
    `Could not find creator for token ${tokenAddress}. Please ensure the token is registered on this chain.`,
  )
}
...
const commands = {
  launch: async (args: string[]) => {
    ...
    if (!name || !symbol || !reserveSymbol) {
      console.error(
        'Usage: launch --name <name> --symbol <symbol> --reserve-symbol <reserveSymbol> [--description <desc>]',
      )
    ...
    if (!reserveConfig) {
      console.error(
        `Error: Unsupported reserve symbol '${reserveSymbol}'. Supported: ${Object.keys(config.tokens).join(', ')}`,
      )
      process.exit(1)
    }
    ...
    const txReceipt = await NewToken.create({
      ...
      agentHash,
      reserveToken: {
        address: reserveConfig.token as `0x${string}`,
...
async function main() {
  ...
  if (commands[command as keyof typeof commands]) {
    ...
    console.log(
      '  ts-node cli.ts launch --network <bsc|bscTestnet> --name <name> --symbol <symbol> --reserve-symbol <UB|WBNB|USD1> [--description <desc>]',
    )
    console.log(
      '  ts-node cli.ts buy --network <bsc|bscTestnet> --token <tokenAddress> --amount <amount>',
    )
    console.log(
      '  ts-node cli.ts sell --network <bsc|bscTestnet> --token <tokenAddress> --amount <amount>',
    )
  }
}
