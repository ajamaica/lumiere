...
export class WorkflowOrchestratorImpl extends WorkflowOrchestrator {
  ...
  getWorkflowStatistics() {
    const stats = {
      ...
      successRate:
        (this.executionState.metrics.successfulExecutions /
          this.executionState.metrics.totalExecutions) *
        100,
  ...
  convertToCSV(history) {
    ...
    for (const exec of history.completed) {
      rows.push(
        `${exec.id},${exec.workflow},${exec.startTime},${exec.duration},${exec.steps},${exec.tokens},completed`,
      )
    }
    ...
    for (const exec of history.failed) {
      rows.push(
        `${exec.id},${exec.workflow},${exec.startTime},${exec.duration},${exec.stepsCompleted},0,failed`,
      )
    }
