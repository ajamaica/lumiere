...
import { getRpcUrl, loadConfig } from './config.js'
import { getBalance, loadWallet } from './wallet.js'
...
export async function sendSol(
  ...
  // Get priority fee
  const priorityFee = await getPriorityFee(connection, [fromWallet.publicKey.toBase58(), toAddress])
...
export async function sendToken(
  ...
  tx.add(createTransferInstruction(fromAta, toAta, fromWallet.publicKey, amountRaw))
