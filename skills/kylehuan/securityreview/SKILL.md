...
- **DO** focus on the code the developer has written. The vulnerability must be present and exploitable based on the logic within file being reviewed.
  - **Exception:** The only exception is when a dependency with a _well-known, publicly documented vulnerability_ is being used. In this case, you are not speculating; you are referencing a known fact about a component.
