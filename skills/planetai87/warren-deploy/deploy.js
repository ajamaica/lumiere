...
const GENESIS_KEY_ADDRESS =
  process.env.GENESIS_KEY_ADDRESS || '0x954a7cd0e2f03041A6Abb203f4Cfd8E62D2aa692'
const MASTER_NFT_ADDRESS =
  process.env.MASTER_NFT_ADDRESS || '0x7bb4233017CFd4f938C61d1dCeEF4eBE837b05F9'
...
function estimateChunkGasLimit(chunkSizeBytes) {
  ...
  const calldataRegular =
    zeroBytes * MEGAETH_GAS.CALLDATA_ZERO_PER_BYTE +
    nonZeroBytes * MEGAETH_GAS.CALLDATA_NONZERO_PER_BYTE
  const calldataFloor =
    zeroBytes * MEGAETH_GAS.EIP7623_FLOOR_ZERO_PER_BYTE +
...
const PAGE_BYTECODE =
  ''
  ...
  for (let bs = 0; bs < chunks.length; bs += BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(async (chunk, idx) => {
        await sleep(idx * 50)
        const receipt = await deployPage(
          wallet,
          provider,
          chunk,
          baseNonce + idx,
          `Chunk ${bs + idx + 1}/${chunks.length}`,
        )
...
async function mintSite(wallet, rootChunk, depth, totalSize, siteType) {
  ...
  const tx = await master.mint(wallet.address, rootChunk, depth, totalSize, siteType, {
...
async function deploy(privateKey, content, options = {}) {
  ...
  if (balance === 0n)
    throw new Error('No ETH. Get testnet ETH from https://docs.megaeth.com/faucet')
  ...
  if (addrs.length === 1) {
    ...
    ;({ rootChunk, depth } = await buildTree(wallet, provider, addrs))
  }

  const tokenId = await mintSite(
    wallet,
    rootChunk,
    depth,
    content.length,
    SITE_TYPES[siteType] ?? 0,
  )
...
`)
  ...
  if (!privateKey) {
    console.error('Error: set PRIVATE_KEY env or use --private-key')
    process.exit(1)
  }
  ...
  if (filePath) content = fs.readFileSync(filePath)
    ...
    if (!content.length) {
      console.error('Error: provide --file, --html, or pipe stdin')
      process.exit(1)
    }
  }

  if (content.length > 500 * 1024) {
    console.error(`Error: ${content.length} bytes exceeds 500KB limit`)
    process.exit(1)
  }

  try {
    const result = await deploy(privateKey, content, {
      name: getArg('name') || 'Untitled',
      siteType: getArg('type') || 'file',
    })
    console.log('\n--- JSON ---')
