...
async function main() {
  ...
  if (!tableMode && !jsonMode) {
    ...
    console.log(
      `⏰ Each model type resets every 5 hours (Claude, Gemini Pro, Flash have separate quotas)`,
    )
  ...
  if (jsonMode) {
      ...
      JSON.stringify(
        {
          ...
          accounts: sortedResults.map((r) => ({
            ...
            quotas: Object.fromEntries(
              r.quotas.map((q) => [q.model, { remaining: q.quota, reset: q.resetTime }]),
            ),
          })),
        },
        null,
    ...
    for (const r of sortedResults) {
      ...
      console.log(
        `| ${shortEmail} | ${opusStr} | ${opusReset} | ${proStr} | ${proReset} | ${flashStr} | ${flashReset} |`,
      )
    }
  } else {
    // Text summary
