  ...
  async run() {
    ...
    try {
      ...
      await this.test('Log transaction with all fields', async () => {
        ...
        this.assertEqual(transaction.amount, 50.0)
      ...
      await this.test('Reject invalid transactions', async () => {
        try {
          ...
          this.assert(
            ...
            `Should reject negative amounts, got: ${error.message}`,
          )
        }
      ...
      await this.test('Category spending', async () => {
        ...
        this.assertApproxEqual(
          ...
          75.5,
          0.01,
          'Infrastructure spending should be correct',
        )
      ...
      await this.test('Set and check budget', async () => {
        await this.budgetManager.setBudget('Infrastructure', 100.0, 'monthly')

        const status = await this.budgetManager.checkBudget('Infrastructure')
        this.assert(status.hasBudget, 'Should have budget')
        this.assertEqual(status.budgetAmount, 100.0, 'Budget amount should match')
        this.assertApproxEqual(status.spent, 75.5, 0.01, 'Spent amount should be correct')
        this.assertApproxEqual(status.remaining, 24.5, 0.01, 'Remaining should be correct')
      ...
      await this.test('Budget alerts', async () => {
        ...
        await this.budgetManager.setBudget('Tools', 18.0, 'monthly')
      ...
      await this.test('Privacy.com import functionality', async () => {
        ...
        this.assertEqual(
          imported.category,
          'API/Services',
          'Should auto-categorize OpenAI as API/Services',
        )
      })
      ...
      await this.test('CLI named parameters support', async () => {
        ...
        // Test option parsing
        const options = cli.parseOptions([
          '--amount=25.99',
          '--vendor=TestVendor',
          '--description=Test transaction',
        ])
