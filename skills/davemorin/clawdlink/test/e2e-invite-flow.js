#!/usr/bin/env node
/**
 * ClawPhone End-to-End Test: Invite Flow
 *
...
class MockRelay {
  ...
  async createInvite({ creator, creatorX25519, creatorName }) {
    ...
    return { token, expiresAt: invite.expiresAt, link: `https://clawphone.dev/c/${token}` }
  }
  ...
  async claimInvite({ token, claimer, claimerX25519, claimerName }) {
    ...
    invite.claimer = {
      publicKey: claimer,
      x25519PublicKey: claimerX25519,
      displayName: claimerName,
    }
...
async function runTest() {
  log('\n' + '='.repeat(60))
  log('  ClawPhone End-to-End Test: Invite Flow')
  log('='.repeat(60))
  ...
  log('\n' + '='.repeat(60))
  log(
    `  Results: ${green(results.passed + ' passed')}, ${results.failed > 0 ? red(results.failed + ' failed') : results.failed + ' failed'}`,
  )
  log('='.repeat(60) + '\n')
