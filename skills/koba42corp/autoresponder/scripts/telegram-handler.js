...
function formatContact(contact, index, state) {
  ...
  const delay =
    contact.minMinutesBetweenReplies !== undefined ? contact.minMinutesBetweenReplies : 15
  ...
  const promptPreview =
    contact.prompt.length > 80 ? contact.prompt.substring(0, 80) + '...' : contact.prompt
...
function handleHistory(identifier, limit = 10) {
  ...
  const lines = logs
    .split('\n')
    .filter((line) => line.includes(identifier) || line.includes(name))
...
Generate a response now:`
  ...
  if (!apiKey && fs.existsSync(configPath)) {
    try {
      ...
      apiKey =
        clawdbotConfig.skills?.['openai-whisper-api']?.apiKey ||
        clawdbotConfig.skills?.['openai-image-gen']?.apiKey
    } catch (e) {
...
function handleSetTimeWindow(identifier, start, end) {
  try {
    execSync(`node "${MANAGE_SCRIPT}" set-time-window "${identifier}" "${start}" "${end}"`, {
      encoding: 'utf8',
    })
    const config = loadConfig()
    const contact = config.watchList.find((c) => c.identifier === identifier)
...
function handleAddKeyword(identifier, keyword) {
  try {
    execSync(`node "${MANAGE_SCRIPT}" add-keyword "${identifier}" "${keyword}"`, {
      encoding: 'utf8',
    })
    const config = loadConfig()
    const contact = config.watchList.find((c) => c.identifier === identifier)
...
function handleRemoveKeyword(identifier, keyword) {
  try {
    execSync(`node "${MANAGE_SCRIPT}" remove-keyword "${identifier}" "${keyword}"`, {
      encoding: 'utf8',
    })
    const config = loadConfig()
    const contact = config.watchList.find((c) => c.identifier === identifier)
...
function handleStats(identifier) {
  ...
  if (identifier) {
    ...
    const avgPerDay =
      daysSince > 0 ? (stats.totalResponses / daysSince).toFixed(1) : stats.totalResponses
...
function handleSetDailyCap(identifier, maxReplies) {
  try {
    execSync(`node "${MANAGE_SCRIPT}" set-daily-cap "${identifier}" ${maxReplies}`, {
      encoding: 'utf8',
    })
    const config = loadConfig()
    const contact = config.watchList.find((c) => c.identifier === identifier)
