...
function getCredentialsPath() {
  const envPath = process.env.GOOGLE_SERVICE_ACCOUNT_KEY || process.env.GOOGLE_SHEETS_KEY_FILE
    ...
    switch (command) {
      ...
      case 'write': {
        ...
        if (!spreadsheetId || !range || !dataJson) {
          ...
          console.error(
            'Example: node sheets.js write ABC123 "Sheet1!A1:B2" \'[["a","b"],["c","d"]]\'',
          )
          process.exit(1)
        }
      ...
      case 'append': {
        ...
        if (!spreadsheetId || !range || !dataJson) {
          ...
          console.error(
            'Example: node sheets.js append ABC123 "Sheet1!A:B" \'[["newRow1","newRow2"]]\'',
          )
          process.exit(1)
        }
        const data = JSON.parse(dataJson)
      ...
      case 'format': {
        ...
        if (!spreadsheetId || !range || !formatJson) {
          ...
          console.error(
            'Format options: backgroundColor, textFormat, horizontalAlignment, verticalAlignment, wrapStrategy',
          )
          console.error(
            'Example: node sheets.js format ABC123 "Sheet1!A1:B2" \'{"backgroundColor":{"red":255,"green":255,"blue":0},"textFormat":{"bold":true}}\'',
          )
          process.exit(1)
        }
      ...
      case 'borders': {
        ...
        if (!spreadsheetId || !range) {
          ...
          console.error(
            'Example: node sheets.js borders ABC123 "Sheet1!A1:C3" \'{"style":"SOLID","color":{"red":0,"green":0,"blue":0}}\'',
          )
          process.exit(1)
        }
      ...
      case 'resize': {
        ...
        if (!spreadsheetId || !sheetName || !dimension || !start || !end || !size) {
          console.error(
            'Usage: node sheets.js resize <spreadsheetId> <sheetName> <cols|rows> <start> <end> <pixelSize>',
          )
      ...
      case 'autoResize': {
        ...
        if (!spreadsheetId || !sheetName || !startCol || !endCol) {
          console.error(
            'Usage: node sheets.js autoResize <spreadsheetId> <sheetName> <startCol> <endCol>',
          )
      ...
      case 'copyFormat': {
        ...
        if (!spreadsheetId || !sourceRange || !destRange) {
          console.error(
            'Usage: node sheets.js copyFormat <spreadsheetId> <sourceRange> <destRange>',
          )
      ...
      default:
        ...
        console.error(
          'Commands: read, write, append, clear, create, info, format, merge, unmerge, getFormat, borders, resize, freeze, addSheet, deleteSheet, renameSheet, autoResize, copyFormat',
        )
        process.exit(1)
    }
