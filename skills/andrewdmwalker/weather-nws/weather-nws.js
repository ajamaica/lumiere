...
class NWSWeather {
  ...
  async getWeather() {
    try {
      ...
      const pointsData = execSync(`curl -s -A "${this.userAgent}" "${pointsUrl}"`, {
        encoding: 'utf8',
        timeout: 15000,
      })
      ...
      const forecastData = execSync(`curl -s -A "${this.userAgent}" "${forecastUrl}"`, {
        encoding: 'utf8',
        timeout: 15000,
      })
      ...
      const stationsData = execSync(`curl -s -A "${this.userAgent}" "${stationsUrl}"`, {
        encoding: 'utf8',
        timeout: 15000,
      })
      ...
      const observationData = execSync(`curl -s -A "${this.userAgent}" "${observationUrl}"`, {
        encoding: 'utf8',
        timeout: 15000,
      })
  ...
  formatWeatherData(observation, forecast) {
    ...
    const feelsLikeF = props.heatIndex.value
      ...
      : props.windChill.value
        ? Math.round((props.windChill.value * 9) / 5 + 32)
        : tempF
    ...
    return {
      current: {
        ...
        pressure: props.barometricPressure.value
          ? `${Math.round(props.barometricPressure.value / 3386.39)} inHg`
          : 'N/A',
        visibility: props.visibility.value
          ? `${Math.round(props.visibility.value / 1609.34)} miles`
          : 'N/A',
        dewpoint: props.dewpoint.value
          ? `${Math.round((props.dewpoint.value * 9) / 5 + 32)}°F`
          : 'N/A',
      },
      forecast: {
        ...
        if (lowerText.includes(keyword)) {
          alerts.push({
            ...
            severity:
              alertType === 'tornado'
                ? 'critical'
                : ['severe-storm', 'flood', 'winter'].includes(alertType)
                  ? 'high'
                  : 'medium',
          })
          break
  ...
  async getActiveAlerts() {
    try {
      ...
      const alertsData = execSync(`curl -s -A "${this.userAgent}" "${alertsUrl}"`, {
        encoding: 'utf8',
        timeout: 15000,
      })
