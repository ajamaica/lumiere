...
import { archiveProject, createProject, getProject, listProjects } from '../lib/api.js'
import { isAuthenticated } from '../lib/config.js'

export function createProjectsCommands(): Command {
  const projects = new Command('projects').description('Manage Basecamp projects')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        projectList.forEach((project) => {
          table.push([project.id, project.name, project.status, project.description || '-'])
        })
  ...
  projects
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to get project:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
  ...
  projects
    ...
    .action(async (id: string) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to archive project:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
