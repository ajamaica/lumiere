...
import type { BasecampAuthorization, BasecampTokens } from '../types/index.js'
import {
  clearTokens,
    ...
    const server = http.createServer(async (req, res) => {
      ...
      if (url.pathname === '/callback') {
        ...
        if (!returnedState || !pendingOAuthState || returnedState !== pendingOAuthState.state) {
          res.writeHead(400, { 'Content-Type': 'text/html' })
          res.end(
            '<html><body><h1>Authentication Failed</h1><p>Invalid state parameter. Possible CSRF attack.</p></body></html>',
          )
          server.close()
          pendingOAuthState = null
        ...
        if (code) {
          try {
            ...
            res.end(
              '<html><body><h1>Token Exchange Failed</h1><p>Please try again.</p></body></html>',
            )
            server.close()
            pendingOAuthState = null
            reject(err)
...
async function exchangeCodeForTokens(
  ...
  const response = await got
    .post(`${OAUTH_BASE}/authorization/token`, {
      searchParams: {
        type: 'web_server',
        client_id: clientId,
...
export async function getAuthorization(): Promise<BasecampAuthorization> {
  ...
  const response = await got
    .get(`${OAUTH_BASE}/authorization.json`, {
      headers: {
