...
import { formatCurrency, printError, printJSON, printTable, truncate } from '../utils/output'
  ...
  .action(async (id, options) => {
    ...
    try {
      ...
      if ((account as any).lastSyncedAt) {
        console.log(
          `  ${chalk.cyan('Last Synced:')} ${new Date((account as any).lastSyncedAt).toLocaleString()}`,
        )
      }
    } catch (error) {
