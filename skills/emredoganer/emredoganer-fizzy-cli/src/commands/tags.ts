...
import { createColumn, deleteColumn, listColumns, listTags, updateColumn } from '../lib/api.js'
import { isAuthenticated } from '../lib/config.js'

export function createTagsCommands(): Command {
  const tags = new Command('tags').description('Manage board tags')
  ...
  tags
    ...
    .action(async (options) => {
      ...
      try {
        ...
        tagList.forEach((tag) => {
          table.push([tag.id, tag.name, chalk.hex(tag.color)(tag.color)])
        })
...
export function createColumnsCommands(): Command {
  const columns = new Command('columns').description('Manage board columns')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        columnList.forEach((column) => {
          table.push([column.id, column.name, column.position, column.cards_count])
        })
