...
export interface AccountsAPI {
  ...
  getNetWorthHistory(
    startDate?: string,
    endDate?: string,
  ): Promise<Array<{ date: string; netWorth: number; assets: number; liabilities: number }>>
...
export class AccountsAPIImpl implements AccountsAPI {
  ...
  async getAll(
    options: { includeHidden?: boolean; verbosity?: VerbosityLevel } = {},
  ): Promise<Account[]> {
  ...
  async requestRefresh(accountIds?: string[]): Promise<boolean> {
    ...
    try {
      ...
      if (
        response.requestAccountsRefresh.errors &&
        response.requestAccountsRefresh.errors.length > 0
      ) {
