...
async function main() {
  ...
  if (isFirstRun) {
    ...
    log(
      `Done. Seeded ${seededCount} filing(s) for new tickers. ${newFilingsCount} new filing(s) notified.`,
    )
  } else {
...
function parseAtomEntries(xml) {
  ...
  while ((match = entryRegex.exec(xml)) !== null) {
    ...
    const accessionNumber =
      extractTag(entryXml, 'accession-number') || extractTag(entryXml, 'accession-nunber')
...
function loadWatchlist() {
  try {
    ...
    console.error(
      'Create a watchlist.json file with format: { "tickers": ["AAPL", "MSFT"], "formTypes": ["10-K", "10-Q", "8-K", "4"] }',
    )
    process.exit(1)
  }
}
