...
import { revokeToken, startOAuthFlow } from '../client/auth.js'
import { readConfig } from '../storage/config.js'
import { clearTokens, readTokens, writeTokens } from '../storage/tokens.js'
...
export function registerAuth(command: Command): void {
  ...
  command
    ...
    .action(async (options: OutputOptions) => {
      ...
      const expiryStatus =
        expiresIn > 5 ? c.green(`${expiresIn} min`) : c.red(`${expiresIn} min (expiring soon)`)
