...
export class ContextMonitor {
  ...
  configure(options) {
    ...
    if (options.compressThreshold != null)
      this.config.compress_threshold = options.compressThreshold
    if (options.emergencyThreshold != null)
      this.config.emergency_threshold = options.emergencyThreshold
  ...
  async compress() {
    ...
    this.session.token_count = compressed.reduce((sum, m) => sum + this.countTokens(m.content), 0)
