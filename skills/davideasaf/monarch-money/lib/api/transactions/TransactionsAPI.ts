...
import { logger, validateDateRange, validateTransactionId } from '../../utils'
...
export class TransactionsAPIImpl implements TransactionsAPI {
  ...
  async getTransactions(options: GetTransactionsOptions = {}): Promise<PaginatedTransactions> {
    ...
    logger.debug(
      `Retrieved ${data.allTransactions.results.length} transactions using web app schema`,
    )
  ...
  async updateTransactionSplits(
    ...
    if (splitResult.updateTransactionSplit.errors) {
      throw new Error(`Split failed: ${JSON.stringify(splitResult.updateTransactionSplit.errors)}`)
    }
  ...
  async createTransactionRule(data: CreateTransactionRuleInput): Promise<TransactionRule> {
    ...
    if (result.createTransactionRule.errors?.length > 0) {
      throw new Error(
        `Transaction rule creation failed: ${result.createTransactionRule.errors[0].messages.join(', ')}`,
      )
    }

    logger.info(
      'Transaction rule created successfully:',
      result.createTransactionRule.transactionRule.id,
    )
  ...
  async updateTransactionRule(
    ...
    if (result.updateTransactionRule.errors?.length > 0) {
      throw new Error(
        `Transaction rule update failed: ${result.updateTransactionRule.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async deleteTransactionRule(ruleId: string): Promise<boolean> {
    ...
    if (result.deleteTransactionRule.errors?.length > 0) {
      throw new Error(
        `Transaction rule deletion failed: ${result.deleteTransactionRule.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async deleteAllTransactionRules(): Promise<boolean> {
    ...
    if (result.deleteAllTransactionRules.errors?.length > 0) {
      throw new Error(
        `Delete all transaction rules failed: ${result.deleteAllTransactionRules.errors[0].messages.join(', ')}`,
      )
    }

    logger.info(
      'All transaction rules deleted successfully:',
      result.deleteAllTransactionRules.deletedCount,
    )
  ...
  async createTransactionCategory(
    ...
    if (result.createCategory.errors?.length > 0) {
      throw new Error(
        `Category creation failed: ${result.createCategory.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async updateTransactionCategory(
    ...
    if (result.updateCategory.errors?.length > 0) {
      throw new Error(
        `Category update failed: ${result.updateCategory.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async deleteTransactionCategory(categoryId: string): Promise<boolean> {
    ...
    if (result.deleteCategory.errors?.length > 0) {
      throw new Error(
        `Category deletion failed: ${result.deleteCategory.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async createTransactionTag(data: CreateTransactionTagInput): Promise<TransactionTag> {
    ...
    if (result.createTransactionTag.errors?.length > 0) {
      throw new Error(
        `Tag creation failed: ${result.createTransactionTag.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async setTransactionTags(transactionId: string, tagIds: string[]): Promise<Transaction> {
    ...
    if (result.setTransactionTags.errors?.length > 0) {
      throw new Error(
        `Set transaction tags failed: ${result.setTransactionTags.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async getRecurringStreams(options: GetRecurringStreamsOptions = {}): Promise<any[]> {
    const { includeLiabilities = true, includePending = true, filters } = options
  ...
  async getAllRecurringTransactionItems(
    options: GetAllRecurringTransactionItemsOptions = {},
  ): Promise<any[]> {
  ...
  async markStreamAsNotRecurring(streamId: string): Promise<boolean> {
    ...
    if (result.markAsNotRecurring.errors?.length > 0) {
      throw new Error(
        `Mark as not recurring failed: ${result.markAsNotRecurring.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async bulkUpdateTransactions(data: BulkUpdateTransactionsInput): Promise<BulkUpdateResult> {
    ...
    if (result.bulkUpdateTransactions.errors && result.bulkUpdateTransactions.errors.length > 0) {
      throw new Error(
        `Bulk update failed: ${result.bulkUpdateTransactions.errors[0].messages.join(', ')}`,
      )
    }
  ...
  async getHiddenTransactions(
    options: GetHiddenTransactionsOptions = {},
