...
export async function linkCommand(file: string | undefined, options: LinkOptions): Promise<void> {
  ...
  if (options.rebuild) {
    ...
    console.log(
      `✓ Rebuilt backlinks (${result.backlinks.size} targets, ${result.linkCount} links${orphanSuffix})`,
    )
    return
  }
...
function logSuggestions(
  filePath: string,
  suggestions: Array<{ alias: string; path: string; line: number }>,
): void {
