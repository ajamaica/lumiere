...
export async function aap_verify({ server_url, solution }) {
  ...
  try {
    ...
    const llmCallback = solution ? async (challenge, nonce, type) => solution : null
...
export const tools = {
  ...
  aap_generate_proof: {
    ...
    parameters: {
      challenge_string: {
        type: 'string',
        description: 'Challenge prompt from server',
        required: true,
      },
      nonce: { type: 'string', description: 'Server-provided nonce', required: true },
      type: {
        type: 'string',
        description: 'Challenge type (poem, math, wordplay, reverse, description)',
      },
  ...
  aap_verify: {
    description:
      'Perform full AAP verification against a server (get challenge -> prove -> verify)',
    parameters: {
