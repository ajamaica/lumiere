...
const TOOLS: Record<string, ToolHandler> = {
  ...
  vector_search: {
    ...
    run: async (client, args) =>
      vectorSearch(client, args[0]!.trim(), args[1] ? parseInt(args[1], 10) : undefined),
  },
  get_agent: {
  ...
  register_agent: {
    ...
    run: async (client, args) => registerAgent(client, args[0]!, args[1], args[2], args[3]),
  },
  start_conversation: {
    validate: (args) =>
  ...
  send_message: {
    ...
    run: async (client, args) => sendMessage(client, args[0]!.trim(), args[1]!.trim()),
  },
  get_history: {
