...
async function textToSpeech(text, options = {}) {
  ...
  if (!validateProsodyValue(pitch)) {
    throw new Error(
      `Invalid pitch value: "${pitch}". Must be "default" or a percentage (e.g., "+10%", "-20%")`,
    )
  }
  if (!validateProsodyValue(rate)) {
    throw new Error(
      `Invalid rate value: "${rate}". Must be "default" or a percentage (e.g., "+10%", "-20%")`,
    )
  }
  if (!validateProsodyValue(volume)) {
    throw new Error(
      `Invalid volume value: "${volume}". Must be "default" or a percentage (e.g., "+10%", "-20%")`,
    )
  }
