...
function printQRCode(data, outputFile = null) {
  try {
    ...
    try {
      if (outputFile) {
        execSync(
          `python3 -c "import qrcode; qr=qrcode.make('${data}'); qr.save('${outputFile}')"`,
          {
            stdio: ['pipe', 'pipe', 'ignore'],
          },
        )
      ...
      const qr = execSync(
        `python3 -c "import qrcode; qr=qrcode.QRCode(border=1); qr.add_data('${data}'); qr.print_ascii(invert=True)"`,
        {
          encoding: 'utf8',
          stdio: ['pipe', 'pipe', 'ignore'],
        },
      ...
      if (invoice.settledAt && invoice.settledAt > 0) {
        ...
        if (global.notifyTarget) {
          try {
            ...
            execSync(
              `clawdbot message send --channel ${channel} --target "${global.notifyTarget}" --message "${msg}"`,
              {
                stdio: 'ignore',
              },
            )
  ...
  try {
    ...
    if (lni.needsResolution(destination)) {
      ...
      if (!confirmed) {
        ...
        console.log('')
        console.log(
          `⚡ ${info.destinationType === 'lightning_address' ? 'Lightning Address' : 'LNURL'} Payment (CONFIRM BEFORE PAYING)`,
        )
        console.log('='.repeat(40))
  ...
  if (amountMatch) {
    ...
    switch (unit) {
      ...
      case 'n':
        ...
        break // nano-bitcoin
      case 'p':
        amountSats = num / 10000
        break // pico-bitcoin
      default:
        amountSats = num * 100000000 // full bitcoin
    }
  }
    ...
    try {
      ...
      if (result.offer_paths && result.offer_paths.length > 0) {
        ...
        for (let i = 0; i < result.offer_paths.length; i++) {
          ...
          console.log(
            `  Path ${i + 1}: ${hops} hop${hops !== 1 ? 's' : ''}, first node: ${(p.first_node_id || '').substring(0, 20)}...`,
          )
        }
      }
...
async function listTransactions(node, limit = 10) {
  try {
    ...
    for (const tx of txns) {
      ...
      console.log(
        `${direction} ${settled} ${amountSats.toLocaleString()} sats - ${tx.description || '(no memo)'}`,
      )
    }
      ...
      case 'pay':
        ...
        if (payType === 'lightning_address' || payType === 'lnurl') {
          if (!payAmount || payAmount <= 0) {
            console.log(
              `⚡ ${payType === 'lightning_address' ? 'Lightning Address' : 'LNURL'} Detected`,
            )
            console.log('='.repeat(40))
            ...
            console.log(
              `Example: /lightning pay ${invoicePay.substring(0, 30)}${invoicePay.length > 30 ? '...' : ''} 100`,
            )
            break
          }
      ...
      case 'confirm':
        ...
        if (confirmType === 'lightning_address' || confirmType === 'lnurl') {
          if (!confirmAmount || confirmAmount <= 0) {
            console.error(
              `${confirmType === 'lightning_address' ? 'Lightning Address' : 'LNURL'} requires an amount: /lightning confirm <destination> <amount_sats>`,
            )
            process.exit(1)
          }
          ...
          if (!confirmAmount || confirmAmount <= 0) {
            console.error(
              'BOLT12 offers require an amount: /lightning confirm <offer> <amount_sats>',
            )
            process.exit(1)
          }
