...
const KNOWN_SERVICES: X402Service[] = [
  {
    name: 'Browserbase',
    description:
      'Cloud browser automation platform. Spin up headless browsers for web scraping, testing, and automation.',
  ...
  },
  {
    name: 'Firecrawl',
    description:
      'Web scraping and search API. Extract structured data from websites with pay-per-search pricing.',
...
async function fetchLiveServices(): Promise<X402Service[] | null> {
  ...
  try {
    ...
    const response = await fetch('https://api.thirdweb.com/mcp?tools=listPayableServices', {
      method: 'GET',
      headers: {
...
export { fetchLiveServices, KNOWN_SERVICES, X402Service }
