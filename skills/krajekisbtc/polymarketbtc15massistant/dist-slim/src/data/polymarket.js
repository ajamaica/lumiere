...
function marketHasSeriesSlug(market, seriesSlug) {
  ...
  if (String(market.seriesSlug ?? '').toLowerCase() === String(seriesSlug).toLowerCase())
    return true
  return false
}
...
export async function fetchBtc15mMarketSnapshot() {
  ...
  const upIndex = outcomes.findIndex(
    (x) => String(x).toLowerCase() === cfg.upOutcomeLabel.toLowerCase(),
  )
  const downIndex = outcomes.findIndex(
    (x) => String(x).toLowerCase() === cfg.downOutcomeLabel.toLowerCase(),
  )
...
export function summarizeOrderBook(book, depthLevels = 5) {
  ...
  const bidLiquidity = bids
    .slice(0, depthLevels)
    .reduce((acc, x) => acc + (toNumber(x.size) ?? 0), 0)
