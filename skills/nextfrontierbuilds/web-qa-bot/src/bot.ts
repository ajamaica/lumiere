...
import { detectModals, diffSnapshots, resolveRef } from './utils/snapshot.js'
import { retry, sleep, waitFor } from './utils/wait.js'
...
export class QABot {
  ...
  private async executeStep(step: TestStep): Promise<void> {
    ...
    if (step.expectText) {
      await this.snapshot()
      this.expectText(step.expectText.ref, step.expectText.text, {
        contains: step.expectText.contains,
      })
    }
    ...
    try {
      ...
      try {
        const screenshotPath = await this.screenshot(
          `failure-${test.name.replace(/\s+/g, '-')}.png`,
        )
  ...
  async generateReport(
    output: string,
    options: { format?: 'markdown' | 'pdf' | 'json'; company?: string } = {},
  ): Promise<string> {
