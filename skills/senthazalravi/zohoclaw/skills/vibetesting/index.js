...
class VibeTesting {
  ...
  async runFunctionalTests() {
    ...
    console.log(
      `  📊 Found ${dom.forms.length} forms, ${dom.inputs.length} inputs, ${dom.buttons.length} buttons`,
    )
  ...
  async runAccessibilityTests() {
    ...
    const totalIssues =
      issues.critical.length + issues.serious.length + issues.moderate.length + issues.minor.length
  ...
  async runPerformanceTests() {
    ...
    metrics.cssRequests = (
      content.match(/<link[^>]*rel=["']stylesheet["'][^>]*href=["']([^"']*)["'][^>]*>/gi) || []
    ).length
    ...
    this.results.tests.performance = {
      ...
      details: [
        ...
        {
          name: 'Page Size',
          value: `${(metrics.pageSize / 1024).toFixed(1)} KB`,
          score: sizeScore,
        },
  ...
  async runVisualTests() {
    ...
    const viewportMatch = this.pageContent.match(
  ...
  async runSecurityTests() {
    ...
    if (this.config.targetUrl.startsWith('https://')) {
      ...
      findings.push({
        ...
        severity: 'critical',
        title: 'Missing HTTPS',
        description: 'Site is not secure',
      })
    }

    const cspMatch = this.pageContent.match(
  ...
  async generateReport() {
    ...
    const testResults = Object.values(this.results.tests).filter(
      (t) => typeof t === 'object' && t.score !== undefined,
    )
    ...
    this.results.summary.totalTests =
      this.results.tests.functional.passed + this.results.tests.functional.failed
