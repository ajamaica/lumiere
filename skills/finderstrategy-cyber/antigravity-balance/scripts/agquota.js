...
async function findAntigravityProcess(processName) {
  ...
  if (process.platform === 'win32') {
    ...
    // macOS/Linux: use ps
    try {
      const result = await execAsync(`ps -ww -eo pid,args | grep "${processName}" | grep -v grep`, {
        timeout: 5000,
      })
      stdout = result.stdout
    } catch (e) {
...
function displayQuota(response) {
  ...
  for (const model of modelsWithQuota) {
    ...
    console.log(
      `  ${colors.bold}${label.padEnd(30)}${colors.reset} ${color}${String(percent).padStart(3)}%${colors.reset} ${bar}  ⏱️  ${resetDisplay}`,
    )
  }
