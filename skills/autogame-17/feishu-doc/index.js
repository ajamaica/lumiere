...
async function readDoc(docToken) {
  const rawContent = await fetchWithAuth(
  ...
  const docInfo = await fetchWithAuth(
  ...
  const blocks = await fetchWithAuth(
...
async function writeDoc(docToken, content) {
  ...
  const blocksRes = await fetchWithAuth(
    `https://open.feishu.cn/open-apis/docx/v1/documents/${docToken}/blocks`,
  )
  ...
  const children =
    blocksData.data?.items?.filter((b) => b.block_type !== 1).map((b) => b.block_id) || []
