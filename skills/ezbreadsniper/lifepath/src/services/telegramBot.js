...
Ready to begin?
  ...
  async advanceYear(ctx, session, life) {
    ...
    if (this.lifeService.shouldDie(nextAge, life.health)) {
      ...
      const deathChapter = await this.storyGen.generateDeathChapter(life, nextAge, cause)
    ...
    const yearChapter = await this.storyGen.generateYearChapter(life, currentYear, nextAge, history)
