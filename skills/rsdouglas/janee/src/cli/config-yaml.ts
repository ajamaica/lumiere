...
import { decryptSecret, encryptSecret, generateMasterKey } from '../core/crypto'
 ...
 */
export function addServiceYAML(name: string, baseUrl: string, auth: AuthConfig): void {
 ...
 */
export function addCapabilityYAML(name: string, capConfig: CapabilityConfig): void {
  const config = loadYAMLConfig()
