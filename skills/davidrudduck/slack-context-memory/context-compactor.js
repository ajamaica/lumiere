...
async function buildCompactedContext(options = {}) {
  ...
  if (includeRecentMessages && channelId) {
    ...
    const client = new Client({ connectionString: DB_URL })
    await client.connect()
    const nameResult = await client.query('SELECT name FROM slack_channels WHERE id = $1', [
      channelId,
    ])
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.log('\n---')
  console.log(
    `📊 Stats: ${result.stats.summaryCount} summaries + ${result.stats.recentMessageCount} recent messages`,
  )
