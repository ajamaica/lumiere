...
import { EncryptionService, logger, MonarchAuthError } from '../../utils'
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (errorData.You === 'Shall Not Pass') {
          throw new MonarchAuthError(
            'IP address temporarily blocked. Please wait or try from different network.',
          )
        }
        ...
        throw new MonarchAuthError(
          `Authentication failed: ${errorData.detail || response.statusText}`,
        )
      }
    } catch (error) {
