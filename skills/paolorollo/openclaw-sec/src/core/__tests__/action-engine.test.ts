...
describe('ActionEngine', () => {
  ...
  describe('determineAction', () => {
    ...
    it('should throw error for invalid severity', async () => {
      ...
      await expect(engine.determineAction('INVALID' as any, 'user-123')).rejects.toThrow(
        'Invalid severity',
      )
    })
    ...
    it('should throw error for null userId', async () => {
      ...
      await expect(engine.determineAction(Severity.LOW, null as any)).rejects.toThrow(
        'User ID is required',
      )
    })
