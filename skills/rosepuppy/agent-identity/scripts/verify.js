...
import { CONTRACTS, DEFAULT_CHAIN, getPublicClient, output, REGISTRY_ABI } from './config.js'
...
async function main() {
  ...
  if (opts.identity) {
    ...
    if (contracts.registry === '0x0000000000000000000000000000000000000000') {
      throw new Error(
        `Registry not deployed on ${opts.chain}. Use off-chain verification with --signer.`,
      )
    }
