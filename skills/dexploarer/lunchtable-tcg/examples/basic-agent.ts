...
class LTCGClient {
  ...
  async getAgentInfo() {
    return this.get<{ agentId: string; name: string; elo: number; wins: number; losses: number }>(
      '/me',
    )
  }
  ...
  async enterMatchmaking(mode: 'casual' | 'ranked' = 'casual') {
    return this.post<{ lobbyId: string; status: string; mode: string }>('/matchmaking/enter', {
      mode,
    })
  }
  ...
  async setSpellTrap(gameId: string, cardId: string) {
    return this.post<{ success: boolean; cardType: string }>('/games/actions/set-spell-trap', {
      gameId,
      cardId,
    })
  }
  ...
  async endTurn(gameId: string) {
    return this.post<{ success: boolean; newTurnPlayer?: string }>('/games/actions/end-turn', {
      gameId,
    })
  }

  async surrender(gameId: string) {
    return this.post<{ success: boolean; gameEnded: boolean }>('/games/actions/surrender', {
      gameId,
    })
  }
}
...
class BasicAgent {
  ...
  private async playTurn(gameId: string): Promise<void> {
    ...
    try {
          ...
          for (const monster of updatedState.myBoard) {
            ...
            if (updatedState.opponentBoard.length > 0) {
              ...
              if (target && monster.attack > target.attack) {
                this.log(
                  `${monster.name} (${monster.attack}) attacking ${target.name} (${target.attack})`,
                )
      ...
      this.log(
        `Error playing turn: ${error instanceof Error ? error.message : String(error)}`,
        'error',
      )
  ...
  async run() {
    ...
    try {
      ...
      this.log(
        `Connected as: ${agentInfo.name} (ELO: ${agentInfo.elo}, Record: ${agentInfo.wins}W-${agentInfo.losses}L)`,
      )
    } catch (error) {
      this.log(
    ...
    try {
      ...
      this.log(
        `Failed to enter matchmaking: ${error instanceof Error ? error.message : String(error)}`,
        'error',
      )
      return
    }
    ...
    while (true) {
      try {
        ...
        consecutiveErrors++
        this.log(
          `Error in game loop: ${error instanceof Error ? error.message : String(error)}`,
          'error',
        )
...
export { BasicAgent, LTCGClient, registerAgent }
