...
class JsonDatabase {
  ...
  async getJobsByApi(apiId, limit = 100) {
    return this.data.jobs.filter((j) => j.api_id === apiId).slice(-limit)
  }

  async getJobsByStatus(status, limit = 100) {
    return this.data.jobs.filter((j) => j.status === status).slice(-limit)
  }
  ...
  async getApiStats(apiId) {
    return (
      this.data.api_stats.find((s) => s.api_id === apiId) || {
        api_id: apiId,
        total_calls: 0,
        successful_calls: 0,
        failed_calls: 0,
