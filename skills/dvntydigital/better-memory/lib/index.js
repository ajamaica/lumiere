...
export class ContextGuardian {
  ...
  async summarizeAndStore(messages, options = {}) {
    if (!this.initialized) await this.initialize()
    const { compressed, scored } = await this.compressor.compress(messages, options.targetTokens)
    const storedCount = await this.compressor.storeCompressed(scored, options.sessionId)
