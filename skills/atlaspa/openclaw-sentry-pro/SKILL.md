...
metadata:
  {
    'openclaw':
      { 'emoji': '🔑', 'requires': { 'bins': ['python3'] }, 'os': ['darwin', 'linux', 'win32'] },
  }
---
...
Auto-generate/update `.gitignore` with common secret patterns (.env, _.pem, _.key, credentials.json, etc.) and create a `.sentry-policy.json` policy file listing which patterns to enforce.
...
| **GitHub**      | PATs (ghp*, gho*, ghs*, ghr*, github*pat*)                      |
| **Slack**       | Bot/user tokens (xox...), webhooks                              |
| **Stripe**      | Secret keys (sk*live*), publishable keys                        |
| **OpenAI**      | API keys (sk-...)                                               |
