...
function parseScan(text) {
  ...
  for (const raw of lines) {
    ...
    if (inServices) {
      ...
      const svcMatch = line.match(
        /^-\s*Protocol:\s*([^,]+),\s*Port:\s*(\d+),\s*Credentials:\s*([^,]+),\s*Requires Password:\s*([^,]+),\s*Password:\s*([^,]+),\s*Pairing:\s*(.+)$/,
      )
      if (svcMatch) {
        cur.services.push({
...
console.log(
  JSON.stringify({ scannedAt: new Date().toISOString(), timeoutSec: timeout, devices }, null, 2),
)
