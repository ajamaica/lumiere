...
class ChallengeService {
  ...
  async joinChallenge(userId, challengeId, lifeId) {
    ...
    await this.db.query('UPDATE challenges SET participants = participants + 1 WHERE id = $1', [
      challengeId,
    ])
  ...
  evaluateChallenge(challenge, lifeStats) {
    ...
    switch (challenge.type) {
      case 'survival':
        return (
          lifeStats.age >= criteria.min_age &&
          lifeStats.alive &&
      ...
      case 'achievement':
        ...
        if (criteria.min_intelligence && lifeStats.intelligence < criteria.min_intelligence)
          return false
        if (criteria.before_age && lifeStats.age > criteria.before_age) return false
        if (criteria.max_starting_wealth && lifeStats.startingWealth > criteria.max_starting_wealth)
          return false
        return true
      ...
      case 'dynasty':
        return (
          lifeStats.dynastyGenerations >= criteria.min_generations &&
  ...
  calculateProgress(challenge, lifeStats) {
    ...
    return {
      ...
      current: Math.min(lifeStats.age, total),
      total,
    }
  }
