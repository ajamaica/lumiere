...
const INPUT =
  args.find((arg) => !arg.startsWith('--')) ?? 'https://www.youtube.com/watch?v=fdQtJ6TD5fE'
...
function folderTreeToString(node: FolderNode, indent: string = ''): string {
  ...
  if (node.type === 'transcript' && node.metadata) {
    ...
    lines.push(
      `${indent}   Theme: ${node.metadata.theme.primaryTheme} > ${node.metadata.theme.subTheme}`,
    )
    ...
    if (node.children) {
      for (const child of node.children) {
        lines.push(
          ...folderTreeToString(child, indent + '  ')
            .split('\n')
            .filter(Boolean),
        )
      }
...
async function getFileDuration(
  filePath: string,
): Promise<{ duration?: number; duration_string?: string }> {
  try {
...
async function main() {
  ...
  if (isUrl(INPUT)) {
    ...
    if (sourceMetadata.duration_string)
      console.log(`   Duration: ${sourceMetadata.duration_string}`)
    if (sourceMetadata.view_count)
      console.log(`   Views: ${sourceMetadata.view_count.toLocaleString()}`)
