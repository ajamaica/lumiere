...
import { Action, SecurityConfig, Severity } from '../types'
...
export class ConfigManager {
  ...
  static validateConfig(config: SecurityConfig): void {
    ...
    if (!validSensitivities.includes(config.sensitivity)) {
      throw new Error(
        `Invalid sensitivity level: ${config.sensitivity}. Must be one of: ${validSensitivities.join(', ')}`,
      )
    }
    ...
    if (config.actions) {
      Object.entries(config.actions).forEach(([severity, action]) => {
        if (!validActions.includes(action)) {
          throw new Error(
            `Invalid action for ${severity}: ${action}. Must be one of: ${validActions.join(', ')}`,
          )
        }
      })
    }
    ...
    if (config.modules) {
      Object.entries(config.modules).forEach(([moduleName, moduleConfig]) => {
        if (moduleConfig.sensitivity && !validSensitivities.includes(moduleConfig.sensitivity)) {
          throw new Error(
            `Invalid sensitivity level for module ${moduleName}: ${moduleConfig.sensitivity}`,
          )
        }
      })
    }
