...
const CHALLENGE_TYPES = {
  nlp_math: (nonce) => {
    ...
    const templates = [
      {
        text: `Start with ${a}. Subtract ${b}. Multiply by ${c}. Divide by ${e}. Add ${d}. Final value?`,
        answer: ((a - b) * c) / e + d,
      },
      {
        text: `Compute: ((${a} + ${b}) × ${c} - ${d}) ÷ ${e}. Round to two decimals.`,
        answer: ((a + b) * c - d) / e,
      },
      {
        text: `Triple ${a}, halve it, add ${b}, subtract ${d}, multiply by ${e}. Result?`,
        answer: ((a * 3) / 2 + b - d) * e,
      },
    ]
  ...
  nlp_logic: (nonce) => {
    ...
    const templates = [
      ...
      },
      {
        ...
        answer: (() => {
          const cnt = [a, b, c, d].filter((n) => n % 3 === 0).length
          return cnt === 0 ? 'NONE' : cnt <= 2 ? 'FEW' : 'MANY'
        })(),
      },
    ]
...
app.get('/metrics', (req, res) => {
  ...
  res.json({
    ...
    successRate:
      metrics.verifications.success /
        (metrics.verifications.success + metrics.verifications.failed) || 0,
  })
})
