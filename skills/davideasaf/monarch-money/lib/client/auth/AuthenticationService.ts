...
export class AuthenticationService {
  ...
  constructor(baseUrl: string = 'https://api.monarchmoney.com', sessionStorage?: SessionStorage) {
      ...
      while (captchaRetryCount <= maxCaptchaRetries) {
        try {
          ...
          if (result.token) {
            ...
            throw new MonarchMFARequiredError(
              'Multi-factor authentication required but no MFA secret provided',
            )
          }
        } catch (error) {
  ...
  private async performLoginWithMFA(
    email: string,
    password: string,
    mfaSecretKey?: string,
  ): Promise<{
    ...
    if (response.status >= 400) {
      ...
      if (
        ...
        data.detail?.includes('Shall Not Pass') ||
  ...
  private async performMFAAuth(
    ...
    if (response.status >= 400) {
      ...
      if (response.status === 403) {
        throw new MonarchMFARequiredError(
          'Multi-factor authentication required or invalid MFA code',
        )
      }
