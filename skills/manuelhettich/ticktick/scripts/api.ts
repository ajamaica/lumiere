...
class TickTickAPI {
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error("Authentication expired. Please run 'ticktick auth' to re-authenticate.")
      }
  ...
  async getInboxData(): Promise<ProjectData> {
    ...
    const inbox = projects.find((p) => p.name.toLowerCase() === 'inbox' || p.kind === 'INBOX')
    if (!inbox) {
  ...
  async findProjectByName(name: string): Promise<Project | undefined> {
    ...
    return projects.find((p) => p.name.toLowerCase() === lowerName || p.id === name)
  }
