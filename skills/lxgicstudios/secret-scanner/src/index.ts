...
export async function scanDirectory(dir: string): Promise<SecretFinding[]> {
  const files = await glob(
    '**/*.{js,ts,jsx,tsx,py,rb,go,java,env,json,yml,yaml,toml,cfg,conf,ini}',
    {
      cwd: dir,
      absolute: true,
    ...
    messages: [
      {
        role: 'system',
        content:
          "You analyze potential secret leaks in code. For each finding, confirm if it's a real leak or false positive, rate severity, and suggest a fix. Be concise.",
      },
