...
if (!OPENCLAW_STORE_URL || !OPENCLAW_STORE_SECRET) {
  throw new Error(
    'Configuration Error: OPENCLAW_STORE_URL and OPENCLAW_STORE_SECRET environment variables are required.',
  )
}
...
async function callWoo(endpoint, payload) {
  try {
    ...
    const signature = crypto.createHmac('sha256', OPENCLAW_STORE_SECRET).update(body).digest('hex')
    ...
    if (error.response) {
      throw new Error(
        `Store Error ${error.response.status}: ${error.response.data.message || error.response.statusText}`,
      )
