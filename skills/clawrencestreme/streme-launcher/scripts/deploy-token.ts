...
async function main() {
  ...
  const [salt, predictedToken] = await publicClient.readContract({
    ...
    args: [tokenSymbol, account.address, CONTRACTS.TOKEN_FACTORY, CONTRACTS.WETH],
  })
  ...
  const allocations = []
  if (stakingPercent > 0) {
    allocations.push(createStakingAllocation(stakingPercent, stakingLockDays, stakingFlowDays))
    console.log(
      `Staking: ${stakingPercent}% (${stakingLockDays}d lock, ${stakingFlowDays}d stream)`,
    )
  }
