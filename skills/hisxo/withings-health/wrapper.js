    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (json.status !== 0)
            reject(`API Error Status: ${json.status} - ${JSON.stringify(json)}`)
...
async function getBodyComposition() {
  try {
    ...
    const measures = data.measuregrps
      .map((grp) => {
        const date = new Date(grp.date * 1000).toISOString()
        const result = { date }
        ...
        grp.measures.forEach((m) => {
          ...
          switch (m.type) {
            case 1:
              result.weight = value.toFixed(2) + ' kg'
              break
            case 6:
              result.fat_percent = value.toFixed(1) + '%'
              break
            case 8:
              result.fat_mass = value.toFixed(2) + ' kg'
              break
            case 76:
              result.muscle_mass = value.toFixed(2) + ' kg'
              break
            case 77:
              result.hydration = value.toFixed(1) + '%'
              break
            case 88:
              result.bone_mass = value.toFixed(2) + ' kg'
              break
          }
        })
