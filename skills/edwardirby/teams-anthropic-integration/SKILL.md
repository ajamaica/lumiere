...
app.on('message', async ({ send, activity }) => {
  ...
  const response = await model.send({ role: 'user', content: activity.text })
...
const prompt = new ChatPrompt(
  {
    instructions:
      'You are a helpful assistant with access to web search and content extraction. Use these tools to provide accurate, up-to-date information.',
