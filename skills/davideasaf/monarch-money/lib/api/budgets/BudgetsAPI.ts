...
import { BillsData, BudgetItem, CashFlowData, CashFlowSummary, Goal } from '../../types'
import { logger, validateDate, validateDateRange } from '../../utils'
...
export class BudgetsAPIImpl implements BudgetsAPI {
  ...
  async setBudgetAmount(params: BudgetAmountParams): Promise<BudgetItem> {
    ...
    if (result.updateBudgetItem.errors?.length > 0) {
      throw new Error(
        `Budget update failed: ${result.updateBudgetItem.errors[0].messages.join(', ')}`,
      )
    }
