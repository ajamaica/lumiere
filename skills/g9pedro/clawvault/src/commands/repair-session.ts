...
import { analyzeSession, type RepairResult, repairSession } from '../lib/session-repair.js'
import {
  findMainSession,
  findSessionById,
  getSessionFilePath,
...
export function formatRepairResult(
  ...
  if (result.parentRelinks.length > 0) {
    ...
    for (const relink of result.parentRelinks.slice(0, 5)) {
      lines.push(
        `  - Line ${relink.lineNumber}: parentId ${relink.oldParentId.slice(0, 8)}… → ${relink.newParentId === 'null' ? 'null' : relink.newParentId.slice(0, 8)}…`,
      )
    }
  ...
  if (dryRun) {
    lines.push(
      `Would remove ${result.removedCount} entries, relink ${result.relinkedCount} parent references.`,
    )
  } else {
    lines.push(
      `✅ Session repaired: removed ${result.removedCount} entries, relinked ${result.relinkedCount} parent references`,
    )
