...
import { createStyler, keyValueTable, OutputOptions, writeOutput } from '../utils/output.js'
...
export function registerActivity(command: Command): void {
    ...
    .action(async (date: string | undefined, options: ActivityOptions) => {
      ...
      console.log(
        keyValueTable([
          {
            key: 'Steps',
            value: `${summary.steps.toLocaleString()} / ${goals.steps.toLocaleString()} (${pct(summary.steps, goals.steps)})`,
          },
          {
            key: 'Calories Out',
            value: `${summary.caloriesOut.toLocaleString()} / ${goals.caloriesOut.toLocaleString()} (${pct(summary.caloriesOut, goals.caloriesOut)})`,
          },
          {
            key: 'Distance',
            value: `${summary.distances.find((d) => d.activity === 'total')?.distance.toFixed(2) ?? '0'} km`,
          },
          {
            key: 'Floors',
            value: `${summary.floors} / ${goals.floors} (${pct(summary.floors, goals.floors)})`,
          },
          {
            key: 'Active Minutes',
            value: `${summary.fairlyActiveMinutes + summary.veryActiveMinutes} min (${summary.veryActiveMinutes} very active)`,
          },
          { key: 'Sedentary Minutes', value: `${summary.sedentaryMinutes} min` },
          {
            key: 'Resting Heart Rate',
            value: summary.restingHeartRate != null ? `${summary.restingHeartRate} bpm` : 'N/A',
          },
        ]),
      )
    })
  ...
  activity
    ...
    .action(async (date: string | undefined, options: ActivityOptions) => {
      ...
      if (options.json) {
        await writeOutput(
          { date: resolvedDate, steps: data.summary.steps, goal: data.goals.steps },
          options,
        )
        return
      }
      ...
      console.log(
        `${resolvedDate}: ${data.summary.steps.toLocaleString()} steps (${pct} of ${goal.toLocaleString()} goal)`,
      )
    })
