...
- near-zero _output_ usage
- no surprise bills

Cons:

- loses automatic early warning (sub-agent death / stuck jobs)
- **important:** if the platform still injects a large conversation context into each heartbeat turn, you may still see high _input/cache_ tokens despite a tiny reply
