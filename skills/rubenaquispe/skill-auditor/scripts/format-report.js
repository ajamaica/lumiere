...
function formatReport(report) {
  ...
  if (report.summary.externalUrls.length > 0) {
    ...
    for (const url of report.summary.externalUrls) {
      try {
        ...
        domains.add(url.substring(0, 50))
      }
    }
  ...
  if (
    ...
    report.accuracyScore.undisclosed &&
    report.accuracyScore.undisclosed.length > 0
  ) {
  ...
  if (report.reputation && report.reputation.tier === 'known' && report.riskLevel !== 'CLEAN') {
    lines.push('')
    lines.push(
      `ℹ️ ${report.reputation.publisher} is a known publisher, but known ≠ safe. Review findings above.`,
    )
