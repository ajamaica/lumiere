...
async function promptHidden(question) {
  if (!process.stdin.isTTY) {
    throw cliError(
      'INVALID_VALUE',
      'cannot prompt for token (stdin is not a TTY). Use --stdin instead.',
    )
  }
...
program
  ...
  .action((maybeCmd) =>
    runOrExit((opts) => commands.listDevices({ ...opts, ...commandOpts(maybeCmd) })),
  )
    ...
    runOrExit((opts) => {
      ...
      if (action === 'set') {
        if (!capability || value === undefined) {
          throw cliError(
            'INVALID_VALUE',
            'usage: homeycli device <nameOrId> set <capability> <value>',
          )
        }
...
program
  ...
  .action((maybeCmd) =>
    runOrExit((opts) => commands.listFlows({ ...opts, ...commandOpts(maybeCmd) })),
  )
...
program
  ...
  .action((maybeCmd) =>
    runOrExit((opts) => commands.snapshot({ ...opts, ...commandOpts(maybeCmd) })),
  )
