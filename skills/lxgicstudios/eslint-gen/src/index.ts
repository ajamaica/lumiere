...
export async function generateConfig(files: string[], format: string): Promise<string> {
  const samples = files
    .map((f) => readFileSync(f, 'utf-8').slice(0, 800))
    .join('\n---\n')
    .slice(0, 8000)

  let pkgContent = '{}'
  try {
    pkgContent = readFileSync(join(process.cwd(), 'package.json'), 'utf-8')
  } catch {}
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You analyze code samples and generate an ESLint config that matches the project's existing patterns.
