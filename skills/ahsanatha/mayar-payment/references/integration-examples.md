...
setInterval(async () => {
  ...
  const paid = data.data.find((t) => t.paymentLinkId === invoiceId && t.status === 'paid')
...
expiringSoon.forEach((member) => {
  const renewalInvoice = createInvoice({
    ...
    items: [{ quantity: 1, rate: 500000, description: 'Premium Membership - 1 Month' }],
  })
...
const invoice = createInvoice({
  ...
  items: [{ quantity: 1, rate: 2500000, description: 'Full Stack Dev Course' }],
})
...
function createDonationInvoice(donorInfo, amount, program) {
  return createInvoice({
    ...
    items: [{ quantity: 1, rate: amount, description: `${program} - Donation` }],
  })
}
...
${transactions
  .slice(0, 5)
  .map((t) => `• ${t.paymentLinkDescription}: Rp ${t.amount.toLocaleString('id-ID')}`)
  .join('\n')}
`.trim()
