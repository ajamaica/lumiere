...
function analyzeLogs(agentFilter = null, failedOnly = false) {
  ...
  const avgDuration =
    logs.filter((l) => l.duration_ms).reduce((sum, l) => sum + l.duration_ms, 0) / (completed || 1)
