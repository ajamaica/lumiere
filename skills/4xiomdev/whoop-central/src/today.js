...
async function getToday(opts = {}) {
  ...
  if (recovery?.score) {
    lines.push(
      `Recovery: ${recovery.score.recovery_score}% (HRV ${recovery.score.hrv_rmssd_milli?.toFixed(1)} ms, RHR ${recovery.score.resting_heart_rate} bpm)`,
    )
  }
  if (sleep?.score?.stage_summary) {
    const st = sleep.score.stage_summary
    const asleepMin = Math.round(
      ((st.total_in_bed_time_milli || 0) - (st.total_awake_time_milli || 0)) / 60000,
    )
    lines.push(
      `Sleep: ${fmtMins(asleepMin)} (Perf ${sleep.score.sleep_performance_percentage}%, Eff ${sleep.score.sleep_efficiency_percentage?.toFixed(1)}%)`,
    )
  }
  if (cycle?.score) {
    lines.push(
      `Strain: ${cycle.score.strain?.toFixed(1)} (AvgHR ${cycle.score.average_heart_rate} / MaxHR ${cycle.score.max_heart_rate})`,
    )
  }
  if (workout) {
    ...
    lines.push(
      `Last workout: ${name} (${mins != null ? fmtMins(mins) : 'duration N/A'}, Strain ${wStrain != null ? wStrain.toFixed(1) : 'N/A'})`,
    )
  }
