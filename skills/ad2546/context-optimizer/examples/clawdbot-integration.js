...
async function exampleUsage() {
  ...
  await contextManager.addMessage(
    'assistant',
    "Of course! I'd be happy to help with coding. What language are you using?",
    5,
  )
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (i % 20 === 0 && i > 0) {
      ...
      console.log(
        `After ${i} messages: ${status.bufferSize} messages, ${status.bufferTokens} tokens`,
      )
      console.log(
        `Health: ${status.health}, Tokens saved: ${status.detailedStats.totalTokensSaved}`,
      )
    }
  }
  ...
  console.log(
    `Total tokens: ${status.bufferTokens}/${status.tokens.limit} (${status.tokens.percentage}%)`,
  )
