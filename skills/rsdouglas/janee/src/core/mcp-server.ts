    ...
    try {
      switch (name) {
        ...
        case 'execute': {
          ...
          const session = sessionManager.createSession(cap.name, cap.service, ttlSeconds, {
            reason,
          })
 ...
 */
export function makeAPIRequest(targetUrl: URL, request: APIRequest): Promise<APIResponse> {
