...
import { ExitCode, WhoopError } from '../utils/errors.js'
import {
  clearTokens,
  getTokenStatus,
  getValidTokens,
...
const SCOPES =
  'read:profile read:body_measurement read:workout read:recovery read:sleep read:cycles offline'
...
export function status(): void {
  ...
  if (!tokenStatus.authenticated) {
    console.log(
      JSON.stringify(
        { authenticated: false, message: 'Not logged in. Run: whoopskill auth login' },
        null,
        2,
      ),
    )
