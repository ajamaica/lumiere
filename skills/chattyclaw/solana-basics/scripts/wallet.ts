...
import { getRpcUrl, getWalletsDir, loadConfig } from './config.js'
...
function encryptPrivateKey(
  ...
  password: string,
): Omit<EncryptedWallet, 'name' | 'publicKey' | 'createdAt'> {
...
function decryptPrivateKey(wallet: EncryptedWallet, password: string): Uint8Array {
  ...
  const decipher = createDecipheriv(ALGORITHM, key, Buffer.from(wallet.iv, 'base64'))
...
export function listWallets(): Array<{ name: string; publicKey: string; createdAt: string }> {
  ...
  return files.map((file) => {
    const wallet: EncryptedWallet = JSON.parse(readFileSync(join(walletsDir, file), 'utf-8'))
    return {
      name: wallet.name,
      publicKey: wallet.publicKey,
  ...
  const tokens =
    result.items
      ...
      .map((item: any) => ({
        ...
        uiAmount:
          parseInt(item.token_info?.balance || '0') / Math.pow(10, item.token_info?.decimals || 0),
      })) || []
