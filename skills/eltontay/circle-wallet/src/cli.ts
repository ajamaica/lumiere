...
program
  ...
  .action(async (options: { apiKey: string; env: string }) => {
    ...
    const defaultWalletFile = path.join(
      process.env.HOME || '~',
      '.openclaw',
      'circle-wallet',
      'default-wallet.txt',
  ...
  .action(async (name: string, options: { chain?: string }) => {
    try {
      ...
      if (walletCount === 0) {
        ...
        console.log(
          '  - Send between wallets: circle-wallet send <to-address> <amount> --from <from-address>',
        )
      }
    } catch (error) {
...
program
  ...
  .option(
    '-f, --from <wallet-identifier>',
    'From wallet (ID or address) - uses default if not provided',
  )
...
program
  ...
  .option(
    '--chain <blockchain>',
    'Blockchain to request tokens on (e.g., ARC-TESTNET, BASE-SEPOLIA)',
  )
...
program
  ...
  .action((options: { showTokens?: boolean; mainnet?: boolean; testnet?: boolean }) => {
    ...
    console.log(
      `Total: ${chains.length} chains (${mainnets.length} mainnet, ${testnets.length} testnet)`,
    )
  })
