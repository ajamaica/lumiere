...
program.name('janee').description('Secrets management for AI agents').version('0.1.0')
...
program.command('remove <service>').description('Remove a service from Janee').action(removeCommand)

program.command('serve').description('Start Janee MCP server').action(serveCommand)

program.command('list').description('List configured services').action(listCommand)
