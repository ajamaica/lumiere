...
async function listGogAccounts(): Promise<GogAccount[]> {
  try {
    ...
    console.warn(
      '[luma] Unable to list gog accounts:',
...
export async function syncEventToGoogleCalendar(
  ...
  if (!account) {
    return {
      success: false,
      message:
        'Multiple or zero Google accounts found. Provide an account email or set GOG_ACCOUNT.',
    }
  }
  ...
  const end =
    toRfc3339(event.end_time) || new Date(new Date(start).getTime() + 60 * 60 * 1000).toISOString()
