...
class ImprovementTracker {
  ...
  getRevertedChanges() {
    try {
      const log = execFileSync('git', ['log', '--oneline', '--grep=revert', '-5', '--', '.'], {
        cwd: this.projectPath,
        encoding: 'utf-8',
        timeout: 5000,
      })
  ...
  getEffectTrend() {
    ...
    const trend = {
      testDelta:
        curr.metrics.testPassed -
        prev.metrics.testPassed -
    ...
    trend.verdict =
      trend.overall > 0 ? '📈 improved' : trend.overall < 0 ? '📉 degraded' : '➡️ unchanged'
...
class RuntimeSignals {
    ...
    try {
      const result = execFileSync(
        ...
        [
          ...
          '--exclude-dir=.git',
          '-E',
          '(catch\\s*\\(|console\\.error|throw new Error)',
          '.',
        ],
        {
          cwd: this.projectPath,
  ...
  getUnusedExports() {
    try {
      ...
      const exportsResult = execFileSync(
        'grep',
        [
          '-rh',
          '--include=*.js',
          ...
          '--exclude-dir=.git',
          '-E',
          'module\\.exports\\s*=|exports\\.[a-zA-Z]',
          '.',
        ],
        {
          cwd: this.projectPath,
      ...
      for (const line of lines) {
        ...
        if (bracketMatch) {
          ...
          names.forEach((n) => {
            if (n && /^[a-zA-Z_]/.test(n)) exportedNames.add(n)
          })
        }
        // Match: exports.name =
...
class HealthChecker {
  ...
  checkDocumentation(health) {
    ...
    if (fs.existsSync(readmePath)) {
      ...
      if (!content.includes('## ') && !content.includes('# ')) {
        docs.issues.push({
          type: 'readme_no_structure',
          message: 'README.md lacks heading structure',
        })
      }
    } else {
  ...
  checkNodeJS(health, pkg) {
    ...
    // 2. Test check
    const hasTestScript =
      pkg.scripts && pkg.scripts.test && !pkg.scripts.test.includes('no test specified')
...
`
  ...
  formatDocIssues(issues) {
    ...
    const issueList = issues.map((e, i) => `${i + 1}. [${e.type}] ${e.message}`).join('\n')
