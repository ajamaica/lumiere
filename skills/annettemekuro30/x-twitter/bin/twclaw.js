...
const mockTweets = [
  mockTweet(
    '1893001',
    '@elonmusk',
    'The future of AI is incredibly exciting. We are building things that will change everything.',
  ),
  mockTweet(
    '1893002',
    '@openai',
    'Introducing our latest research on reasoning models. Blog post in thread. 🧵',
  ),
  mockTweet(
    '1893003',
    '@steipete',
    'Just shipped a massive OpenClaw update — 50 new skills, faster inference, and better memory. Try it!',
  ),
  mockTweet(
    '1893004',
    '@github',
    'GitHub Copilot now supports multi-file editing. The future of dev is here.',
  ),
  mockTweet('1893005', '@elonmusk', 'Starship flight test 7 was a success. Next stop: Mars.'),
  mockTweet(
    '1893006',
    '@openai',
    'We are hiring researchers across safety, alignment, and capabilities. Apply now.',
  ),
  mockTweet(
    '1893007',
    '@steipete',
    'OpenClaw can now order food, control your lights, and manage your calendar. All locally. 🦞',
  ),
  mockTweet('1893008', '@github', 'Over 100 million developers now call GitHub home. Thank you.'),
  mockTweet(
    '1893009',
    '@elonmusk',
    'FSD v13 rolling out this week. Biggest neural net update yet.',
  ),
...
switch (command) {
    ...
    if (process.env.TWITTER_BEARER_TOKEN) {
      ...
      console.log(
        process.env.TWITTER_API_KEY
          ? '✓ TWITTER_API_KEY is set (write ops enabled)'
          : '⚠ TWITTER_API_KEY not set (read-only mode)',
      )
    } else {
  ...
  case 'read': {
    ...
    const tweet =
      mockTweets.find((t) => t.id === id) ||
      mockTweet(id, '@elonmusk', 'This is the requested tweet content. [mock data]')
  ...
  case 'thread': {
    ...
    const threadTweets = [
      ...
      mockTweet(
        '1893002a',
        '@openai',
        '1/ We trained a new model that can reason step-by-step through complex problems.',
      ),
      mockTweet(
        '1893002b',
        '@openai',
        '2/ It achieves state-of-the-art on math, coding, and science benchmarks.',
      ),
      mockTweet(
        '1893002c',
        '@openai',
        '3/ Read the full paper and try the model: https://openai.com/research [mock]',
      ),
    ]
  ...
  case 'replies': {
    ...
    const replyTweets = Array.from({ length: Math.min(count, 5) }, (_, i) =>
      mockTweet(
        `r${i}`,
        ['@dev_jane', '@ai_bob', '@coder42', '@techfan', '@oss_lover'][i],
        [
  ...
  case 'user': {
    ...
    const user = mockUsers[handle] || {
      name: handle.slice(1),
      handle,
      followers: 1234,
      following: 567,
      bio: 'Twitter user. [mock data]',
      verified: false,
    }
  ...
  case 'user-tweets': {
    ...
    if (tweets.length === 0) {
      console.log(
        fmtTweet(mockTweet('u1', handle, 'Latest thoughts from this account. [mock data]')),
      )
    } else {
  ...
  case 'search': {
    checkAuth()
    const query = subarg || ''
    const results = mockTweets.filter(
      (t) =>
  ...
  case 'reply': {
    ...
    if (!replyTo || !replyText) {
      console.error('Error: usage: twclaw reply <tweet-id> "text"')
      process.exit(1)
    }
    const newId = Date.now().toString().slice(-7)
  ...
  case 'quote': {
    ...
    if (!quoteId || !quoteText) {
      console.error('Error: usage: twclaw quote <tweet-id> "text"')
      process.exit(1)
    }
    const newId = Date.now().toString().slice(-7)
  ...
  case 'followers': {
    ...
    Object.values(mockUsers)
      .slice(0, count)
      .forEach((u) => console.log(fmtUser(u)))
    break
  }
  ...
  case 'lists': {
    ...
    if (json) {
      ...
      lists.forEach((l) => console.log(`${l.id} — ${l.name} (${l.members} members)`))
    }
    break
  }
