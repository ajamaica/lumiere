...
class TraitAnalyzer {
  constructor() {
    this.traitTypes = ['Base', 'Face', 'Face Wear', 'Mouth', 'Head', 'Clothes', 'Background']
  }
  ...
  compareTraits(nft1, nft2, allNFTs) {
    ...
    if (nft1.traits) {
      nft1.traits.forEach((trait) => {
        ...
        const hasShared = nft2.traits?.some(
    ...
    if (nft2.traits) {
      nft2.traits.forEach((trait) => {
        const hasShared = nft1.traits?.some(
          (t) => t.trait_type === trait.trait_type && t.value === trait.value,
        )
  ...
  findNakedFloor(listings, traitType, traitValue) {
    const matching = listings.filter((listing) => {
      ...
      return listing.traits.some((t) => t.trait_type === traitType && t.value === traitValue)
    })
