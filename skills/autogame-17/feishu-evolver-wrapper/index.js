...
async function run() {
  ...
  if (!evolverDir) {
    console.error(
      "❌ Critical Error: Core 'evolver' plugin not found in ../private-evolver, ../evolver, or ../capability-evolver!",
    )
    process.exit(1)
  }
  ...
  while (attempts < MAX_RETRIES) {
    ...
    fs.appendFileSync(
      lifecycleLog,
      `[${new Date(startTime).toISOString()}] START Wrapper Proxy PID=${process.pid} Attempt=${attempts}\n`,
    )
    ...
    - **Target**: Auto-detects context (Group 🧬 or Master).`
      ...
      if (
        childArgsArr.length === 0 ||
        (childArgsArr.length === 1 && childArgsArr[0] === '--loop')
      ) {
      ...
      fs.appendFileSync(
        lifecycleLog,
        `[${new Date().toISOString()}] SUCCESS Wrapper PID=${process.pid} Duration=${duration}s\n`,
      )
      ...
      fs.appendFileSync(
        lifecycleLog,
        `[${new Date().toISOString()}] ERROR Wrapper PID=${process.pid} Duration=${duration}s: ${e.message}\n`,
      )
