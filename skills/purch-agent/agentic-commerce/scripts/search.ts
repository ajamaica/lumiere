...
Options:
  ...
  try {
    ...
    console.log(
      `📦 Found ${result.totalResults} products (showing ${result.products.length}, page ${result.page})`,
    )
    ...
    result.products.forEach((product, i) => {
      ...
      console.log(
        `   💰 ${product.currency} ${product.price} | ⭐ ${product.rating ?? 'N/A'} | 🏷️ ${product.source}`,
      )
    ...
    if (result.hasMore) {
      console.log(
        `📄 More results available. Use --page ${(options.page || 1) + 1} to see next page.`,
      )
    }
  } catch (error) {
