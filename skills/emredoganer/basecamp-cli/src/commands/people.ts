...
import { getMe, getPerson, listPeople } from '../lib/api.js'
import { isAuthenticated } from '../lib/config.js'

export function createPeopleCommands(): Command {
  const people = new Command('people').description('Manage people')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        peopleList.forEach((person) => {
          ...
          table.push([person.id, person.name, person.email_address, person.title || '-', role])
        })
        ...
        console.error(
          chalk.red('Failed to list people:'),
  ...
  people
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to get person:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
  ...
  const me = new Command('me')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to get profile:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
