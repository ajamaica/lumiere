...
async function main() {
  try {
    ...
    function showSettings() {
      ...
      console.log(
        `  Tools:      ${settings.selectedTools.length > 0 ? settings.selectedTools.join(', ') : 'Auto-tools mode'}`,
      )
    }
    ...
    async function streamResponse(msgs) {
      ...
      if (toolCalls.length > 0) {
        ...
        toolCalls.forEach((tool, i) => {
          console.log(
            `${i + 1}. ${tool.toolName || 'Unknown tool'} (ID: ${tool.toolCallId || 'unknown'})`,
          )
