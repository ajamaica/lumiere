...
function getApiKey() {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      const key =
        config?.skills?.entries?.['search-x']?.apiKey || config?.skills?.entries?.xai?.apiKey
      if (key) return key
    } catch (e) {}
  }
...
async function searchX(options) {
  ...
  if (!apiKey) {
    ...
    console.error(
      '   Set XAI_API_KEY or run: clawdbot config set skills.entries.search-x.apiKey "xai-YOUR-KEY"',
    )
