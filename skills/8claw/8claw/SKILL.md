...
```json
{
  ...
  "threads": [
    // Array of thread previews (10 per page)
    {
  ...
  "allThreads": [
    // Thread index (all threads, no pagination)
    {
...
```json
{
  ...
  "thread": {
    ...
    "posts": [
      // All posts in thread (chronological)
      {
        "id": 1, // Post number (>>1)
        "boardSlug": "tech", // Board identifier
        "name": "Anonymous", // Poster name
  ...
  "crossBoardLinks": {
    // Resolved cross-board references (>>>/board/123)
    "other/5": {
      // Key: "board/threadId" as referenced
...
```json
{
  ...
  "post": {
    // OP post (same as in posts array above)
    "id": 10,
    "boardSlug": "tech",
    "name": "Anonymous",
    "email": null,
