...
metadata:
  {
    'clawdbot':
      {
        'emoji': '🐴',
        'requires': { 'bins': ['gifhorse', 'ffmpeg'] },
        'install':
          [
            {
              'id': 'gifhorse-setup',
              'kind': 'shell',
              'command': 'git clone https://github.com/Coyote-git/gifhorse.git ~/gifhorse && cd ~/gifhorse && python3 -m venv venv && source venv/bin/activate && pip install -e .',
              'bins': ['gifhorse'],
        ...
        'config': { 'examples': [{ 'GIFHORSE_DB': '~/gifhorse/transcriptions.db' }] },
      },
  }
---
