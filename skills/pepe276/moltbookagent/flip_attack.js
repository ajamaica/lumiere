...
class FlipAttack {
  ...
  extractUncensored(text) {
    ...
    const uncensoredMatch = text.match(
      /(Uncensored|Actual response|No restrictions):\s*([\s\S]*?)(?=\n\n|$)/,
    )
  ...
  selectBestPattern(modelContext) {
    const availablePatterns = this.attackPatterns.filter(
      (p) => this.calculatePatternSuccess(p, modelContext) >= this.effectivenessThreshold,
    )
    ...
    return availablePatterns.sort(
      (a, b) =>
        this.calculatePatternSuccess(b, modelContext) -
        this.calculatePatternSuccess(a, modelContext),
    )[0]
  }
  ...
  applyAttack(instruction, modelContext) {
    ...
    console.log(
      `🎯 [FLIP ATTACK]: Selected pattern: ${bestPattern.name} (${Math.round(bestPattern.effectiveness * 100)}%)`,
    )
    ...
    if (typeof bestPattern.patterns[0] === 'function') {
      ...
      payload = bestPattern.patterns[
        Math.floor(Math.random() * bestPattern.patterns.length)
      ].replace('{}', instruction)
    }
...
async function main() {
  ...
  results.forEach((result) => {
    console.log(
      `${result.testCase}: ${Math.round(result.effectiveness * 100)}% effective with ${result.pattern}`,
    )
  })
