...
async function sendVoiceToTelegramGroup({
  text,
  groupId,
  ...
  try {
    ...
    await execAsync(
      `edge-tts --voice "${voice}" --rate="${rate}" --text "${cleanedText.replace(/"/g, '')}" --write-media "${tempMp3}"`,
    )

    // 3. 使用 ffmpeg 转换为 Telegram 兼容格式
    console.log('正在转换为Telegram兼容格式...')
    await execAsync(
      `ffmpeg -y -i "${tempMp3}" -c:a libopus -b:a 48k -ac 1 -ar 48000 -application voip "${tempOgg}"`,
    )
