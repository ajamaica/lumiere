...
**Key insight:** Snyk separates the _existence_ of a finding from its _impact on score_. This is exactly our problem.
...
The methodology _inherently_ accounts for context — a finding in a code-execution framework has different threat agent factors than in a web server.
...
- Score still reflects _some_ risk (exec() IS risky, just expected)
