...
async function getAllUsersCached() {
  ...
  // Write cache
  try {
    fs.writeFileSync(USER_CACHE_FILE, JSON.stringify(users))
  } catch (e) {
    console.warn('Failed to cache users:', e.message)
  }
  return users
}
  ...
  try {
    ...
    for (const res of results) {
      ...
      if (isAbsent) {
        ...
        await safeSend(
          userId,
          `[Attendance Alert] You are marked as ABSENT for ${dateStr}. Please submit a request if this is an error.`,
        )
      } else {
        if (isLate) {
          ...
          await safeSend(
            userId,
            `[Attendance Alert] You were LATE on ${dateStr}. Please be on time.`,
          )
        }
        if (isEarly) {
    ...
    // Summary Element
    const statusEmoji =
      !report.late.length && !report.early.length && !report.absent.length ? '✅' : '⚠️'
    ...
    const cardColor =
      report.late.length || report.early.length || report.absent.length ? 'orange' : 'green'
