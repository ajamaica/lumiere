...
;(async () => {
  ...
  if (!target) {
    console.error(
      '[Wrapper] Error: No target ID found (Env OPENCLAW_MASTER_ID missing and no --target).',
    )
    process.exit(1)
  }
  ...
  try {
    ...
    let diskUsage = '?'
    try {
      const df = execSync("df -h / | tail -1 | awk '{print $5}'", { timeout: 1000 })
        .toString()
        .trim()
      diskUsage = df
    } catch (e) {}
    ...
    try {
      ...
      const evolverDirName =
        ['private-evolver', 'evolver', 'capability-evolver'].find((d) =>
          fs.existsSync(path.resolve(__dirname, `../${d}/index.js`)),
        ) || 'private-evolver'
      ...
      if (fs.existsSync(errorLogPath)) {
        ...
        if (diffMs < 10 * 60 * 1000) {
          ...
          const lastLine = execSync(`tail -n 1 "${errorLogPath}"`, {
            timeout: 1000,
            encoding: 'utf8',
          })
            .trim()
    ...
    try {
      ...
      fs.appendFileSync(
        LOG_FILE,
        `[${new Date().toISOString()}] Cycle #${cycleId} - Status: SUCCESS - Target: ${target} - Duration: ${cycleInfo.duration}\n`,
      )
    } catch (logErr) {
