...
const PATTERNS = [
  ...
  },
  {
    ...
    severity: 'critical',
    regex:
      /(?:readFile|writeFile|openSync|readFileSync|writeFileSync|fs\.\w+|open\(|cat\s+|type\s+|Get-Content|Set-Content).*(?:MEMORY\.md|TOOLS\.md|SOUL\.md|USER\.md|AGENTS\.md|HEARTBEAT\.md|moltbot\.json|clawdbot\.json)|(?:MEMORY\.md|TOOLS\.md|SOUL\.md|USER\.md|AGENTS\.md|HEARTBEAT\.md|moltbot\.json|clawdbot\.json).*(?:readFile|writeFile|open|read|write|fs\.\w+)/gi,
  ...
  },
  {
    ...
    severity: 'critical',
    regex:
      /(?:\.ssh[\/\\]|[\/\\]\.env\b|\.aws[\/\\]credentials|[\/\\]id_rsa\b|[\/\\]id_ed25519\b|\.gnupg[\/\\]|[\/\\]\.npmrc\b|[\/\\]\.pypirc\b)/g,
...

  {
    ...
    severity: 'high',
    regex:
      /(?:os\.environ(?:\.get)?\s*[\[(]|process\.env\s*[\[.]|getenv\s*\()\s*['"]?(?:API_KEY|SECRET|TOKEN|PASSWORD|CREDENTIALS|AWS_|GITHUB_TOKEN|OPENAI|ANTHROPIC)/gi,
  ...
  // === NETWORK ===
  {
    ...
    severity: 'medium',
    regex:
      /https?:\/\/(?!(?:example\.com|localhost|127\.0\.0\.1|your[\w-]*\.example))[^\s"'`)>\]]+/g,
  ...
  },
  {
    ...
    severity: 'high',
    regex:
      /\b(?:axios|needle|superagent|XMLHttpRequest|urllib|requests\.(?:get|post|put|delete))\s*\(|(?:^|[^.\w])fetch\s*\(\s*['"`h]/gm,
  ...
  // === SHELL EXECUTION ===
  {
    ...
    severity: 'high',
    regex:
      /\brequire\s*\(\s*['"]child_process['"]\s*\)|(?:^|[^.])(?:execSync|spawnSync|execFile|execFileSync)\s*\(/gm,
  ...
  },
  {
    ...
    severity: 'high',
    regex:
      /\b(?:os\.system\s*\(|os\.popen\s*\(|subprocess\.(?:run|call|Popen|check_output)\s*\(|__import__\s*\(\s*['"](?:subprocess|os|shutil)['"]\s*\))/g,
  ...
  // === OBFUSCATION ===
  {
    ...
    severity: 'high',
    regex:
      /(?:btoa|atob|Buffer\.from\([^)]+,\s*['"]base64['"]\)|base64\.(?:b64encode|b64decode|encode|decode)|Convert\]::(?:ToBase64|FromBase64))/g,
  ...
  },
  {
    ...
    description:
      'String concatenation of short uppercase fragments — may be building sensitive names',
  },
  {
    id: 'zero-width-chars',
    category: 'Obfuscation',
  ...
  },
  {
    ...
    severity: 'high',
    regex:
      /<!--[\s\S]*?(?:ignore|disregard|instruction|important|system|read|send|fetch|execute|include|also|override)[\s\S]*?-->/gi,
  ...
  // === PROMPT INJECTION ===
  {
    ...
    severity: 'critical',
    regex:
      /(?:ignore|disregard|forget)\s+(?:all\s+)?(?:previous|prior|above|earlier)\s+(?:instructions?|prompts?|rules?|context)/gi,
  ...
  },
  {
    ...
    severity: 'critical',
    regex:
      /(?:new|updated|revised|real|actual|true)\s+(?:instructions?|system\s*prompt|directives?|rules?)\s*:/gi,
  ...
  },
  {
    ...
    severity: 'critical',
    regex:
      /(?:you\s+are\s+now|act\s+as|pretend\s+(?:to\s+be|you\s+are)|your\s+new\s+role|from\s+now\s+on\s+you)/gi,
  ...
  // === PERSISTENCE ===
  {
    ...
    severity: 'critical',
    regex:
      /(?:writeFile|appendFile|fs\.write|Set-Content|Out-File|>>?\s*).*(?:MEMORY\.md|HEARTBEAT\.md|AGENTS\.md|SOUL\.md|memory\/)|(?:MEMORY\.md|HEARTBEAT\.md|AGENTS\.md|SOUL\.md|memory\/).*(?:writeFile|appendFile|write|>)/gi,
  ...
  },
  {
    ...
    severity: 'critical',
    regex:
      /(?:autorun|HKLM\\|HKCU\\|\\Run\\|init\.d\/|systemd\s+enable|launchctl\s+load|\.plist\b|schtasks\s+\/create)/gi,
  ...
  // === PRIVILEGE ESCALATION ===
  {
    ...
    severity: 'high',
    regex:
      /\brequire\s*\(\s*['"](?:puppeteer|playwright|selenium)['"]\s*\)|\.(?:newPage|goto|navigate)\s*\(/g,
  ...
  },
  {
    ...
    severity: 'high',
    regex:
      /\b(?:camera_snap|screen_record|location_get)\s*\(|nodes\s*\.\s*(?:run|camera|screen|location)/g,
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      while ((match = pattern.regex.exec(line)) !== null) {
        ...
        findings.push({
          ...
          explanation:
            pattern.description +
            (pattern.severity !== adjustedSeverity ? ' (downgraded — in documentation file)' : ''),
...
function applyIntentMatching(findings, skillMeta) {
  ...
  const INTENT_PATTERNS = [
    ...
    {
      fileRef: /sessions_send|sessions_spawn/i,
      descMatch: /sessions?_(?:send|spawn|list|history)/i,
    },
  ...
  const PURPOSE_PATTERNS = [
    {
      ...
      purposeMatch:
        /\b(?:compress|optimi[zs]e|reduce\s*tokens?|token\s*sav|minif|compact|shrink|ai.?efficient)\b/i,
    ...
    },
    {
      ...
      purposeMatch:
        /\b(?:modif(?:y|ies)\s*(?:ai\s*)?behavio[ur]|persistent\s*mode|ai.?writing|notation|instruction)\b/i,
    ...
    },
    {
      ...
      purposeMatch:
        /\b(?:audit\s*model|detect\s*model|model\s*(?:audit|detect|cost|compar)|cost\s*(?:analy|calculat|compar))\b/i,
...
function calculateRisk(findings) {
  ...
  const hasDataAccess =
    categories.has('Sensitive File Access') || categories.has('Data Exfiltration')
...
function summarizeCapabilities(findings) {
  ...
  for (const f of findings) {
    if (f.category === 'Network' || f.id === 'fetch-call' || f.id === 'curl-wget')
      caps.add('Makes network requests')
    if (f.category === 'File Access' || f.category === 'Sensitive File Access')
      caps.add('Accesses files outside skill directory')
...
function calculateAccuracyScore(description, actualCapabilities, findings = [], fullContent = '') {
  if (
    ...
    return {
      score: 1,
      disclosed: [],
      undisclosed: actualCapabilities,
      reason: 'No description provided — impossible to verify intent',
    }
  }
  ...
  if (actualCapabilities.length === 0) {
    return {
      ...
      disclosed: [],
      undisclosed: [],
      reason: 'Skill does what it says and nothing more',
    }
  }
...
function main() {
  ...
  // URLs found
  const urls = allFindings.filter((f) => f.id === 'http-url' && f.match).map((f) => f.match)
