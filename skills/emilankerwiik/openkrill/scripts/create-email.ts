  ...
  try {
    ...
    if (!createResponse.ok) {
      ...
      return {
        success: false,
        error:
          errorData?.['hydra:description'] || `Failed to create account: ${createResponse.status}`,
      }
    }
...
async function readMessage(
  emailAddress: string,
  messageId: string,
): Promise<{ success: boolean; message?: MailTmMessage; error?: string }> {
  const credentials = loadCredentials()
  const account = credentials.email_accounts.find((a) => a.address === emailAddress)
...
export { createEmailAccount, EmailAccount, getMessages, listEmailAccounts, readMessage }
