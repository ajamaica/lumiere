...
```javascript
const result = await mcpManager.callTool('kr-legal', 'search_statute', { query: '민법', limit: 5 })
...
```javascript
const transport = new StreamableHTTPClientTransport('http://localhost:3000/mcp', { debug: true })
```
