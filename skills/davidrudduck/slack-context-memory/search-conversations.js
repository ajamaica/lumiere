...
async function semanticSearch(query, options = {}) {
  ...
  try {
    ...
    const params = channelId ? [embeddingStr, limit, channelId] : [embeddingStr, limit]
...
async function fullTextSearch(query, options = {}) {
  ...
  try {
    ...
    const params = channelId ? [query, limit, channelId] : [query, limit]
...
async function searchByTopic(topic, options = {}) {
  ...
  try {
    ...
    const params = channelId ? [topic, limit, channelId] : [topic, limit]
  ...
  async function run() {
    try {
      ...
      if (command === 'semantic') {
        ...
        if (!query) {
          console.error(
            'Usage: search-conversations.js semantic "your query" [--channel ID] [--limit N]',
          )
          process.exit(1)
        }
        ...
        if (!query) {
          console.error(
            'Usage: search-conversations.js text "your query" [--channel ID] [--limit N]',
          )
          process.exit(1)
        }
        ...
        if (!topic) {
          console.error(
            'Usage: search-conversations.js topic "topic-name" [--channel ID] [--limit N]',
          )
          process.exit(1)
        }
