...
export class MoltGatewayClient {
  ...
  async enableSkill(skillKey: string): Promise<Skill> {
    return this.request<Skill>(GatewayMethods.SKILLS_UPDATE, { skillKey, enabled: true })
  }

  async disableSkill(skillKey: string): Promise<Skill> {
    return this.request<Skill>(GatewayMethods.SKILLS_UPDATE, { skillKey, enabled: false })
  }
  ...
  private handleEvent(frame: EventFrame): void {
    ...
    switch (event) {
      ...
      case GatewayEvents.DEVICE_PAIR_RESOLVED:
        // The device was approved on the gateway dashboard. Trigger an
        // immediate reconnect so the user doesn't have to wait for the
        // next scheduled reconnect timer to fire.
        wsLogger.info('Device pairing resolved — reconnecting immediately')
        this.clearReconnectTimer()
        this.reconnectAttempt = 0
