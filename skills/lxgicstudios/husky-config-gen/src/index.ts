...
export async function analyzeProject(): Promise<string> {
  ...
  let pkgContent = '{}'
  try {
    pkgContent = readFileSync(join(cwd, 'package.json'), 'utf-8')
  } catch {}
  return pkgContent
}
...
export async function generateHooks(
  pkgContent: string,
): Promise<{ preCommit: string; prePush: string; commitMsg: string }> {
  const response = await openai.chat.completions.create({
    model: 'gpt-4o-mini',
    messages: [
      {
        role: 'system',
        content: `You generate git hook scripts based on a project's package.json. Return JSON:
