...
async function cmdStatus() {
  ...
  const symbol = status.status === 'HEALTHY' ? 'OK' : status.status === 'WARNING' ? 'WARN' : 'CRIT'

  console.log(`\n${c('Context Health:', 'bold')} ${c(`${status.status} [${symbol}]`, healthColor)}`)
  console.log(
    `  Used: ${c(status.used.toLocaleString(), 'cyan')} / ${status.limit.toLocaleString()} tokens (${c(status.percent + '%', healthColor)})`,
  )
...
async function cmdSearch(query) {
  ...
  if (results.length === 0) {
    ...
    results.forEach((r, i) => {
      console.log(
        `${i + 1}. ${c((r.similarity * 100).toFixed(0) + '% match', 'cyan')} ${c('(priority: ' + r.priority + ')', 'blue')}`,
      )
...
async function cmdStats() {
  ...
  console.log(
    `  Input: ${status.input_tokens.toLocaleString()} | Output: ${status.output_tokens.toLocaleString()}`,
  )
  console.log(`  Compressions: ${status.compressions}`)
...
async function cmdRelevant(query) {
  ...
  if (memories.length === 0) {
    ...
    memories.forEach((r, i) => {
      console.log(
        `${i + 1}. ${c((r.similarity * 100).toFixed(0) + '%', 'cyan')} ${r.content.substring(0, 150)}${r.content.length > 150 ? '...' : ''}`,
      )
    })
    console.log(
...
async function cmdCompress() {
  ...
  if (result) {
    console.log(
      `Compressed: ${result.before.messages} -> ${result.after.messages} messages, ${result.before.tokens} -> ${result.after.tokens} tokens`,
    )
  } else {
  ...
  try {
    switch (command) {
      ...
      case 'search':
        ...
        break
      case 'store':
        await cmdStore(args.slice(1).join(' '))
        break
      case 'identity':
        await cmdIdentity(...args.slice(1))
        break
      case 'stats':
        await cmdStats()
        break
      case 'relevant':
        await cmdRelevant(args.slice(1).join(' '))
        break
      case 'compress':
        await cmdCompress()
        break
