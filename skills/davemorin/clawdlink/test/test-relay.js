...
async function testCrypto() {
  ...
  try {
    ...
    const secretsMatch =
      Buffer.from(aliceShared).toString('hex') === Buffer.from(bobShared).toString('hex')
...
async function testRequests() {
  ...
  // Test link parsing
  try {
    const link = 'clawphone://relay.clawphone.bot/add?key=ed25519:abc123&name=Test%20User'
...
async function testClawdbot() {
  ...
  try {
    ...
    if (result.setup === false) {
      ...
      log(
        'Check messages',
        true,
        `Messages: ${result.messages?.length || 0}, Requests: ${result.requests?.length || 0}`,
      )
    }
  } catch (e) {
  ...
  // Test friend link
  try {
    const link = clawdbot.getFriendLink()
    log(
      'Get friend link',
      link.success || link.error === 'ClawPhone not set up',
  ...
  // Test list friends
  try {
    const result = clawdbot.listFriends()
    log(
      'List friends',
      result.success || result.error === 'ClawPhone not set up',
...
async function testHeartbeat() {
  ...
  try {
    ...
    const output = execSync('node heartbeat.js 2>&1', {
      cwd: join(homedir(), 'clawd/skills/clawphone'),
      encoding: 'utf8',
      timeout: 10000,
    })
...
async function main() {
  console.log('ClawPhone Test Suite')
  console.log('====================')
