...
async function checkAccountStatus(apiKey, baseUrl) {
  ...
  if (!res.ok) {
    ...
    throw new Error(error.message || `HTTP ${res.status}: Failed to check account status`)
  }
...
async function createOnboardingLink(apiKey, baseUrl) {
  ...
  if (!res.ok) {
    const error = await res.json().catch(() => ({}))
    throw new Error(error.message || `HTTP ${res.status}: Failed to create onboarding link`)
  }
...
function displayStatus(status) {
  ...
  if (status.requirements?.currently_due?.length > 0) {
    console.log(`  Requirements:    ${status.requirements.currently_due.length} item(s) pending`)
  }
  ...
  try {
    ...
    try {
      ...
      if (err.message.includes('401') || err.message.includes('Unauthorized')) {
        ...
        console.error('The backend needs to be updated to support X-API-Key authentication.')
