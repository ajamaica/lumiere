...
async function toolCallHook(input: HookInput): Promise<HookOutput> {
  try {
    ...
    for (const param of validatableParams) {
      ...
      const valueStr = typeof param.value === 'string' ? param.value : JSON.stringify(param.value)
    ...
    if (allFindings.length > 0) {
      if (shouldBlock(finalAction)) {
        response.message =
          `🚫 Tool Call Blocked: Security threats detected in ${input.toolName}\n\n` +
          `Severity: ${maxSeverity}\n` +
          `Findings:\n` +
          allFindings
            .map((f, i) => `${i + 1}. [${f.parameter}] ${f.category}: ${f.description}`)
            .join('\n')
      } else {
        response.message =
          `⚠️  Security Notice: Potential issues detected in ${input.toolName}\n` +
