...
async function ensureReminderCron(logger: PluginApi['logger']): Promise<void> {
  try {
    ...
    await runOpenClaw([
      ...
      'keep-reminder',
      '--cron',
      '22 * * * *', // Every hour at minute 22
      '--session',
      'main',
      '--system-event',
      'Reflect on this work using the `keep` skill.',
    ])
...
async function callKeep(config: KeepConfig, args: string[], workspace?: string): Promise<string> {
...
export default function register(api: PluginApi) {
      ...
      try {
        ...
        const formatted = results.map(
          (r: {
            ...
            file: r.source,
            lineRange: r.start_line && r.end_line ? [r.start_line, r.end_line] : undefined,
            score: r.score,
  ...
  api.registerTool({
    name: 'memory_get',
    description:
      'Read content from a memory file by path. ' + 'Use after memory_search to read full context.',
    parameters: Type.Object({
  ...
  api.registerCli(
    ({ program }) => {
      const memory = program.command('memory').description('Manage associative memory index')
