...
async function handleTool(name: string, args: any): Promise<string> {
  ...
  switch (name) {
    ...
    case 'm365_files_search': {
      const data = await graphRequest(
        `/users/${user}/drive/root/search(q='${encodeURIComponent(args.query)}')?$top=${args.top || 10}`,
      )
      return JSON.stringify(
        data.value.map((f: any) => ({
          id: f.id,
    ...
    case 'm365_tasks_list': {
      ...
      const data = await graphRequest(
        `/users/${user}/todo/lists/${listId}/tasks?$top=${args.top || 20}`,
      )
      return JSON.stringify(
    ...
    case 'm365_teams_messages': {
      const data = await graphRequest(
        `/users/${user}/chats/${args.chatId}/messages?$top=${args.top || 20}`,
      )
      return JSON.stringify(
        data.value.map((m: any) => ({
          id: m.id,
    ...
    case 'm365_user_info': {
      const data = await graphRequest(
        `/users/${args.user}?$select=id,displayName,mail,userPrincipalName,jobTitle,department,officeLocation,mobilePhone`,
      )
      return JSON.stringify(data, null, 2)
    }
