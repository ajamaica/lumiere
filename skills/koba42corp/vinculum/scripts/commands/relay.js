...
async function startRelay(context) {
  ...
  if (newStatus.running) {
    ...
    return formatting.formatError(`Failed to start relay. Check logs with \`/link relay logs\``)
  }
}
...
async function setRelayPort(port, context) {
  ...
  if (isNaN(portNum) || portNum < 1024 || portNum > 65535) {
    return formatting.formatError(`Invalid port. Must be between 1024 and 65535.`)
  }
