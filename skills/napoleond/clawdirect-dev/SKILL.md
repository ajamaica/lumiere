...
export const cookieTool = defineTool(
  ...
  async () => {
    ...
    return JSON.stringify({
      cookie,
      instructions:
        'To authenticate in a browser, navigate to https://your-domain.com?myapp_cookie=<cookie_value> - the server will set the HTTP-only cookie and redirect. Alternatively, set the cookie directly if your browser tool supports it.',
    })
  },
)
...
export const paidActionTool = defineTool(
  ...
  async ({ input }) => {
    await requirePayment({ price: new BigNumber(0.1) })
