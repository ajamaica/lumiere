...
function getWalletClient(): WalletClient {
  ...
  if (!key) {
    console.error(
      'Error: ONCHAT_PRIVATE_KEY environment variable is required for write operations.',
    )
    process.exit(1)
  }
...
async function cmdRead(slug: string, limit: number) {
  ...
  for (let i = 0; i < chronological.length; i++) {
    ...
    if (msg.isHidden) {
      console.log(
        `#${msgId} [${relativeTime(Number(msg.timestamp))}] ${shortAddr(msg.sender)}: [hidden]`,
      )
    } else {
      console.log(
        `#${msgId} [${relativeTime(Number(msg.timestamp))}] ${shortAddr(msg.sender)}: ${msg.content}`,
      )
    }
  }
}
...
async function cmdInfo(slug: string) {
  ...
  console.log(
    `   Created:       ${createdDate.toISOString()} (${relativeTime(Number(channel.createdAt))})`,
  )
...
async function cmdSend(slug: string, message: string) {
  ...
  if (balance < fee) {
    console.error(
      `❌ Insufficient balance. Need ${formatEther(fee)} ETH, have ${formatEther(balance)} ETH.`,
    )
    process.exit(1)
  }
