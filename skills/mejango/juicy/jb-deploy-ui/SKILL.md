      ...
      window.connectWallet = async function () {
        ...
        try {
          ...
          publicClient = createPublicClient({
            chain: CHAINS[chainId],
            transport: http(CHAIN_CONFIGS[chainId]?.rpc),
          })
      ...
      window.deploy = async function () {
        ...
        try {
          ...
          const terminalConfig = {
            ...
            accountingContexts: [
              { token: '0x000000000000000000000000000000000000EEEe', decimals: 18, currency: 0 },
            ],
          }
          ...
          showStatus(
            'success',
            `Project deployed! <a href="${getTxUrl(chainId, hash)}" target="_blank">View tx</a>`,
          )
        } catch (error) {
  ...
  <body>
    ...
    <script type="module">
      ...
      window.addTier = function () {
        tierCount++
        document.getElementById('tiers-container').insertAdjacentHTML(
          'beforeend',
          `
        ...
        document.querySelectorAll('.tier-card').forEach((card, i) => {
          ...
          if (price && supply) {
            tiers.push({
              ...
              encodedIPFSUri: encodeIPFSUri(ipfs),
              category: i + 1,
              discountPercent: 0,
      ...
      window.deploy = async function () {
        ...
        if (tiers.length === 0) {
          alert('Add at least one tier')
          return
        }
        const addresses = chainConfig?.chains[chainId]?.contracts
        if (!addresses?.JB721TiersHookProjectDeployer) {
          alert('721 Deployer not available')
          return
        }
  ...
  <body>
    <div class="container" style="max-width: 640px;">
      ...
      <p style="color: var(--text-muted); margin-bottom: 1.5rem;">
        Launch an autonomous tokenized treasury
      </p>
      ...
      <div class="card">
        <div class="badge-warning" style="margin-bottom: 1rem;">
    ...
    <script type="module">
      ...
      window.deployRevnet = async function () {
        ...
        if (!revDeployer) {
          alert('REVDeployer not available on this network')
          return
        }
        ...
        const stageConfig = {
          ...
          issuanceCutPercent: parseInt(
            parseFloat(document.getElementById('issuance-cut').value) * 100,
          ),
