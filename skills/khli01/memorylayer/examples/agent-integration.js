...
Based on the relevant memories above, provide a helpful response.`
    ...
    if (message.includes('preferences')) {
      response =
        "Based on your preferences, I see you prefer dark mode UI with blue accents. I'll keep that in mind!"
      ...
      response =
        'To export your data, go to Settings > Account > Export Data button. Let me know if you need help!'
    } else {
...
async function main() {
  ...
  try {
    ...
    await memory.remember('User prefers dark mode UI with blue accent colors', {
      type: 'semantic',
      importance: 0.8,
    })

    await memory.remember('User is a developer who likes concise, technical explanations', {
      type: 'semantic',
      importance: 0.9,
    })

    await memory.remember('To export data: Settings > Account > Export Data button', {
      type: 'procedural',
      importance: 0.7,
    })
    ...
    console.log('\n' + '='.repeat(50))
    await agent.learnFromFeedback('User found the export feature confusing - add a tutorial', 0.8)
