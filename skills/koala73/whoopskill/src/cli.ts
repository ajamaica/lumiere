...
import { fetchAllTypes, fetchData } from './api/client.js'
import { login, logout, refresh as authRefresh, status as authStatus } from './auth/oauth.js'
import type { DataType, WhoopData } from './types/whoop.js'
import { analyzeTrends, formatInsights, formatTrends, generateInsights } from './utils/analysis.js'
import { getDaysAgo, getWhoopDay, validateISODate } from './utils/date.js'
import { ExitCode, handleError, WhoopError } from './utils/errors.js'
...
program.name('whoopskill').description('CLI for fetching WHOOP health data').version('1.1.0')
    ...
    try {
      ...
      const [recovery, sleep, cycle, workout] = await Promise.all([
        ...
        import('./api/client.js').then((m) =>
          m.getWorkout({ start: date + 'T00:00:00.000Z', end: date + 'T23:59:59.999Z' }, true),
        ),
      ])
