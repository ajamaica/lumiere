...
<html lang="en">
  ...
  <body>
    ...
    <div class="card">
      ...
      <div id="wallet-status" class="hidden">Connected: <span id="wallet-address"></span></div>
    </div>
    ...
    <div class="card hidden" id="payment-section">
      ...
      <div
        style="background: var(--bg-primary); padding: 0.75rem; border-radius: 4px; margin-bottom: 1rem; font-size: 0.875rem;"
      >
        <div class="stat-row">
          <span class="stat-label">Total Gas Cost</span
    ...
    <script type="module">
      ...
      window.startDeploy = async function () {
        ...
        try {
          ...
          for (const chainId of selectedChains) {
            ...
            const signed = await signForwardRequest(
              parseInt(chainId),
              calldata,
              forwarder,
              controller,
      ...
      async function selectPaymentChain(payment, chip) {
        document
          .querySelectorAll('#payment-chains .chain-chip')
          .forEach((c) => c.classList.remove('payment'))
      ...
      async function pollBundleStatus(bundleUuid) {
        const poll = async () => {
          try {
            ...
            const allDone = status.transactions.every((tx) =>
              ['Success', 'Completed', 'Failed'].includes(tx.status),
            )
    ...
    <script type="module">
      ...
      async function loadDashboard() {
        ...
        `,
          ...
          document.getElementById('total-contributors').textContent = formatNumber(
            group.contributorsCount,
          )

          document.getElementById('chain-breakdown').innerHTML = group.projects_rel
            .map(
              (project) => `
      ...
      `,
        ...
        document.getElementById('activity-feed').innerHTML = events
          .map(
            (e) => `
...
export default async function handler(req, res) {
  ...
  const response = await fetch(`https://bendystraw.xyz/${process.env.BENDYSTRAW_API_KEY}/graphql`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
