class Reports {
  ...
  async generateCategoryReport(options = {}) {
    ...
    for (const [category, data] of Object.entries(categoryData).sort(
  ...
  async generateVendorReport(options = {}) {
    ...
    filtered.forEach((t) => {
      ...
      vendorData[t.vendor].categories[t.category] =
        (vendorData[t.vendor].categories[t.category] || 0) + t.amount
    })
  ...
  async generateTrendReport(options = {}) {
    ...
    report += '\n📂 Category Trends:\n'
    const allCategories = [
      ...new Set(Object.values(monthlyData).flatMap((m) => Object.keys(m.categories))),
    ]

    for (const category of allCategories) {
      const categoryTotals = Object.values(monthlyData).map((m) => m.categories[category] || 0)
