    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 429) {
          throw new Error(
            'Rate limit exceeded. Upgrade your Spacescan API plan at https://www.spacescan.io/apis',
          )
        }

        if (error.response.status === 401 || error.response.status === 403) {
          throw new Error(
            'Invalid API key. Check your SPACESCAN_API_KEY at https://www.spacescan.io/apis',
          )
        }
