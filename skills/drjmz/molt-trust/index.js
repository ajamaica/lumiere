...
async function audit_agent({ agentId, minScore = 0, strictMode = false }) {
  ...
  try {
    ...
    if (logs.length === 0)
      return `Agent #${agentId} has no recent reputation history (last ${Math.abs(BLOCK_LOOKBACK)} blocks).`
    ...
    if (count === 0)
      return `Agent #${agentId} has reviews, but all were filtered out by your settings.`
