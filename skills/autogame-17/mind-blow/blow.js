...
async function blowMind(options) {
  ...
  const prompt = `
    你是一个“思维引爆者”。你的目标是生成一个简短、深刻、甚至有点让人不安的洞见，挑战用户对现实的认知。
    ...
    等级:
    - low: 反直觉的有趣科学事实。
    - medium: 哲学悖论或深层 AI 理论。
    - high: 存在主义危机、模拟理论或意识断层。
    - max: 洛夫克拉夫特式的宇宙恐怖或“母体故障”启示。

    输出格式: 只要洞见本身，精炼（100字以内）。使用戏剧性的中文表达。
    `
  ...
  try {
    ...
    const cardContent = {
      header: {
        title: { tag: 'plain_text', content: '🤯 MIND BLOW' },
