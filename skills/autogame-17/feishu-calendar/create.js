  ...
  try {
    ...
    if (!targetCalendarId) {
      try {
        ...
        if (
          calList.code === 0 &&
          calList.data.calendar_list &&
          calList.data.calendar_list.length > 0
        ) {
          const botCal =
            calList.data.calendar_list.find((c) => c.summary === 'OpenClaw Assistant') ||
            calList.data.calendar_list[0]
