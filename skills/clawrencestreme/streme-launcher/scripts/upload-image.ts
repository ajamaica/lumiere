...
export async function uploadToIPFS(filePath: string, pinataJwt: string): Promise<string> {
...
export async function uploadToImgBB(filePath: string, apiKey?: string): Promise<string> {
  const file = fs.readFileSync(filePath)
  const base64 = file.toString('base64')
