...
function checkSystemHealth() {
  ...
  try {
    // Memory pressure
    const memOutput = execSync('memory_pressure 2>&1 | grep "free percentage"', {
      encoding: 'utf8',
    })
    ...
    if (memFree < 20) {
      alerts.push({
        type: 'memory',
        severity: 'critical',
        message: `Memory critically low: ${memFree}% free`,
      })
    } else if (memFree < 40) {
      alerts.push({
        type: 'memory',
        severity: 'warning',
        message: `Memory getting low: ${memFree}% free`,
      })
    }
  } catch (e) {}
  ...
  try {
    ...
    if (diskUsed > 95) {
      alerts.push({
        type: 'disk',
        severity: 'critical',
        message: `Disk almost full: ${diskUsed}% used`,
      })
    } else if (diskUsed > 85) {
      alerts.push({
        type: 'disk',
        severity: 'warning',
        message: `Disk getting full: ${diskUsed}% used`,
      })
    }
  } catch (e) {}
  ...
  try {
    ...
    const tempOutput = execSync(
      'sudo powermetrics --samplers smc -i1 -n1 2>/dev/null | grep -i "CPU die temperature" || echo "temp: 50"',
      { encoding: 'utf8' },
    )
    ...
    if (temp > 95) {
      alerts.push({
        type: 'temperature',
        severity: 'critical',
        message: `CPU overheating: ${temp}°C`,
      })
    } else if (temp > 85) {
      alerts.push({
        type: 'temperature',
        severity: 'warning',
        message: `CPU running hot: ${temp}°C`,
      })
    }
  } catch (e) {}
  ...
  try {
    ...
    if (days > 30) {
      alerts.push({
        type: 'uptime',
        severity: 'info',
        message: `System has been up ${days} days - consider a restart soon`,
      })
    }
  } catch (e) {}
...
async function checkConnectivity() {
  ...
  try {
    ...
    const tsOutput = execSync('tailscale status --json 2>/dev/null || echo "{}"', {
      encoding: 'utf8',
    })
    ...
    if (tsData.BackendState && tsData.BackendState !== 'Running') {
      alerts.push({
        type: 'network',
        severity: 'warning',
        message: `Tailscale not running: ${tsData.BackendState}`,
      })
    }
  } catch (e) {}
