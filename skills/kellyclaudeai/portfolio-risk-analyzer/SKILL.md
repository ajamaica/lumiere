...
```javascript
const riskScore =
  concentrationRisk * 0.3 + volatilityRisk * 0.3 + liquidationRisk * 0.2 + protocolRisk * 0.2
```
...
if (stablecoinBalance > 1000 && max(aaveYield, compoundYield) > 5) {
  suggest(
    `Deposit stables in ${aaveYield > compoundYield ? 'Aave' : 'Compound'} for ${Math.max(aaveYield, compoundYield)}% APY`,
  )
}
```
