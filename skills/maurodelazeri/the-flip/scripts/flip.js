 ...
 */
import { flipToStr, getGamePDA, getProgram, loadGameWallet, TOTAL_FLIPS } from './lib/solana.js'
...
async function main() {
  ...
  if (game.gameOver) {
    console.log('Game already over. All 20 flips complete.')
    return
  }
  ...
  if (remaining <= 0) {
    console.log('All flips already done.')
    return
  }
  ...
  if (doOne) {
    ...
    console.log(
      'Flip #' + updated.currentFlip + ': ' + (result === 'H' ? 'HEADS' : 'TAILS') + '  TX: ' + tx,
    )
