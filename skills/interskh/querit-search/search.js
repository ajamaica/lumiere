...
function buildPayload(args) {
  ...
  if (query.length > MAX_QUERY_LENGTH) {
    console.error(`Warning: Query truncated from ${query.length} to ${MAX_QUERY_LENGTH} characters`)
...
async function fetchContent(url) {
  ...
  try {
    const { stdout } = await execFileAsync(process.execPath, [`${scriptDir}content.js`, url], {
      timeout: 30_000,
    })
    return stdout.trim()
  } catch (err) {
