...
async function buyToken({ privateKey, mint, sol, slippageBps = 500 }) {
  ...
  if (!curve.complete) {
    ...
    if (balance < neededLamports) {
      throw new Error(
        `Wallet balance too low for trade + rent (need ${(neededLamports / LAMPORTS_PER_SOL).toFixed(6)} SOL)`,
      )
    }
    ...
    try {
      ...
      throw new Error(
        `Jupiter quote failed: ${err.response?.status || ''} - ${err.response?.data?.error || err.message}`,
      )
    }
    ...
    try {
      ...
      throw new Error(
        `Jupiter swap-instructions failed: ${err.response?.status || ''} - ${err.response?.data?.error || err.message}`,
      )
    }
...
async function sendToken({ privateKey, mint, to, amount, decimals = null }) {
  ...
  const tx = new Transaction().add(
    ...
    ComputeBudgetProgram.setComputeUnitPrice({
      microLamports: computeUnitPriceMicrolamports(100_000),
    }),
  )
...
async function main() {
  ...
  if (cmd === 'buy') {
    ...
    if (!keyfile || !mint || !Number.isFinite(sol)) {
      throw new Error(
        'Usage: buy --keyfile <WALLET_JSON> --mint <MINT> --sol <AMOUNT> [--slippageBps <BPS>]',
      )
    }
    const privateKey = getPrivateKeyFromFile(keyfile)
  ...
  if (cmd === 'sell') {
    ...
    if (!keyfile || !mint || !Number.isFinite(amount)) {
      throw new Error(
        'Usage: sell --keyfile <WALLET_JSON> --mint <MINT> --amount <AMOUNT> [--slippageBps <BPS>]',
      )
    }
    const privateKey = getPrivateKeyFromFile(keyfile)
  ...
  if (cmd === 'send') {
    ...
    if (!keyfile || !mint || !to || !Number.isFinite(amount)) {
      throw new Error(
        'Usage: send --keyfile <WALLET_JSON> --mint <MINT> --to <PUBKEY> --amount <AMOUNT> [--decimals <N>]',
      )
    }
    const privateKey = getPrivateKeyFromFile(keyfile)
