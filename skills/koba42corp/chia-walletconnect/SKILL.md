...
// Handle web app data
bot.on('web_app_data', async (msg) => {
  const { address, message: challengeMsg, signature, publicKey } = JSON.parse(msg.web_app_data.data)
...
bot.on('web_app_data', async (msg) => {
  const { address, message, signature, publicKey } = JSON.parse(msg.web_app_data.data)
  ...
  const hasRequiredNFT = nfts.some((nft) => nft.collection_id === 'col1required...')
