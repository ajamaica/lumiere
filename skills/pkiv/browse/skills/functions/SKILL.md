...
async function invokeFunction(functionId: string, params: object) {
  // Start invocation
  const invokeRes = await fetch(`https://api.browserbase.com/v1/functions/${functionId}/invoke`, {
    method: 'POST',
    headers: {
...
defineFn('scrape', async ({ session, params }) => {
  ...
  const items = await page.$$eval(params.selector, (els) => els.map((el) => el.textContent?.trim()))
