...
describe('MemoryStore', () => {
  ...
  it('stores and searches memories', async () => {
    ...
    assert.ok(
      ...
      'Best match should be the TypeScript memory',
    )
  })
  ...
  it('tracks access count and last_accessed_at on search', async () => {
    const id = await store.store('Access tracking test memory with specific content', {
      priority: 7,
    })
  ...
  it('packs memories within token budget', async () => {
    ...
    const { memories, tokensUsed } = await store.getRelevantContext('fact about memory', 50, {
      threshold: 0.3,
    })
  ...
  it('memory decay reduces effective priority over time', () => {
    ...
    assert.ok(
      effective < oldMemory.priority,
      'Old unaccessed memory should have lower effective priority',
    )
  ...
  it('access boost increases effective priority', () => {
    ...
    assert.ok(
      effective > frequentMemory.priority,
      'Frequently accessed memory should have boosted priority',
    )
  })
})
...
describe('Compressor', () => {
  ...
  it('scores architectural decisions higher than pleasantries', async () => {
    const decision = {
      role: 'user',
      content:
        'After analyzing the tradeoffs between microservices and monolith, we decided to go with a modular monolith because deployment complexity outweighs benefits at our current scale.',
    }
    ...
    assert.ok(
      decisionScore > pleasantryScore,
      `Decision (${decisionScore}) should score higher than pleasantry (${pleasantryScore})`,
    )
  })
  ...
  it('compress returns scored messages with numeric priority', async () => {
    const messages = [
      ...
      {
        role: 'assistant',
        content: 'Good choice for relational data',
        tokens: 8,
        timestamp: Date.now(),
      },
    ...
    assert.ok(
      scored.every((m) => typeof m.priority === 'number'),
      'All scored messages should have numeric priority',
    )
  })
  ...
  it('uses pluggable summarizer when provided', async () => {
    ...
    const messages = [{ role: 'user', content: 'Some conversation context here', priority: 6 }]
...
describe('ContextMonitor', () => {
  ...
  it('emergency compression does not drop everything', async () => {
    ...
    smallMonitor.session.messages = [
      {
        role: 'user',
        content: 'My name is Alice and I always prefer TypeScript over JavaScript',
        tokens: 20,
        timestamp: Date.now() - 5000,
      },
      {
        role: 'assistant',
        content: 'Noted, I will remember your preference for TypeScript',
        tokens: 15,
      ...
      {
        role: 'user',
        content: 'What should I use for this project?',
        tokens: 10,
        timestamp: Date.now() - 2000,
      },
      {
        role: 'assistant',
        content: 'Based on your preferences, TypeScript would be ideal',
        tokens: 12,
...
describe('ContextGuardian (integration)', () => {
  ...
  it('getRelevantContext respects token budget', async () => {
    ...
    const { memories, tokensUsed } = await cg.getRelevantContext('tech stack', 100, {
      threshold: 0.3,
    })
    ...
    const messages = [
      {
        role: 'user',
        content:
          'We decided to use Redis for caching. This is an important architectural decision.',
        tokens: 15,
        timestamp: Date.now(),
      },
