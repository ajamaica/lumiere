...
query V5Projects($limit: Int!) {
  projects(where: { version: 5 }, orderBy: "volumeUsd", orderDirection: "desc", limit: $limit) {
    items {
      projectId
      chainId
      version
...
query TrendingProjects($limit: Int!) {
  projects(orderBy: "trendingScore", orderDirection: "desc", limit: $limit) {
    items {
      projectId
      chainId
      name
...
query GetNFTHook($projectId: Int!, $chainId: Int!) {
  nftHooks(where: { projectId: $projectId, chainId: $chainId }, limit: 1) {
    items {
      address
      name
      symbol
...
query ListPermissionHolders($projectId: Int!, $chainId: Int!) {
  permissionHolders(where: { projectId: $projectId, chainId: $chainId }, limit: 100) {
    items {
      account
      operator
      permissions
...
```graphql
query GetParticipantHistory($projectId: Int!, $chainId: Int!, $address: String!, $limit: Int!) {
...
async function main() {
  ...
  if (project.suckerGroupId) {
    ...
    console.log(
      `Omnichain total: ${omni.volume} wei volume across ${omni.projects_rel.length} chains`,
    )
  }
...
const balances = await Promise.all(chains.map((chainId) => getProjectBalance(projectId, chainId)))
