...
async function build() {
  ...
  // Build agent details
  const agentDetails = agentFiles.map((a) => createAgentDetails(a.name, a.content)).join('\n\n')
  ...
  if (skillContent) {
    ...
    skillContent = skillContent.replace(/version: "[^"]*"/, `version: "${version}"`)

    // Update version in content
    skillContent = skillContent.replace(/\*\*CC_GodMode v[0-9.]+ - /, `**CC_GodMode v${version} - `)
