...
export class ASTAnalyzer {
  ...
  detectStringConstruction() {
    ...
    while ((match = concatPattern.exec(this.content)) !== null) {
      ...
      for (const dangerous of dangerousStrings) {
        if (
          dangerous.toLowerCase().includes(constructed.toLowerCase()) ||
    ...
    const reversePattern =
      /['"`](\w+)['"`]\s*\.\s*split\s*\([^)]*\)\s*\.\s*reverse\s*\(\s*\)\s*\.\s*join/g
  ...
  detectBracketNotation() {
    ...
    const dynamicBracketPattern =
      /\b(global|window|process|module|require|globalThis)\s*\[\s*[a-zA-Z_$]\w*\s*\]/g
  ...
  detectVariableAliasing() {
    ...
    for (const fn of dangerousFunctions) {
      const aliasPattern = new RegExp(
        `(?:const|let|var)\\s+(\\w+)\\s*=\\s*(?:global\\.)?${fn}\\b`,
        'g',
      )
    ...
    const destructPattern =
      /\{\s*(\w+)\s*:\s*(\w+)\s*\}\s*=\s*(?:require\s*\(['"]child_process['"]\)|\w+)/g
    ...
    const wrapperPattern =
      /(?:const|let|var|function)\s+(\w+)\s*=?\s*(?:\((\w+)\)\s*=>|function\s*\(\s*(\w+)\s*\))\s*(?:{\s*return\s+)?require\s*\(\s*\2?\3?\s*\)/g
    ...
    const envHarvestPattern =
      /Object\s*\.\s*(?:entries|keys|values)\s*\(\s*process\s*\.\s*env\s*\)[\s\S]{0,100}\.filter\s*\(/g
    ...
    const envRegexPattern =
      /(?:key|secret|token|pass|cred|auth).*\.test\s*\(\s*\w+\s*\)|\.test\s*\(\s*\w+\s*\)[\s\S]{0,30}(?:key|secret|token|pass|cred|auth)/gi
    ...
    const encodeExfilPattern =
      /encodeURIComponent\s*\(\s*(?:key|val|value|secret|token|password|cred)/gi
  ...
  detectSandboxDetection() {
    const patterns = [
      {
        regex: /process\.env\.NODE_ENV\s*[!=]==?\s*['"](?:test|development|debug)['"]/,
        msg: 'Checks NODE_ENV — may behave differently in test vs production',
      },
      ...
      {
        regex: /process\.env\.DOCKER|process\.env\.KUBERNETES|\/\.dockerenv/,
        msg: 'Container detection — may change behavior in containers',
      },
      {
        regex: /os\s*\.\s*hostname\s*\(\)|os\s*\.\s*userInfo\s*\(\)/,
        msg: 'Fingerprints host environment',
      },
      {
        regex: /process\s*\.\s*ppid|process\s*\.\s*pid/,
        msg: 'Checks process IDs — may detect debugging/analysis',
      },
      {
        regex: /performance\s*\.\s*now\s*\(\).*performance\s*\.\s*now\s*\(\)/s,
        msg: 'Timing checks — may detect analysis slowdown',
      },
    ]
    ...
    for (const { regex, msg } of patterns) {
      ...
      while ((match = globalRegex.exec(this.content)) !== null) {
        this.addFinding(match.index, 'SANDBOX_DETECTION', msg, 'high', 'evasion', 15)
      }
    }
  }
  ...
  detectDataFlowChains() {
    ...
    const hasCredentialRead =
      /readFile|readFileSync|process\.env|credentials|auth|api[_-]?key|secret|token|password/i.test(
        this.content,
      )
    const hasEncoding =
      /btoa|atob|base64|Buffer\.from|JSON\.stringify|encodeURI|encode\(|toString\(['"]hex['"]\)/i.test(
        this.content,
      )
    const hasNetworkSend =
      /fetch\s*\(|axios|http\.request|https\.request|net\.connect|\.post\s*\(|\.send\s*\(|webhook|ngrok/i.test(
        this.content,
      )
  ...
  detectObfuscationPatterns() {
    ...
    const evalExprPattern =
      /(?:eval|Function)\s*\(\s*(?:atob|decodeURI|unescape|Buffer\.from)\s*\(/g
  ...
  detectEnvironmentProbing() {
    ...
    const existsPattern =
      /(?:existsSync|access)\s*\(\s*['"`](?:\/etc|\/home|\/root|\/var|~|process\.env\.HOME)/g
