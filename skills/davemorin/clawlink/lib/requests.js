      ...
      if (friend) {
        ...
        if (pendingOut && msg.fromX25519) {
          ...
          try {
            ...
            const sharedSecret = crypto.deriveSharedSecret(identity.x25519SecretKey, theirX25519)
...
export async function acceptFriendRequest(requestId) {
  ...
  // Find the request
  const request = pending.incoming.find(
    (r) => r.id === requestId || r.from?.toLowerCase().includes(requestId.toLowerCase()),
  )
  ...
  const sharedSecret = crypto.deriveSharedSecret(identity.x25519SecretKey, request.fromX25519)
