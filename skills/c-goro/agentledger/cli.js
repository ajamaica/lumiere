...
class CLI {
  ...
  async handleLog(args) {
    ...
    if (options.amount && options.vendor && options.description) {
      ...
      if (positionalArgs.length < 3) {
        ...
        console.error(
          '   OR: agentledger log --amount=<amount> --vendor=<vendor> --description=<description> [options]',
        )
    ...
    const displayAmount =
      result.currency === 'USD' ? `$${result.amount}` : `${result.amount} ${result.currency}`
    ...
    switch (action) {
      case 'set':
        if (!category || !amount) {
          ...
          console.error(
            '   OR: agentledger budget --set --category=<category> --amount=<amount> [--period=<period>]',
          )
          process.exit(1)
        }
  ...
  async handleQuery(args) {
    ...
    if (query.includes('spend') && query.includes('month')) {
      ...
      console.log(
        `You've spent $${summary.total.toFixed(2)} this month across ${summary.count} transactions.`,
      )
