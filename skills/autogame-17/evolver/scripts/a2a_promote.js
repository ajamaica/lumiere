...
function main() {
  ...
  const typeRaw = String(args.kv.get('type') || '')
    .trim()
    .toLowerCase()
  ...
  const candidate = (Array.isArray(external) ? external : []).find(
    (x) => x && x.type === type && String(x.id) === id,
  )
  if (!candidate) {
  ...
  const exists =
    Array.isArray(localGenes) &&
    localGenes.some((g) => g && g.type === 'Gene' && String(g.id) === id)
  if (exists) {
