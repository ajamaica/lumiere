...
import { CHAIN, CONTRACTS } from '../../../shared/contracts.js'
...
async function list(limit = 10, offset = 0) {
  ...
  for (const token of tokens) {
    ...
    console.log(
      `${token.symbol} | ${token.token} | FDV: ${fdvEth} ETH | ${date.toISOString().split('T')[0]}`,
    )
  }
}
...
async function create(opts: {
  ...
  if (fdv && opts.creator) {
    hash = await walletClient.writeContract({
      ...
      args: [opts.name, opts.symbol, image, fdv, opts.creator as `0x${string}`],
    })
  } else if (fdv) {
    hash = await walletClient.writeContract({
      address: CONTRACTS.FACTORY as `0x${string}`,
    ...
    switch (command) {
      ...
      case 'create': {
        ...
        if (!opts.name || !opts.symbol) {
          console.error(
            'Usage: pumpclaw.ts create --name <name> --symbol <symbol> [--image <url>] [--fdv <eth>] [--creator <address>]',
          )
          process.exit(1)
        }
