...
> _"Memory is where the spirit rests."_
...
Patterns the agent has learned for _how_ to do things. These are templates, not events.
...
When a remember trigger fires, the agent makes a classification call to determine _where_ the memory goes:
...
Each reflection cycle is capped at **8,000 tokens of processing output** (not input — the engine can _read_ as much as it needs, but its _output_ is bounded). This prevents runaway consolidation costs while allowing genuine depth.
...
4. **Multi-user support (group chats)**: The current design is single-user. If the bot serves multiple _human users_ (e.g., group chats, team workspaces), how should memories be scoped? (Note: multi-_agent_ access is addressed in § 13 — this is about multiple humans.)
...
_This is a living document. It will evolve as implementation reveals what works and what doesn't._
