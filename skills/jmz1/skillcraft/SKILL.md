...
**Assumes:** The agent knows how to write code, structure projects, and handle errors. This skill teaches _clawdbot-specific_ concerns.
...
_If extracting:_ Derive from actual usage, not just hypotheticals. It's ok to generalise the problem if that's what the user wants.
...
_If extracting:_ Also decide what stays in workspace (user config, state) vs. what moves to skill (scripts, instructions, references).
...
The split: scripts handle the _mechanics_, instructions handle the _judgment_.
...
_If extracting:_ Include migration notes — what moves where, what workspace files need updating.
...
2. **SKILL.md skeleton** — frontmatter + section headers
   → _Review: is the structure right?_
3. **Scripts** (if any) — get executable pieces working
   → _Review: test each script_
4. **SKILL.md body** — complete instructions
5. **Test against Stage 1 examples**
   → _Review: does it handle all examples?_

_If extracting:_ 6. Update workspace files (remove migrated content, add skill references) 7. Clean up old locations 8. Verify skill works standalone
...
metadata:
  {
    'clawdbot':
      { 'emoji': '🔧', 'requires': { 'bins': ['tool'], 'env': ['API_KEY'] }, 'install': [...] },
  }
---
```
...
**Principle:** Skills document _what_ workspace files they touch and _why_.
