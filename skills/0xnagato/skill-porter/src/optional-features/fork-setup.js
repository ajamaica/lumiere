...
export class ForkSetup {
   ...
   */
  async setup(options = {}) {
    const { forkLocation, repoUrl, branchName = 'dual-platform-setup' } = options
  ...
  async _ensureDualPlatform(forkPath) {
    ...
    const hasGeminiConfig = await this._checkFileExists(
      path.join(forkPath, 'gemini-extension.json'),
    )
