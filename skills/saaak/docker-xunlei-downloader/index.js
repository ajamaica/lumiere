...
Commands:
  ...
  async getStatus() {
    try {
      ...
      tasks.forEach((task, index) => {
        ...
        const speed = task.params?.speed
          ? Math.round((task.params.speed / 1024 / 1024) * 100) / 100
          : 0 // Convert to MB/s
  ...
  async submitTask(args) {
    ...
    try {
      ...
      console.log(
        `File analysis complete: ${classified.mainContent.length} main content, ${classified.ads.length} ads`,
      )
      ...
      if (selectedFiles.length === 0) {
        ...
        const sortedFiles = classified.all.sort(
          (a, b) => parseInt(b.file_size) - parseInt(a.file_size),
        )
