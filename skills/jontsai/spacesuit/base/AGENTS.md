...
**When making changes:** 4. Make ONLY the intended change 5. `git diff` — review what changed 6. Verify diff matches expected change EXACTLY 7. `git add <specific-files>` — never `git add .` blindly 8. `git commit -m "..."` — atomic, descriptive

**After finishing:** 9. `git status` — confirm clean main branch 10. Report any stashed/branched WIP 11. **ALWAYS explicitly state commits** — even for side-quests: - "**Committed:** `<file(s)>` — `<message>`" - Include commit hash AND repo link when pushed
