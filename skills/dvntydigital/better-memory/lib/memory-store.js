...
export class MemoryStore {
  ...
  async store(content, options = {}) {
    await this._ensureDb()
    const { metadata = {}, priority = 5, sessionId = null, tags = [] } = options
    ...
    if (exactStmt.step()) {
      ...
      exactStmt.free()
      this.db.run(
        'UPDATE memories SET last_accessed_at = ?, priority = MAX(priority, ?) WHERE id = ?',
    ...
    if (nearest.length > 0) {
      ...
      this.db.run(
        'UPDATE memories SET last_accessed_at = ?, priority = MAX(priority, ?), access_count = access_count + 1 WHERE id = ?',
  ...
  async search(query, options = {}) {
    await this._ensureDb()
    const { limit = 5, threshold = 0.5, sessionId = null, useDecay = false } = options
    ...
    for (const c of topCandidates) {
      const stmt = this.db.prepare(
        'SELECT id, content, metadata, priority, tags, session_id, created_at, last_accessed_at, access_count FROM memories WHERE id = ?',
      )
      stmt.bind([c.id])
      if (stmt.step()) {
        const row = stmt.getAsObject()
