...
async function analyzeData(state: AgentState) {
  ...
  const analysis = {
    ...
    insights: ['Price trend analysis', 'Volume pattern', 'Market sentiment'],
  }
...
workflow.addConditionalEdges('classify', routeByType, {
  query: 'query',
  analysis: 'analysis',
...
// Good: Parallel
const [btcData, ethData] = await Promise.all([fetchToken('bitcoin'), fetchToken('ethereum')])
```
