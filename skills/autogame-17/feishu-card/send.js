...
async function uploadImage(filePath) {
  ...
  if (fs.existsSync(IMAGE_KEY_CACHE_FILE)) {
    try {
      cache = JSON.parse(fs.readFileSync(IMAGE_KEY_CACHE_FILE, 'utf8'))
    } catch (e) {}
  }
...
function scanForSecrets(content) {
  ...
  for (const p of secretPatterns) {
    if (p.test(content)) {
      console.error(
        '\x1b[31m%s\x1b[0m',
        '⛔ SECURITY ALERT: Potential secret detected in message body.',
      )
...
async function resolveContent(options) {
  ...
  if (options.textFile) {
    ...
    const isPotentialPath =
      options.text.length < 255 && !options.text.includes('\n') && !/[<>:"|?*]/.test(options.text)
