...
class XunleiDockerClient {
  ...
  async getDeviceId() {
    try {
      const response = await this.request(
        'GET',
        '/webman/3rdparty/pan-xunlei-com/index.cgi/drive/v1/tasks?type=user%23runner&device_space=',
      )
  ...
  async extractFileList(magnetLink) {
    try {
      const response = await this.request(
        'POST',
        '/webman/3rdparty/pan-xunlei-com/index.cgi/drive/v1/resource/list?device_space=',
        {
          urls: magnetLink,
        },
      )
  ...
  async submitTask(magnetLink, taskName = '', selectedFiles = []) {
    ...
    if (!this.parentFolderId) {
      try {
        const res = await this.request(
          'GET',
          `/webman/3rdparty/pan-xunlei-com/index.cgi/drive/v1/files?space=${encodeURIComponent(this.deviceId)}&limit=200&parent_id=&filters=%7B%22kind%22%3A%7B%22eq%22%3A%22drive%23folder%22%7D%7D&page_token=&device_space=`,
        )
    ...
    try {
      const response = await this.request(
        'POST',
        '/webman/3rdparty/pan-xunlei-com/index.cgi/drive/v1/task?device_space=',
        submitBody,
      )
