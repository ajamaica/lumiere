...
async function generateResponse(contact, incomingMessage, messageHistory) {
  const historyText = messageHistory
    ...
    .map(
      (m) =>
        `${m.is_from_me ? 'Me' : contact.name || contact.identifier}: ${m.text || '[attachment]'}`,
    )
    .join('\n')
...
Generate a response now:`
  ...
  if (!apiKey && fs.existsSync(configPath)) {
    try {
      ...
      apiKey =
        config.skills?.['openai-whisper-api']?.apiKey || config.skills?.['openai-image-gen']?.apiKey
    } catch (e) {
...
function shouldRespond(contact, state, config, messageText) {
  ...
  const minMs =
    (contact.minMinutesBetweenReplies !== undefined
      ? contact.minMinutesBetweenReplies
      : config.defaultMinMinutesBetweenReplies) *
    60 *
    1000
  ...
  if (!messageMatchesKeywords(contact, messageText)) {
    log(
      `Keyword filter: Message doesn't contain required keywords for ${contact.name || identifier}`,
    )
    return false
  }
  ...
  if (contact.maxRepliesPerDay && contact.maxRepliesPerDay > 0) {
    ...
    if (stats) {
      ...
      if (stats.dailyCount >= contact.maxRepliesPerDay) {
        log(
          `Daily cap: Already sent ${stats.dailyCount} responses today for ${contact.name || identifier} (max: ${contact.maxRepliesPerDay})`,
        )
        return false
      }
    }
  ...
  if (elapsed < minMs) {
    log(
      `Rate limit: Only ${Math.floor(elapsed / 1000 / 60)} minutes since last response to ${identifier} (need ${Math.floor(minMs / 1000 / 60)})`,
    )
    return false
  }
...
async function checkForNewMessages(config, state) {
  try {
    ...
    for (const chat of chats) {
      const contact = config.watchList.find(
        (c) => c.identifier === chat.identifier && c.enabled !== false,
      )
      ...
      log(
        `New message from ${contact.name || contact.identifier}: ${latestMessage.text || '[attachment]'}`,
      )
...
async function startPolling() {
  ...
  log(
    `Watching ${config.watchList.filter((c) => c.enabled !== false).length} contacts (polling every ${POLL_INTERVAL / 1000}s)`,
  )
