...
function convertMarkdownToBlocks(markdown) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('# ')) {
      blocks.push({
        block_type: 3,
        heading1: { elements: [{ text_run: { content: trimmed.substring(2) } }] },
      })
    } else if (trimmed.startsWith('## ')) {
      blocks.push({
        block_type: 4,
        heading2: { elements: [{ text_run: { content: trimmed.substring(3) } }] },
      })
    } else if (trimmed.startsWith('### ')) {
      blocks.push({
        block_type: 5,
        heading3: { elements: [{ text_run: { content: trimmed.substring(4) } }] },
      })
    } else if (trimmed.startsWith('- ')) {
      blocks.push({
        block_type: 12,
        bullet: { elements: [{ text_run: { content: trimmed.substring(2) } }] },
      })
    } else {
