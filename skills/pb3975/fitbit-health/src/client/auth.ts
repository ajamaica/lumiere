...
import { FitbitErrorResponse, FitbitTokenResponse } from '../types/fitbit.js'
...
export async function startOAuthFlow(config: ConfigData, verbose = false): Promise<OAuthResult> {
  ...
  const { port, server, waitForCallback } = await startCallbackServer(config.callbackPort ?? 18787)
