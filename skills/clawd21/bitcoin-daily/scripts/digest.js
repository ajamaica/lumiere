...
async function fetchThread(thread) {
  try {
    ...
    const datePatterns =
      text.match(/(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2}(?:,\s*\d{4})?/gi) ||
      []
...
function generateSummary(date, threads, commits) {
  ...
  if (validThreads.length === 0) {
    ...
    for (let i = 0; i < validThreads.length; i++) {
      ...
      const preview = t.content.replace(/\s+/g, ' ').substring(0, 500).trim()
  ...
  if (commits.length === 0) {
    ...
    for (const c of commits) {
      ...
      const prLink = prMatch
        ? `[#${prMatch[1]}](https://github.com/bitcoin/bitcoin/pull/${prMatch[1]})`
        : ''
