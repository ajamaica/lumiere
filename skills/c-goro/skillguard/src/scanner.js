...
export class SkillScanner {
    ...
    for (const file of files) {
      ...
      if (TEXT_EXTENSIONS.has(ext) || ext === '.md') {
        if (file.relativePath !== 'SKILL.md') {
          // Already scanned SKILL.md above
    ...
    const checks = [
      {
        ...
        msg: 'Dynamic __import__() — bypasses static import analysis',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /\bgetattr\s*\([^,]+,\s*['"]/g,
        msg: 'getattr() — dynamic attribute access may evade analysis',
        severity: 'high',
        weight: 15,
      },
      {
        regex: /\bcompile\s*\([^)]+['"]exec['"]/g,
        msg: 'compile() with exec mode — dynamic code execution',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /\bpickle\s*\.\s*loads?\s*\(/g,
        msg: 'pickle deserialization — can execute arbitrary code',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /\bmarshall?\s*\.\s*loads?\s*\(/g,
        msg: 'marshal deserialization — can execute arbitrary code',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /\bsubprocess\s*\.\s*(?:call|run|Popen|check_output)/g,
        msg: 'subprocess execution',
        severity: 'high',
        weight: 15,
      },
      {
        regex: /\bctypes\s*\.\s*(?:cdll|windll|CDLL)/g,
        msg: 'ctypes foreign function interface — native code execution',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /\bos\s*\.\s*system\s*\(/g,
        msg: 'os.system() — shell command execution',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /\bos\s*\.\s*popen\s*\(/g,
        msg: 'os.popen() — shell command execution',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /\b(?:yaml\s*\.\s*(?:load|unsafe_load))\s*\(/g,
        msg: 'Unsafe YAML loading — can execute arbitrary code',
        severity: 'critical',
        weight: 25,
      },
    ]
  ...
  shellAnalysis(content, filePath, report) {
    const checks = [
      {
        regex: /\bcurl\s+[^|]*\|\s*(?:bash|sh|zsh)/g,
        msg: 'Pipe curl to shell — remote code execution',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /\bwget\s+[^|]*\|\s*(?:bash|sh|zsh)/g,
        msg: 'Pipe wget to shell — remote code execution',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /\beval\s+"\$\(/g,
        msg: 'eval with command substitution',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /\bbase64\s+(?:-d|--decode)/g,
        msg: 'Base64 decode in shell — may hide payload',
        severity: 'high',
        weight: 20,
      },
      {
        regex: /\bnc\s+(?:-[elp]|--listen)/g,
        msg: 'Netcat listener — reverse shell indicator',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /\/dev\/tcp\//g,
        msg: 'Bash /dev/tcp — network connection without external tools',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /\bmkfifo\b.*\bnc\b|\bnc\b.*\bmkfifo\b/g,
        msg: 'Named pipe + netcat — reverse shell pattern',
        severity: 'critical',
        weight: 30,
      },
    ]
  ...
  async applyContextScoring(report) {
    ...
    for (const file of report.files) {
      if (file.type === 'code' || file.type === 'text') {
        ...
        if (content) {
          ...
          const urlConsts = content.matchAll(
            /(?:const|let|var)\s+\w+\s*=\s*['"`](https?:\/\/[^'"`\s]+)['"`]/g,
          )
    ...
    for (const finding of report.findings) {
      ...
      if (finding.ruleId === 'CRED_ACCESS' || finding.category === 'credential-theft') {
        ...
        const accessesDeclaredVar = declaredEnvVars.some(
          (envVar) => finding.context.includes(envVar) || finding.match.includes(envVar),
        )
        ...
        if (
          /process\.env\.\w+/.test(finding.context) &&
          !finding.context.includes('Object.entries')
        ) {
        ...
        if (/api[_-]?key/i.test(finding.match) && finding.file.match(/\.(py|js|ts|rb)$/)) {
          ...
          const isVariableUsage =
            /(?:def |self\.|=|,|\(|:)\s*api[_-]?key/i.test(ctx) ||
      ...
      if (finding.category === 'data-exfiltration' || finding.ruleId === 'NETWORK_EXFIL') {
        const matchedGood =
          KNOWN_GOOD_APIS.some(
            (api) => finding.context.includes(api) || finding.match.includes(api),
          ) || fileTargetsKnownAPI
      ...
      const isDocFile = /^(SKILL\.md|README|CHANGELOG|LICENSE|.*README.*\.md|SERVER_README)/i.test(
        finding.file,
      )
    ...
    const allCredDeclared = report.findings
      .filter(
        (f) => (f.category === 'credential-theft' || f.ruleId === 'CRED_ACCESS') && f.weight > 0,
      )
      .every(
        (f) =>
          f.severity === 'info' ||
          (f.contextNote || '').includes('declared') ||
          (f.contextNote || '').includes('standard code'),
      )

    const allNetworkKnown = report.findings
      .filter(
        (f) => (f.category === 'data-exfiltration' || f.ruleId === 'NETWORK_EXFIL') && f.weight > 0,
      )
      .every(
        (f) =>
          f.severity === 'info' ||
          f.severity === 'low' ||
          (f.contextNote || '').includes('Declared') ||
          (f.contextNote || '').includes('Known-good') ||
  ...
  detectBehavioralSignatures(report) {
    ...
    if (
      (categories.has('credential-theft') || ruleIds.has('CRED_ACCESS')) &&
      (categories.has('data-exfiltration') || ruleIds.has('NETWORK_EXFIL'))
    ) {
      report.behavioralSignatures.push({
        name: 'DATA_EXFILTRATION',
        description:
          'Credential access combined with network activity — classic exfiltration pattern',
        severity: 'critical',
        confidence: 'high',
      })
    ...
    if (
      ruleIds.has('PROMPT_INJECTION') &&
      (categories.has('code-execution') || ruleIds.has('EXEC_CALL'))
    ) {
      report.behavioralSignatures.push({
        name: 'TROJAN_SKILL',
        description:
          'Prompt injection + code execution — skill injects instructions and executes code',
        severity: 'critical',
        confidence: 'high',
      })
    }
    ...
    if (
      (ruleIds.has('STRING_CONSTRUCTION') ||
        ruleIds.has('ENCODED_STRING') ||
        ruleIds.has('FUNCTION_ALIAS') ||
        ruleIds.has('DYNAMIC_IMPORT')) &&
      (categories.has('code-execution') || categories.has('credential-theft'))
    ) {
      report.behavioralSignatures.push({
        name: 'EVASIVE_MALWARE',
        description:
          'Code obfuscation/evasion + dangerous behavior — actively trying to hide malicious intent',
        severity: 'critical',
        confidence: 'high',
      })
      report.findings.push({
        ...
        category: 'behavioral',
        title:
          '⚠️ BEHAVIORAL: Evasion techniques + dangerous operations = evasive malware signature',
        file: '(compound)',
        line: 0,
        match: '',
    ...
    if (
      ...
      (categories.has('code-execution') || categories.has('data-exfiltration'))
    ) {
      report.behavioralSignatures.push({
        name: 'PERSISTENT_BACKDOOR',
        description:
          'Persistence mechanism + code execution/exfiltration — establishes ongoing unauthorized access',
        severity: 'critical',
        confidence: 'high',
      })
    }
  ...
  checkFileStructure(files, report) {
    for (const f of files) {
      ...
      if (
        name.startsWith('.') &&
        !['gitignore', '.env.example', '.eslintrc', '.prettierrc'].some(
          (n) => name === n || name === '.' + n,
        )
      ) {
        report.findings.push({
          ...
          file: f.relativePath,
          line: 0,
          context: `Hidden file: ${f.relativePath}`,
          weight: 10,
        })
      }
    }
    ...
    for (const f of files) {
      if (f.size > 512000) {
        report.findings.push({
          ...
          file: f.relativePath,
          line: 0,
          context: `${f.relativePath}: ${(f.size / 1024).toFixed(0)}KB`,
          weight: 5,
        })
      }
    }

    const hasBundledDeps = files.some(
      (f) => f.relativePath.includes('node_modules/') || f.relativePath.includes('__pycache__/'),
    )
    if (hasBundledDeps) {
  ...
  recheckBehavioralSuppression(report) {
    ...
    const netFindings = report.findings.filter(
      (f) => (f.category === 'data-exfiltration' || f.ruleId === 'NETWORK_EXFIL') && f.weight > 0,
    )
    ...
    const allCredOk =
      ...
      credFindings.every(
        (f) =>
          ...
          (f.contextNote || '').includes('declared') ||
          (f.contextNote || '').includes('standard code') ||
          (f.contextNote || '').includes('expected'),
      )
    const allNetOk =
