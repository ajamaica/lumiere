...
import { getMe, login, logout } from '../lib/auth.js'
import {
  getCurrentAccountSlug,
  getToken,
  isAuthenticated,
  setCurrentAccountSlug,
} from '../lib/config.js'
  ...
  auth
    ...
    .action(async () => {
      try {
        if (isAuthenticated()) {
          console.log(
            chalk.yellow('Already authenticated. Use "fizzy auth logout" to logout first.'),
          )
          return
        }
...
export function createAccountCommand(): Command {
  ...
  account
    ...
    .action(async () => {
      ...
      if (!currentSlug) {
        console.log(
          chalk.yellow('No account selected. Run "fizzy account set <slug>" to select one.'),
        )
        return
      }
