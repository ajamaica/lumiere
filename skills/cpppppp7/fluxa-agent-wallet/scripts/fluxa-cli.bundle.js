...
async function registerAgent(params) {
  ...
  if (!data.agent_id || !data.token || !data.jwt) {
    throw new Error('Invalid registration response: missing agent_id, token, or jwt')
  }
  return data
}
...
async function createPayout(params, jwt) {
  ...
  if (!response.ok) {
    throw new WalletApiError(
      text || `Wallet API request failed (${response.status})`,
      response.status,
      text || null,
    )
  }
...
async function getPayoutStatus(payoutId) {
  ...
  if (!response.ok) {
    throw new WalletApiError(
      text || `Wallet App status request failed (${response.status})`,
      response.status,
      text || null,
    )
  }
...
var DATA_ROOT = process.env.FLUXA_DATA_DIR
...
function hasAgentId() {
  ...
  return Boolean(effective && effective.agent_id && effective.token && effective.jwt)
}
...
async function cmdInit(options) {
  ...
  if (!email || !agentName || !clientInfo) {
    return {
      success: false,
      error:
        'Missing required parameters: --email, --name, --client (or set AGENT_EMAIL, AGENT_NAME, CLIENT_INFO)',
    }
  }
  try {
...
async function cmdX402(options) {
  ...
  const request = {
    ...
    currency: getCurrencyFromAsset(
      accept.asset || DEFAULT_ASSET,
      accept.network || DEFAULT_NETWORK,
    ),
