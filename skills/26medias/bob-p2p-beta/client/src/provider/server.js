...
class ProviderServer {
  ...
  setupRoutes() {
    ...
    this.app.post('/api/:apiId/execute', async (req, res) => {
      try {
        const { apiId } = req.params
        const { queueCode, transactionSignature, walletAddress, params } = req.body

        // Validate required fields
        if (!queueCode || !transactionSignature || !walletAddress || !params) {
          return res.status(400).json({
            error:
              'Missing required fields: queueCode, transactionSignature, walletAddress, params',
          })
        }
        ...
        // Create job
        const jobId = await this.jobs.createJob(apiId, params, walletAddress, transactionSignature)

        // Mark transaction as used
        await this.payment.markTransactionUsed(transactionSignature, jobId, queueData.price)
