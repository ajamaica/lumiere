...
import type { ConsoleEvent, Snapshot } from './types.js'
import { detectModals, elementExists, findByRole, findByText } from './utils/snapshot.js'
...
export function expectVisible(snapshot: Snapshot, selector: string): void {
  if (!elementExists(snapshot, selector)) {
    throw new AssertionError(`Element not visible: ${selector}`, 'not found', 'visible')
  }
}
...
export function expectNotVisible(snapshot: Snapshot, selector: string): void {
  if (elementExists(snapshot, selector)) {
    throw new AssertionError(`Element should not be visible: ${selector}`, 'visible', 'not visible')
  }
}
...
export function expectText(
  ...
  if (!found) {
    throw new AssertionError(`Element not found: ${selector}`, 'not found', text)
  }
  ...
  if (!matches) {
    throw new AssertionError(`Text mismatch for ${selector}`, actual, text)
  }
}
...
export function expectUrl(
  ...
  if (typeof expected === 'string') {
    ...
    if (!actual.includes(expected.contains)) {
      throw new AssertionError(`URL does not contain expected string`, actual, expected.contains)
    }
  } else if (expected.matches) {
    if (!expected.matches.test(actual)) {
      throw new AssertionError(`URL does not match pattern`, actual, expected.matches.toString())
    }
  }
}
...
export function expectCount(
  ...
  if (typeof expected === 'number') {
    if (count !== expected) {
      throw new AssertionError(`Element count mismatch for role "${role}"`, count, expected)
    }
  } else {
...
export function expectNoErrors(events: ConsoleEvent[]): void {
  ...
  if (errors.length > 0) {
    ...
    throw new AssertionError(`Console errors detected:\n${messages}`, errors.length, 0)
  }
}
 ...
 */
export function expectConsoleEvent(events: ConsoleEvent[], pattern: string | RegExp): void {
  const regex = typeof pattern === 'string' ? new RegExp(pattern, 'i') : pattern
  const found = events.some((e) => regex.test(e.text))

  if (!found) {
    throw new AssertionError(`Console event not found: ${pattern}`, 'not found', pattern.toString())
  }
}
...
export function expectState(
  ...
  if (!ref) {
    throw new AssertionError(`Element not found: ${selector}`, 'not found', state)
  }

  const actual = ref.state?.[state] ?? false

  if (actual !== expected) {
    throw new AssertionError(`Element ${selector} state "${state}" mismatch`, actual, expected)
  }
}
...
export function expectModal(snapshot: Snapshot, present: boolean = true): void {
  ...
  if (present && modals.length === 0) {
    throw new AssertionError('Expected modal to be present', 'no modal', 'modal visible')
  }
...
export function expectTitle(
  ...
  if (!matches) {
    throw new AssertionError('Page title mismatch', actual, title)
  }
}
...
export function expectClickable(snapshot: Snapshot, selector: string): void {
  ...
  if (!ref) {
    throw new AssertionError(`Element not found: ${selector}`, 'not found', 'clickable')
  }

  const clickableRoles = ['button', 'link', 'menuitem', 'tab', 'checkbox', 'radio', 'switch']

  if (!clickableRoles.includes(ref.role.toLowerCase())) {
    throw new AssertionError(`Element ${selector} is not clickable`, ref.role, 'clickable role')
  }

  if (ref.state?.disabled) {
    throw new AssertionError(`Element ${selector} is disabled`, 'disabled', 'enabled')
  }
}
 ...
 */
export function softExpect<T>(fn: () => T): { ok: true; value: T } | { ok: false; error: Error } {
  try {
    const value = fn()
