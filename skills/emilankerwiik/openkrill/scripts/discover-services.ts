...
async function discoverServices(options: DiscoverOptions = {}): Promise<DiscoveryResponse | null> {
  const { limit = 20, offset = 0, type = 'http', facilitatorUrl = DEFAULT_FACILITATOR } = options
  ...
  try {
    ...
    console.error(
      'Error querying discovery endpoint:',
...
export { DiscoveredResource, DiscoverOptions, discoverServices, DiscoveryResponse, PaymentOption }
