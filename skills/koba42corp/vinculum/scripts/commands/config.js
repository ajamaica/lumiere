...
async function setConfig(key, value, context) {
  ...
  switch (normalizedKey) {
    case 'share_activity':
      ...
      return formatting.formatSuccess(`Activity sharing ${shareActivity ? 'enabled' : 'disabled'}.`)
    ...
    case 'share_memory':
      ...
      return formatting.formatSuccess(`Memory sharing ${shareMemory ? 'enabled' : 'disabled'}.`)
    ...
    case 'auto_sync':
      ...
      return formatting.formatSuccess(`Auto-sync ${autoSync ? 'enabled' : 'disabled'}.`)

    case 'sync_interval':
      const interval = parseInt(value, 10)
      if (isNaN(interval) || interval < 5 || interval > 3600) {
        return formatting.formatError(`Invalid interval. Must be between 5 and 3600 seconds.`)
      }
      await configManager.set({ sync_interval: interval })
      return formatting.formatSuccess(`Sync interval set to ${interval} seconds.`)
    ...
    case 'agent_name':
      ...
      return formatting.formatSuccess(`Agent name set to "${value.trim()}".`)
