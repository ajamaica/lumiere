...
import { getCampfireLines, listCampfires, sendCampfireLine } from '../lib/api.js'
import { isAuthenticated } from '../lib/config.js'

export function createCampfiresCommands(): Command {
  const campfires = new Command('campfires').description('Manage campfires (chat)')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        campfireList.forEach((campfire) => {
          table.push([campfire.id, campfire.title || 'Campfire', campfire.topic || '-'])
        })

        console.log(table.toString())
      } catch (error) {
        console.error(
          chalk.red('Failed to list campfires:'),
  ...
  campfires
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to get campfire lines:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
