...
async function getSleep(limit = 3, opts = {}) {
  ...
  if (records.length > 0) {
    ...
    records.forEach((rec) => {
      ...
      console.log(
        `  Time Asleep: ${formatDuration(stages.total_in_bed_time_milli - stages.total_awake_time_milli)}`,
      )
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    ...
    const computedStart =
      start ||
      (Number.isFinite(days) ? new Date(Date.now() - days * 86400_000).toISOString() : null) ||
      parseSince(since)

    const records = await getSleep(limit, {
      json: isJson,
      jsonl: isJsonl,
      start: computedStart,
      end: end || null,
