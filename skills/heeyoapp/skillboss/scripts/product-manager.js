...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--help' || arg === '-h') {
      ...
      if (!parsed.command) {
        parsed.command = arg
      } else if (!parsed.productId && ['update', 'delete', 'get'].includes(parsed.command)) {
...
async function getProduct(projectId, productId) {
  ...
  const product = await apiRequest('GET', `/admin-products/${productId}`, null, projectId)
...
async function createProduct(projectId, productData) {
  ...
  const product = await apiRequest('POST', '/admin-products', productData, projectId)
...
async function updateProduct(projectId, productId, productData) {
  ...
  const product = await apiRequest('PUT', `/admin-products/${productId}`, productData, projectId)
