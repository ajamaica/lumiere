...
bot.onTip(async (handler, event) => {
  ...
  if (event.receiverAddress === bot.appAddress) {
    await handler.sendMessage(event.channelId, 'Thanks for ' + formatEther(event.amount) + ' ETH!')
  }
})
```
  ...
  switch (event.response.payload.content?.case) {
    ...
    case 'transaction':
      ...
      if (tx.txHash) {
        ...
        await handler.sendMessage(event.channelId, 'TX: https://basescan.org/tx/' + tx.txHash)
      }
      break
  }
})
