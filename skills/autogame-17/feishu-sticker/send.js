...
async function executeWithAuthRetry(operation) {
  ...
  try {
    ...
    const msg = e.message || ''
    const isAuthError =
      msg.includes('9999166') ||
      (msg.toLowerCase().includes('token') &&
        (msg.toLowerCase().includes('invalid') || msg.toLowerCase().includes('expire')))
  ...
  if (ffmpegPath) {
    try {
      ...
      if (stats.size > LIMIT_BYTES) {
        console.log(
          `[Compression] Image is ${(stats.size / 1024 / 1024).toFixed(2)}MB (>5MB). Compressing...`,
        )
        // Use a temp file for compression output
        const compressedPath = path.join(
          path.dirname(selectedFile),
          `temp_compressed_${Date.now()}.webp`,
        )
  ...
  if (!imageKey) {
    ...
    if (isTempFile && fs.existsSync(selectedFile)) {
      try {
        fs.unlinkSync(selectedFile)
        console.log(
          `[Cleanup] Deleted temporary compressed file (Cache Hit): ${path.basename(selectedFile)}`,
        )
      } catch (cleanupErr) {}
    }
  }
