import type { WhoopCycle, WhoopRecovery, WhoopSleep, WhoopWorkout } from '../types/whoop.js'
...
function calcStats(values: number[]): TrendStats | null {
  ...
  const recentAvg =
    values.slice(0, Math.min(3, values.length)).reduce((a, b) => a + b, 0) /
...
export function analyzeTrends(
  ...
  const strainValues = sortedCycle.filter((c) => c.score?.strain != null).map((c) => c.score.strain)
...
export function generateInsights(
  ...
  if (todaySleep) {
    ...
    const actualSleepTime =
      todaySleep.stage_summary.total_in_bed_time_milli -
      todaySleep.stage_summary.total_awake_time_milli
    const remPct =
      actualSleepTime > 0
  ...
  if (todayCycle && today) {
    const optimalStrain = today.recovery_score >= 67 ? 14 : today.recovery_score >= 34 ? 10 : 6
...
export function formatTrends(data: TrendData, pretty: boolean): string {
  ...
  if (data.recovery) {
    lines.push(
      `💚 Recovery: ${data.recovery.avg}% avg (${data.recovery.min}-${data.recovery.max}) ${arrow(data.recovery.trend)}`,
    )
  }
  if (data.hrv) {
    lines.push(
      `💓 HRV: ${data.hrv.avg}ms avg (${data.hrv.min.toFixed(0)}-${data.hrv.max.toFixed(0)}) ${arrow(data.hrv.trend)}`,
    )
  }
  if (data.rhr) {
    lines.push(
      `❤️ RHR: ${data.rhr.avg}bpm avg (${data.rhr.min}-${data.rhr.max}) ${arrow(data.rhr.trend)}`,
    )
  }
  if (data.sleepPerformance) {
    lines.push(
      `😴 Sleep: ${data.sleepPerformance.avg}% avg (${data.sleepPerformance.min}-${data.sleepPerformance.max}) ${arrow(data.sleepPerformance.trend)}`,
    )
  }
  if (data.sleepHours) {
    lines.push(
      `🛏️ Hours: ${data.sleepHours.avg.toFixed(1)}h avg (${data.sleepHours.min.toFixed(1)}-${data.sleepHours.max.toFixed(1)}) ${arrow(data.sleepHours.trend)}`,
    )
  }
  if (data.strain) {
    lines.push(
      `🔥 Strain: ${data.strain.avg.toFixed(1)} avg (${data.strain.min.toFixed(1)}-${data.strain.max.toFixed(1)}) ${arrow(data.strain.trend)}`,
    )
  }
