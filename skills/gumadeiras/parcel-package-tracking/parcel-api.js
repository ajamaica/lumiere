  ...
  try {
    ...
    res.deliveries.forEach((d) => {
      ...
      if (d.events && d.events.length > 0) {
        ...
        console.log(
          `   Latest: ${lastEvent.date} - ${lastEvent.event} @ ${lastEvent.location || 'Unknown'}`,
        )
      }
      console.log('')
    })
  ...
  try {
    ...
    if (search) {
      ...
      carriers = carriers.filter(
        (c) =>
          c.code.toLowerCase().includes(lowerSearch) || c.name.toLowerCase().includes(lowerSearch),
      )
    }
...
if (command === 'list') {
  ...
  if (!tracking || !carrier || !description) {
    console.error(
      'Usage: parcel-api add --tracking <num> --carrier <code> --description <desc> [--notify]',
    )
    process.exit(1)
  }
