...
class SimpleContextManager {
  ...
  async compactTo(targetTokens) {
    ...
    this.conversation = await this.pruner.adaptiveCompact(
      this.conversation,
      targetTokens,
      'balanced',
    )
...
async function demonstrate() {
  ...
  await manager.addMessage(
    'assistant',
    'Hi there! I can help with programming. What language are you using?',
    5,
  )
  await manager.addMessage(
    'user',
    "I'm using TypeScript. I need to create a function that sorts an array.",
    7,
  )
  await manager.addMessage(
    'assistant',
    "Here's a TypeScript function that sorts an array:\n\n```typescript\nfunction sortArray<T>(arr: T[]): T[] {\n  return [...arr].sort();\n}\n```",
    6,
  )

  let status = manager.getStatus()
  console.log(
    `Initial: ${status.messages} messages, ${status.tokens.used}/${status.tokens.limit} tokens`,
  )
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (i % 5 === 0 && i > 0) {
      ...
      if (status.relevanceStats) {
        console.log(
          `  Relevance: avg ${status.relevanceStats.avg.toFixed(2)}, ` +
  ...
  console.log(
    `Token usage: ${status.tokens.used}/${status.tokens.limit} (${status.tokens.percentage}%)`,
  )
  console.log(`Health: ${status.health}`)
  console.log(`\nStatistics:`)
  ...
  if (status.archive.enabled) {
    ...
    console.log(
      `  Archive size: ${Math.round(status.archive.stats.totalSize / 1024)}KB / ${Math.round(status.archive.stats.maxSize / 1024 / 1024)}MB`,
    )
    console.log(`  Usage: ${Math.round(status.archive.stats.usagePercentage)}%`)
    console.log(
      `  Searches: ${status.archive.stats.searches} (Hits: ${status.archive.stats.hits}, Misses: ${status.archive.stats.misses})`,
    )
  }
  ...
  if (archiveResult.found) {
    ...
    archiveResult.sources.forEach((source, i) => {
      console.log(
        `  Source ${i + 1}: Score ${source.score.toFixed(2)}, ${source.snippetsCount} snippets`,
      )
    })
  ...
  conversation.forEach((msg, i) => {
    const shortContent =
      msg.content.length > 60 ? msg.content.substring(0, 57) + '...' : msg.content
    const compressedFlag = msg.compressed ? ' [C]' : ''
    console.log(
      `  [${i}] ${msg.role.toUpperCase()}${compressedFlag} (prio: ${msg.priority || 5}): ${shortContent}`,
    )
  })
