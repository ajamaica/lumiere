...
import { createPersistStore, indexedDBStorage, mergeSessions, Session } from '../src'
  ...
  (set, get) => ({
    ...
    deleteSession: (id: string) => {
      ...
      set({
        ...
        currentSessionId: currentSessionId === id ? (newSessions[0]?.id ?? null) : currentSessionId,
      })
    },
    ...
    syncWithRemote: (remoteSessions: ChatSession[]) => {
      const { sessions } = get()
      const mergedSessions = mergeSessions(sessions, remoteSessions) as ChatSession[]
