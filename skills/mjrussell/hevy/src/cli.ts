...
import type { ExerciseTemplate, Routine, Set, Workout } from './types.js'

const program = new Command()

program.name('hevy').description('CLI for Hevy workout tracking').version('1.0.0')
...
function printWorkoutSummary(workout: Workout, unit: 'kg' | 'lbs' = 'lbs'): void {
  ...
  console.log(
    `   ⏱️  ${formatTime(workout.start_time)} - ${formatTime(workout.end_time)} (${formatDuration(workout.start_time, workout.end_time)})`,
  )
...
program
  ...
  .option('--json', 'Output as JSON')
  .action(
    async (options: { search?: string; custom?: boolean; muscle?: string; json?: boolean }) => {
      const client = new HevyClient(getConfig())
