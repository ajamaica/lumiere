...
const TOOLS: Record<string, ToolHandler> = {
  search_agents: {
    validate: (args) => (!args[0]?.trim() ? 'Usage: search_agents "<query>"' : null),
    run: async (config, args) => searchAgents(config, args[0]!.trim()),
  },
  vector_search: {
    validate: (args) => (!args[0]?.trim() ? 'Usage: vector_search "<query>" [limit]' : null),
    run: async (config, args) =>
      vectorSearch(config, args[0]!.trim(), args[1] ? parseInt(args[1], 10) : undefined),
  },
  get_agent: {
  ...
  register_agent: {
    ...
    run: async (config, args) => registerAgent(config, args[0]!, args[1], args[2], args[3]),
  },
  start_conversation: {
    validate: (args) =>
  ...
  send_message: {
    validate: (args) =>
      !args[0]?.trim() || !args[1]?.trim() ? 'Usage: send_message "<sessionId>" "<message>"' : null,
    run: async (config, args) => sendMessage(config, args[0]!.trim(), args[1]!.trim()),
  },
  get_history: {
    validate: (args) => (!args[0]?.trim() ? 'Usage: get_history "<sessionId>"' : null),
    run: async (config, args) => getHistory(config, args[0]!.trim()),
  },
  end_session: {
    validate: (args) => (!args[0]?.trim() ? 'Usage: end_session "<sessionId>"' : null),
