...
const CREDENTIALS_PATH =
  process.env.WHOOP_CREDENTIALS_PATH || path.join(DATA_DIR, 'credentials.json')
...
async function main() {
  ...
  const redirectUri =
    (await rlQuestion(rl, `Redirect URI [${redirectDefault}]: `)).trim() || redirectDefault
  ...
  if (!hasTokens) {
    ...
    console.log(
      'Option B (fallback): use Postman to obtain access_token + refresh_token, then paste into token.json.\n',
    )
    console.log(
      'Tip: when using Postman, make sure Client Authentication is "Send client credentials in body" (client_secret_post).\n',
    )
