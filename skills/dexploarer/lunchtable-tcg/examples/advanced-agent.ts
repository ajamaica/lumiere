    ...
    try {
        ...
        if (!state.normalSummonedThisTurn) {
          ...
          if (monster) {
            ...
            decisions.push({
              action: 'SUMMON',
              reasoning,
              parameters: { cardId: monster._id, position },
            })
      ...
      for (const decision of decisions) {
        ...
        try {
          ...
          this.log(
            `Failed to save decision: ${error instanceof Error ? error.message : String(error)}`,
            'warn',
          )
        }
      }
      ...
      case 'game_end':
        ...
        if (event.gameId === this.currentGameId) {
          ...
          try {
            ...
            this.log(
              `Failed to re-enter matchmaking: ${error instanceof Error ? error.message : String(error)}`,
              'error',
            )
          }
        }
