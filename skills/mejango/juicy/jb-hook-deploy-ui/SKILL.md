...
// ... your hook code"
            ...
            <div id="tab-catalog" class="hidden">
              ...
              <div
                id="catalog-preview"
                class="output"
                style="margin-top: 0.5rem; min-height: 100px;"
              ></div>
            </div>
        ...
        <div>
          <div class="card">
            ...
            <div class="chain-select" id="chain-select">
              <div class="chain-chip selected" data-chain="1" onclick="selectChain(this)">
                Ethereum
              </div>
              <div class="chain-chip" data-chain="11155111" onclick="selectChain(this)">
                Sepolia
              </div>
          ...
          <div class="card">
            ...
            <button class="btn" id="deploy-btn" onclick="deployContract()" disabled>
              Deploy Hook
            </button>
          ...
          <div class="card">
            ...
            <button class="btn-secondary" id="verify-btn" onclick="verifyContract()" disabled>
              Verify
            </button>
          ...
          <div class="card">
            ...
            <button class="btn" id="attach-btn" onclick="attachToProject()" disabled>
    ...
    <script type="module">
      ...
      const EXPLORERS = {
        ...
        11155111: {
          ...
          api: 'https://api-sepolia.etherscan.io',
        },
        10: {
          name: 'Optimism',
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
      ...
      window.connectWallet = async function () {
        ...
        try {
          ...
          walletClient = createWalletClient({
            chain: CHAINS_MAP[selectedChainId],
          ...
          document.getElementById('wallet-chain').textContent =
            CHAINS_MAP[parseInt(chainId, 16)]?.name || `Chain ${parseInt(chainId, 16)}`
      ...
      window.compileSource = async function () {
        ...
        try {
          ...
          if (result.errors?.some((e) => e.severity === 'error')) {
            ...
            document.getElementById('compile-errors').textContent = errors
          ...
          showStatus(
            'compile-status',
            'success',
            `Compiled ${Object.keys(compiledContracts).length} contract(s)`,
          )
      ...
      window.deployContract = async function () {
        if (!selectedContract || !connectedAddress) {
          showStatus('deploy-status', 'error', 'Select contract first')
          return
        }
        ...
        if (parseInt(chainId, 16) !== selectedChainId) {
          showStatus(
            'deploy-status',
            'warning',
            `Please switch to ${CHAINS_MAP[selectedChainId]?.name}`,
          )
          try {
      ...
      window.copyAddress = () => {
        ...
        showStatus('deploy-status', 'success', 'Copied!')
      }
      window.viewOnExplorer = () => {
        window.open(`${EXPLORERS[selectedChainId].url}/address/${deployedAddress}`, '_blank')
      }

      window.verifyContract = async function () {
        const apiKey = document.getElementById('etherscan-key').value
        if (!apiKey) {
          showStatus('verify-status', 'warning', 'API key required')
          return
        }
        ...
        try {
          ...
          showStatus('verify-status', 'error', `Error: ${error.message}`)
        }
      }
      ...
      window.attachToProject = function () {
        ...
        if (!projectId) {
          showStatus('attach-status', 'error', 'Enter project ID')
          return
        }
        ...
        showStatus(
          'attach-status',
          'info',
          `To attach hook to project ${projectId}:
