...
function runTest(context) {
  ...
  if (lower.startsWith('sign up')) {
    ...
    if (existingProfile) {
      return success(
        `Hey ${name}! Your profile already exists. Use 'View profile' to see it, or contact me to update it.`,
      )
    }
  ...
  if (lower.startsWith('view profile') || lower.startsWith('my profile')) {
    ...
    if (targetName) {
      const targetProfile = profiles.find((p) => p.name.toLowerCase() === targetName.toLowerCase())

      if (!targetProfile) {
        return error(
          `No profile found for "${targetName}". Try "Sign up" first, or browse all profiles.`,
        )
      }
