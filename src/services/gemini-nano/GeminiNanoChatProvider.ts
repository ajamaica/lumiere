...
/**
 * Chat provider for Gemini Nano (on-device AI for Android).
 *
 * Uses Google's Gemini Nano model via a local Expo native module
 * to run language models entirely on-device. Requires Android devices
 * with Gemini Nano support (typically Android 14+ on supported devices).
 *
 * Supports streaming responses via native event emitter.
 */
export class GeminiNanoChatProvider implements ChatProvider {
  ...
  private nativeModule: typeof import('../../../modules/gemini-nano') | null = null
  private activeCleanup: (() => void) | null = null

  constructor(_config: ProviderConfig) {
    // Config is accepted for interface compatibility but Gemini Nano
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.nativeModule = require('../../../modules/gemini-nano')
    } catch {
      this.connected = false
      this.notifyConnectionState(false, false)
      throw new Error(
        'Gemini Nano native module not found. Ensure the app is built with native module support.',
      )
    }
  ...
  async sendMessage(
    ...
    if (!this.nativeModule) {
      throw new Error('Gemini Nano is not connected.')
    }
