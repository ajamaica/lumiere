...
class MintGardenAPI {
  ...
  async getCollections(options = {}) {
    const { sort = null, cursor = null, limit = 50, verified = null } = options
    const params = { limit }
  ...
  async _get(path, params = {}) {
    try {
      ...
      if (error.response) {
        throw new Error(
          `MintGarden API error: ${error.response.status} - ${error.response.data?.message || error.response.statusText}`,
        )
