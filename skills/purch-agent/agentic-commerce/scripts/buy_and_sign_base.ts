...
import { createWalletClient, http, parseTransaction, type TransactionSerializable } from 'viem'
...
Required packages:
  ...
  try {
    ...
    console.log(
      `   Total: ${orderResult.totalPrice?.amount ?? 'N/A'} ${(orderResult.totalPrice?.currency ?? 'USDC').toUpperCase()}`,
    )
    console.log()
