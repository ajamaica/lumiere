...
export class DatabaseManager {
  ...
  public upsertUserReputation(reputation: UserReputation): void {
    ...
    if (typeof reputation.trust_score !== 'number' || reputation.trust_score < 0) {
      throw new DatabaseError(
        `trust_score must be a non-negative number, got: ${reputation.trust_score}`,
      )
    }
    ...
    try {
      ...
      throw new DatabaseError(
        `Failed to upsert user reputation for user ${reputation.user_id}`,
        error,
      )
    }
  }
  ...
  public getNotificationLogsByEventId(eventId: number): NotificationLog[] {
    ...
    try {
      const stmt = this.db.prepare(
        'SELECT * FROM notifications_log WHERE event_id = ? ORDER BY timestamp DESC',
      )
