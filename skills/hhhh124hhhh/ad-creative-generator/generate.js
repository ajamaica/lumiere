...
function printHeader() {
  ...
  console.log(
    `${colors.cyan}${colors.bright}║          🎨 AD CREATIVE GENERATOR v1.0                  ║${colors.reset}`,
  )
  console.log(
    `${colors.cyan}${colors.bright}║          Generate Compelling Ad Prompts                 ║${colors.reset}`,
  )
  console.log(
    `${colors.cyan}${colors.bright}╚════════════════════════════════════════════════════════════╝${colors.reset}\n`,
  )
...
function displayCategoryMenu() {
  ...
  categories.forEach((cat, index) => {
    ...
    console.log(
      `  ${colors.cyan}${index + 1}.${colors.reset} ${colors.bright}${cat.charAt(0).toUpperCase() + cat.slice(1)}${colors.reset}`,
    )
...
async function getCategorySelection() {
  ...
  while (true) {
    ...
    if (selectedCategories.size > 0) {
      console.log(
        `${colors.green}Selected Categories: ${Array.from(selectedCategories)
    ...
    const choice = await question(
      'Select category (number, "a" for all, "d" for done): ',
    ).toLowerCase()
...
function displayPrompts(product, prompts) {
  ...
  for (const [category, items] of Object.entries(grouped)) {
    console.log(
      `${colors.bright}${colors.magenta}${category.charAt(0).toUpperCase() + category.slice(1)}${colors.reset}`,
    )
    console.log(`${colors.dim}${'-'.repeat(70)}${colors.reset}`)
...
function commandLineMode(options) {
  try {
    ...
    if (!options.export || options.export === 'display') {
      ...
      const filename =
        options.output ||
        `ad-creative-${options.product.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.${format === 'markdown' ? 'md' : format}`
      saveToFile(content, filename)
    }
