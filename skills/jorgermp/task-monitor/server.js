...
async function fetchStatusData() {
  try {
    ...
    if (cronJobsCache.jobs.length === 0 || cronCacheAge > CRON_CACHE_TTL_MS) {
      try {
        const { stdout: cronOutput } = await execAsync('openclaw cron list --json', {
          timeout: 10000,
        })
        ...
        if (freshJobs.length > 0) {
          ...
          cronJobs = freshJobs
          console.log(
            `Cron cache updated: ${freshJobs.length} jobs (age was ${Math.round(cronCacheAge / 1000)}s)`,
          )
        } else {
        ...
        console.warn(
          `Failed to load cron jobs (using cached: ${cronJobsCache.jobs.length}):`,
          e.message,
        )
      ...
      console.log(
        `Using cached cron jobs (${cronJobs.length}, age: ${Math.round(cronCacheAge / 1000)}s)`,
      )
    }
    ...
    const enhancedCrons = cronSessions.map((s) => {
      ...
      if (!cronJob && cronJobs.length > 0) {
        console.log(
          `No match for jobId: ${jobId}, available IDs: ${cronJobs
            .slice(0, 3)
            .map((j) => j.id)
            .join(', ')}`,
        )
