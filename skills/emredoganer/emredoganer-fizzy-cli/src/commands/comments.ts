...
import { addComment, deleteComment, listComments, updateComment } from '../lib/api.js'
import { isAuthenticated } from '../lib/config.js'

export function createCommentsCommands(): Command {
  const comments = new Command('comments').description('Manage card comments')
  ...
  comments
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to list comments:'),
  ...
  comments
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to add comment:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
  ...
  comments
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to update comment:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
  ...
  comments
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to delete comment:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
