...
function getMemoryStats() {
  ...
  try {
    ...
    if (fs.existsSync(memoryDir)) {
      const files = collectMemoryFiles(memoryDir, memoryDir).sort((a, b) => b.modified - a.modified)
...
function getLlmUsage() {
  ...
  try {
    if (fs.existsSync(stateFile)) {
      ...
      const weeklyValid =
        data.claude?.weekly_all_models?.resets && data.claude.weekly_all_models.resets !== 'unknown'
