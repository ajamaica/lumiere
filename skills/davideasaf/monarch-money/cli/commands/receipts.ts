...
export const receiptsCommand = new Command('receipts').description(
  'Receipt splitting and management',
)
  ...
  .action(async (transactionId, options) => {
    try {
        ...
        console.log(
          JSON.stringify(
            [
              ...
              { description: 'Cleaning supplies', amount: 12.5, category: 'Home' },
            ],
            null,
            2,
          ),
      ...
      const splits = items.map((item) => {
        const categoryId =
          categoryMap.get(item.category.toLowerCase()) || categoryMap.get(item.category)
