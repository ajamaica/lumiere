...
async function executeOmnichain(signer, targetChains, targetContract, calldata) {
  ...
  console.log(
    'Payment options:',
    quote.payment_info.map((p) => `Chain ${p.chain}: ${ethers.formatEther(p.amount)} ETH`),
  )
...
async function waitForCompletion(bundleUuid, onUpdate) {
  while (true) {
    ...
    console.log(
      'Status:',
      status.transactions.map((t) => `Chain ${t.chain}: ${t.status}`).join(', '),
    )
...
async function main() {
  ...
  const result = await waitForCompletion(quote.bundle_uuid, (status) => {
    console.log(
      'Update:',
      status.transactions.map((t) => t.status),
    )
  })
  ...
  console.log(
    'Tx hashes:',
    result.transactions.map((t) => `${t.chain}: ${t.tx_hash}`),
  )
}
```
