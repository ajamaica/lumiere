...
export function formatPretty(data: WhoopData): string {
  ...
  if (data.recovery?.length) {
    ...
    lines.push(
      `💚 Recovery: ${r.recovery_score}% | HRV: ${r.hrv_rmssd_milli.toFixed(1)}ms | RHR: ${r.resting_heart_rate}bpm`,
    )
  ...
  if (data.sleep?.length) {
    ...
    lines.push(
      `😴 Sleep: ${s.sleep_performance_percentage}% | ${hours}h | Efficiency: ${s.sleep_efficiency_percentage.toFixed(0)}%`,
    )
    lines.push(
  ...
  if (data.cycle?.length) {
    ...
    lines.push(
      `🔄 Day strain: ${c.strain.toFixed(1)} | ${(c.kilojoule / 4.184).toFixed(0)} cal | Avg HR: ${c.average_heart_rate}`,
    )
  }
...
export function formatSummaryColor(data: WhoopData): string {
  ...
  if (data.recovery?.length) {
    ...
    lines.push(
      `${icon} Recovery: ${r.recovery_score}% | HRV: ${r.hrv_rmssd_milli.toFixed(0)}ms | RHR: ${r.resting_heart_rate}bpm`,
    )
  }
  ...
  if (data.sleep?.length) {
    ...
    const hours = (s.stage_summary.total_in_bed_time_milli / 3600000).toFixed(1)
    lines.push(
      `${icon} Sleep: ${s.sleep_performance_percentage}% | ${hours}h | Efficiency: ${s.sleep_efficiency_percentage.toFixed(0)}%`,
    )
  }
  ...
  if (data.cycle?.length) {
    ...
    lines.push(
      `${icon} Strain: ${c.strain.toFixed(1)} (optimal: ~${optimal}) | ${(c.kilojoule / 4.184).toFixed(0)} cal`,
    )
  }

  if (data.workout?.length) {
    lines.push(
      `🏋️ Workouts: ${data.workout.length} | ${data.workout.map((w) => w.sport_name).join(', ')}`,
    )
  }
