...
import { addStep, completeStep, deleteStep, listSteps, uncompleteStep } from '../lib/api.js'
import { isAuthenticated } from '../lib/config.js'

export function createStepsCommands(): Command {
  const steps = new Command('steps').description('Manage card steps (subtasks)')
  ...
  steps
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to list steps:'),
  ...
  steps
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to complete step:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
  ...
  steps
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to uncomplete step:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
  ...
  steps
    ...
    .action(async (id: string, options) => {
      ...
      try {
        ...
        console.error(
          chalk.red('Failed to delete step:'),
          error instanceof Error ? error.message : error,
        )
        process.exit(1)
      }
