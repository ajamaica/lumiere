...
class AgentLedger {
  ...
  async loadTransactions() {
    ...
    try {
      ...
      console.warn(
        `Warning: Could not load transactions (${error.message}). Attempting backup recovery...`,
      )
  ...
  async getCategorySpending(category, period = 'this-month') {
    const transactions = await this.loadTransactions()
    const filtered = this.filterTransactionsByPeriod(transactions, period).filter(
  ...
  async exportTransactions(format, filePath, options = {}) {
    ...
    if (format === 'csv') {
      const csvHeader =
        'Date,Amount,Currency,Vendor,Description,Category,Account,Context,ReceiptURL,ConfirmationID\n'
