    ...
    >
      ...
      >
        ...
        >
          ...
          {isOutgoing && showCheck && <span style={{ color: COLORS.blue, fontSize: 24 }}>✓✓</span>}
        </div>
      </div>
    </div>
  )
...
const TypingIndicator = ({ delay = 0, duration = 60 }: { delay?: number; duration?: number }) => {
  ...
  const opacity = interpolate(frame - delay, [0, 8, duration - 8, duration], [0, 1, 1, 0], {
...
Includes template, docs, and render script. Just tell me what conversation to show and I'll generate the video.`}
        ...
        {/* Pokey's answer */}
        <Message text="Publish it to ClawdHub! 🚀" isOutgoing={false} time="9:21 AM" delay={440} />
      </div>
    </div>
  )
}
