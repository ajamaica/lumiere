...
import type { GuildMember, StageChannel, VoiceBasedChannel, VoiceChannel } from 'discord.js'
...
export class VoiceConnectionManager {
  ...
  async join(channel: VoiceBasedChannel): Promise<VoiceSession> {
    ...
    try {
      ...
      this.logger.info(
        `[discord-voice] Joined voice channel ${channel.name} in ${channel.guild.name}`,
      )
    } catch (error) {
      connection.destroy()
  ...
  private async attemptReconnect(
    ...
    this.logger.info(
      `[discord-voice] Reconnection attempt ${attempt}/${this.MAX_RECONNECT_ATTEMPTS}`,
    )
  ...
  private startListening(session: VoiceSession): void {
    ...
    receiver.speaking.on('start', (userId: string) => {
      ...
      if (session.lastSpokeAt && Date.now() - session.lastSpokeAt < SPEAK_COOLDOWN_MS) {
        this.logger.debug?.(
          `[discord-voice] Ignoring speech during cooldown (likely residual echo)`,
        )
        return
      }
      ...
      if (session.speaking) {
        if (this.config.bargeIn) {
          this.logger.info(
            `[discord-voice] Barge-in detected from user ${userId}! Stopping speech.`,
          )
  ...
  private startRecording(session: VoiceSession, userId: string): void {
    ...
    const useStreaming =
      this.streamingSTT && this.config.sttProvider === 'deepgram' && this.config.streamingSTT
  ...
  private async processRecording(
    ...
    if (session.processing) {
      this.logger.debug?.(
        `[discord-voice] Skipping processing - already processing another request`,
      )
      return
    }
    ...
    try {
      ...
      try {
        ...
        response = await this.onTranscript(
          userId,
          session.guildId,
          session.channelId,
          transcribedText,
  ...
  async speak(guildId: string, text: string): Promise<void> {
    ...
    try {
      this.logger.info(
        `[discord-voice] Speaking: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`,
      )
  ...
  private async playThinkingSoundSimple(session: VoiceSession): Promise<void> {
    try {
      ...
      this.logger.debug?.(
        `[discord-voice] Error playing thinking sound: ${error instanceof Error ? error.message : String(error)}`,
      )
    }
  }
