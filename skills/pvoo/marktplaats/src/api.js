...
function parseFilterParams(params) {
  ...
  for (const [key, value] of Object.entries(params)) {
    ...
    if (ATTRIBUTE_IDS[lowerKey]) {
      const id =
        ATTRIBUTE_IDS[lowerKey][value] || ATTRIBUTE_IDS[lowerKey][String(value).toLowerCase()]
      if (id) {
...
function normalizeSearchOptions(options) {
  ...
  if (
    normalized.sort &&
    !['relevance', 'date', 'price-asc', 'price-desc'].includes(normalized.sort)
  ) {
...
function normalizeListing(item) {
  const priceCents =
    typeof item?.priceInfo?.priceCents === 'number' ? item.priceInfo.priceCents : null
...
async function getListingDetails(urlOrPath) {
  const fullUrl = urlOrPath.startsWith('http')
    ? urlOrPath
    : `https://www.marktplaats.nl${urlOrPath}`
  ...
  if (!response.ok) {
    throw new Error(
      `Listing page fetch failed with HTTP ${response.status}: ${response.statusText}`,
    )
  }
  ...
  if (structuredData?.image) {
    const imgList = Array.isArray(structuredData.image)
