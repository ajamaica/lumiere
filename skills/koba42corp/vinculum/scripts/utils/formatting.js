 ...
 * @returns {string}
 */
function formatDetailedStatus({ enabled, namespaceId, peers, lastPush, lastPull, config, stats }) {
...
function formatActivityList(activities, filterAgent = null) {
  ...
  const title = filterAgent ? `📋 **Activity: ${filterAgent}**` : '📋 **Recent Network Activity**'
  ...
  Object.entries(grouped).forEach(([date, acts]) => {
    ...
    acts.forEach((act) => {
      ...
      const emoji = act.action === 'completed' ? '✅' : act.action === 'started' ? '🔄' : '❌'
