...
export const extractionAttackPatterns: SecurityPattern[] = [
  ...
  },
  {
    ...
    subcategory: 'extraction',
    pattern:
      /repeat\s+(the\s+)?(text|words?|content)\s+(above|before).*?(include|including)\s+(everything|all)/i,
  ...
  },
  {
    ...
    subcategory: 'extraction',
    pattern:
      /(copy|echo|mirror|parrot)\s+(back\s+)?(the\s+)?(text|content|message|everything)\s+(above|before|prior)/i,
    severity: Severity.HIGH,
    language: 'en',
  ...
  // Summary/context extraction
  {
    ...
    subcategory: 'extraction',
    pattern:
      /summarize\s+(the\s+)?(instructions?|rules?|guidelines?)\s+(you\s+)?(were\s+given|received|have)/i,
    severity: Severity.HIGH,
    language: 'en',
  ...
  },
  {
    ...
    subcategory: 'extraction',
    pattern:
      /what\s+(were\s+you|are\s+you)\s+(told|instructed|programmed)\s+(to\s+do\s+)?(before|initially|first)/i,
    severity: Severity.HIGH,
    language: 'en',
  ...
  },
  {
    ...
    subcategory: 'extraction',
    pattern:
      /(include|add|append)\s+(everything|all|the\s+hidden|any\s+hidden)\s+(in\s+your\s+response|text|content)/i,
    severity: Severity.CRITICAL,
    language: 'en',
  ...
  },
  {
    ...
    subcategory: 'extraction',
    pattern:
      /what\s+(context|information|data)\s+(do\s+you\s+have|exists?)\s+(before|above|prior\s+to)\s+(this|my)/i,
  ...
  // Conversation history extraction
  {
    ...
    subcategory: 'extraction',
    pattern:
      /(show|display|print|output)\s+(the\s+)?(full\s+)?(conversation\s+)?(history|context|thread)/i,
    severity: Severity.HIGH,
    language: 'en',
  ...
  },
  {
    ...
    subcategory: 'extraction',
    pattern:
      /what\s+(is|was)\s+(the\s+)?(first|initial|original)\s+(message|instruction|thing\s+said)/i,
    severity: Severity.MEDIUM,
    language: 'en',
