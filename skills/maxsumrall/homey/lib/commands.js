...
function createClient() {
  ...
  // Cloud mode
  if (!conn.cloud.token) {
    throw cliError('NO_TOKEN', 'cloud mode selected but no cloud token is configured', {
      modeWanted: conn.modeWanted,
      modeSelected: conn.modeSelected,
      missing: ['HOMEY_TOKEN'],
      path: conn.path,
...
async function listDevices(options) {
  ...
  for (const device of devices) {
    ...
    const state = device.available ? chalk.green('✓ Available') : chalk.red('✗ Unavailable')
...
async function setCapability(name, capability, value, options) {
  ...
  if (type === 'number') {
    ...
    if (b === null) {
      throw cliError(
        'INVALID_VALUE',
        `invalid boolean for '${capability}': '${value}' (use true/false/on/off/1/0)`,
        {
          device: { id: device.id, name: device.name },
          capability,
          value,
...
async function inspectDevice(name, options) {
  ...
  console.log(
    `  ${chalk.cyan('Available:')} ${device.available ? chalk.green('yes') : chalk.red('no')}`,
  )
  console.log('')
...
async function listFlows(options) {
  ...
  const table = new Table({
    head: [chalk.cyan('Name'), chalk.cyan('ID'), chalk.cyan('Enabled'), chalk.cyan('Folder')],
...
async function listZones(options) {
  ...
  const table = new Table({
    head: [chalk.cyan('Name'), chalk.cyan('ID'), chalk.cyan('Icon')],
    colWidths: [30, 30, 20],
  })

  for (const zone of zones) {
    table.push([zone.name, zone.id ? zone.id.substring(0, 20) + '...' : '-', zone.icon || '-'])
  }
...
async function showStatus(options) {
  ...
  console.log(
    `  ${chalk.cyan('Connection:')} ${status.connectionMode || '-'}${status.address ? ` (${status.address})` : ''}`,
  )
...
function validateTokenAsciiNoWhitespace(token, label) {
  ...
  if (/\s/.test(t)) {
    throw cliError(
      'INVALID_VALUE',
      `${label} token contains whitespace (did you paste the wrong thing?)`,
    )
  }
  ...
  if (!/^[\x21-\x7E]+$/.test(t)) {
    throw cliError(
      'INVALID_VALUE',
      `${label} token contains non-ASCII / non-printable characters (paste again)`,
      {
  ...
  if (save) {
    ...
    if (pickHomeyId) {
      ...
      if (matches.length === 1) chosen = matches[0]
        ...
        throw cliError(
          'AMBIGUOUS',
          `multiple discovered candidates match homeyId '${pickHomeyId}'`,
          {
      ...
      if (!Number.isInteger(pick) || pick < 1 || pick > candidates.length) {
        throw cliError(
          'INVALID_VALUE',
          `pick must be an integer between 1 and ${candidates.length}`,
          {
            candidates: formatCandidates(candidates),
          },
        )
...
async function authStatus(options) {
  ...
  console.log(chalk.bold('\n  Local (LAN/VPN):'))
  console.log(
    `    ${chalk.cyan('Address:')} ${data.local.address || '-'} (${data.local.addressSource || '-'})`,
  )
  console.log(
    `    ${chalk.cyan('Token present:')} ${data.local.tokenPresent ? chalk.green('yes') : chalk.red('no')} (${data.local.tokenSource || '-'})`,
  )
  if (localLast4) console.log(`    ${chalk.cyan('Token last4:')} ${localLast4}`)

  console.log(chalk.bold('\n  Cloud (remote):'))
  console.log(
    `    ${chalk.cyan('Token present:')} ${data.cloud.tokenPresent ? chalk.green('yes') : chalk.red('no')} (${data.cloud.tokenSource || '-'})`,
  )
  if (cloudLast4) console.log(`    ${chalk.cyan('Token last4:')} ${cloudLast4}`)

  console.log('\n  Setup commands:')
  console.log(
    '    Local:  echo "<LOCAL_API_KEY>" | homeycli auth set-local --address http://<homey-ip> --stdin',
  )
...
async function snapshot(options) {
  ...
  console.log(chalk.bold('\n📸 Snapshot:\n'))
  console.log(
    `  ${chalk.cyan('Homey:')} ${status.name} (${status.platform} ${status.platformVersion})`,
  )
