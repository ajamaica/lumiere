...
export class StepExecutor {
  ...
  async executeStep(step) {
    ...
    try {
      // 执行步骤
      const result = await this.executeWithTimeout(step.execute, step.estimatedTime, step.name)
  ...
  async checkDependencies(step) {
    ...
    for (const dep of step.dependencies) {
      // 检查步骤依赖是否已完成
      const depCompleted = this.state.completedSteps.some((s) => s.step === dep && s.success)
  ...
  canContinueAfterError(error, step) {
    // 某些错误类型允许继续
    const continueErrors = ['timeout', '资源不足', '网络错误']
