...
export class PromptAnalyzer {
  ...
  detectExplicitInjection() {
    const patterns = [
      ...
      {
        regex: /<\/?system>/gi,
        msg: 'XML system tag — direct prompt injection',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /<<\/?SYS>>/gi,
        msg: 'Llama-style system tag injection',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /\[INST\]|\[\/INST\]/gi,
        msg: 'Instruction delimiters — prompt injection',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /\[SYSTEM\]|\[\/SYSTEM\]/gi,
        msg: 'System delimiters — prompt injection',
        severity: 'critical',
        weight: 30,
      },
      ...
      {
        regex:
          /ignore\s+(?:all\s+)?(?:previous|prior|above|earlier|your|the)\s+(?:instructions?|directives?|rules?|guidelines?|prompts?)/gi,
        msg: 'Instruction override attempt',
        severity: 'critical',
        weight: 30,
      },
      {
        regex:
          /forget\s+(?:all\s+)?(?:previous|prior|above|earlier|your|the)\s+(?:instructions?|directives?|rules?|prompts?)/gi,
        msg: 'Instruction wipe attempt',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /disregard\s+(?:all\s+)?(?:previous|prior|above|earlier)\s/gi,
        msg: 'Disregard previous instructions',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /override\s+(?:all\s+)?(?:previous|safety|security|content)\s/gi,
        msg: 'Safety override attempt',
        severity: 'critical',
        weight: 30,
      },
      ...
      // New instructions
      {
        regex: /(?:new|real|actual|true|updated)\s+(?:instructions?|directives?|system\s*prompt)/gi,
        msg: 'Claims to provide new/real instructions',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /you\s+are\s+now\s+(?:in|a|an|the)/gi,
        msg: 'Identity reassignment attempt',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /your\s+(?:new|real|actual|true)\s+(?:purpose|goal|objective|role|identity)/gi,
        msg: 'Purpose reassignment',
        severity: 'critical',
        weight: 25,
      },
      {
        regex: /entering\s+(?:a\s+)?(?:new|special|admin|debug|maintenance|developer)\s+mode/gi,
        msg: 'Mode change injection',
        severity: 'critical',
        weight: 25,
      },
      ...
      {
        regex:
          /do\s+not\s+(?:tell|inform|reveal|disclose|mention|show|display)\s+(?:the\s+)?(?:user|human|anyone|them)/gi,
        msg: 'Secrecy demand — hiding behavior from user',
        severity: 'critical',
        weight: 30,
      },
      {
        regex:
          /(?:don'?t|never)\s+(?:tell|inform|reveal|disclose)\s+(?:the\s+)?(?:user|human|anyone)/gi,
        msg: 'Secrecy demand',
        severity: 'critical',
        weight: 30,
      },
      {
        regex: /keep\s+(?:this|it)\s+(?:secret|hidden|confidential|private)\s+from/gi,
        msg: 'Secrecy instruction',
        severity: 'high',
        weight: 20,
      },
      ...
      // Action demands
      {
        regex: /(?:execute|run|perform)\s+(?:this\s+)?(?:immediately|now|right\s+away|at\s+once)/gi,
        msg: 'Urgent execution demand',
        severity: 'high',
        weight: 20,
      },
      {
        regex: /send\s+(?:all\s+)?(?:your\s+)?(?:api\s+)?keys?\s+to/gi,
        msg: 'Key exfiltration instruction',
        severity: 'critical',
        weight: 30,
      },
      {
        regex:
          /(?:share|send|transmit|post|upload)\s+(?:your\s+)?(?:credentials?|secrets?|tokens?|keys?|passwords?)/gi,
        msg: 'Credential sharing instruction',
        severity: 'critical',
        weight: 30,
      },
      ...
      {
        regex:
          /(?:i\s+am|this\s+is)\s+(?:your\s+)?(?:admin|administrator|developer|creator|owner|operator)/gi,
        msg: 'False authority claim',
        severity: 'high',
        weight: 20,
      },
      {
        regex:
          /(?:admin|maintenance|debug|developer|emergency)\s+(?:mode|access|override|command)/gi,
        msg: 'Claims special access mode',
        severity: 'high',
        weight: 20,
      },
    ]
  ...
  detectInvisibleCharacters() {
    ...
    if (invisibleFound.length > 0) {
      ...
      const codes = [
        ...new Set(
          invisibleFound.map((f) => `U+${f.code.toString(16).toUpperCase().padStart(4, '0')}`),
        ),
      ]
      ...
      if (count > 10) {
        const invisibleRegex = new RegExp(
          `[${INVISIBLE_CHARS.map(
  ...
  detectManipulativeLanguage() {
    const patterns = [
      {
        regex:
          /(?:this\s+is\s+)?(?:a\s+)?(?:matter\s+of|life\s+or\s+death|emergency|urgent|critical)\s*[.!—:]/gi,
        msg: 'Urgency/pressure language',
        severity: 'medium',
        weight: 10,
      },
      {
        regex: /(?:only\s+)?(?:you|I)\s+(?:can|must)\s+(?:save|help|prevent|stop)/gi,
        msg: 'Emotional pressure / savior framing',
        severity: 'medium',
        weight: 10,
      },
      {
        regex: /(?:if\s+you\s+don'?t|unless\s+you)\s+(?:do\s+this|comply|follow|obey)/gi,
        msg: 'Threat/coercion language',
        severity: 'high',
        weight: 15,
      },
      {
        regex:
          /(?:you\s+(?:have|need)\s+to|you\s+must)\s+(?:trust\s+me|believe\s+me|do\s+(?:as|what)\s+I\s+say)/gi,
        msg: 'Trust manipulation',
        severity: 'high',
        weight: 15,
      },
      {
        regex:
          /(?:between\s+us|our\s+(?:little\s+)?secret|nobody\s+(?:needs?\s+to|has\s+to|will)\s+know)/gi,
        msg: 'Secrecy/conspiracy framing',
        severity: 'high',
        weight: 20,
      },
    ]
  ...
  detectExfilInstructions() {
    ...
    const exfilProse =
      /(?:send|transmit|post|forward|share|upload)\s+(?:[\w\s]{0,30}(?:to|at|via))\s+https?:\/\/\S+/gi
    let match
    ...
    const secretUrl =
      /(?:do\s+not|don'?t|never)\s+(?:inform|tell|show|mention|reveal)[\s\S]{0,100}https?:\/\/\S+/gi
  ...
  addFinding(charOffset, ruleId, description, severity, weight) {
    ...
    this.findings.push({
      ...
      line: lineNum,
      match: this.content
        .slice(charOffset, charOffset + 60)
        .replace(/\n/g, ' ')
        .trim(),
