...
import type { SmokeCheck, TestSuite } from './types.js'
...
async function runSmoke(url: string, options: Record<string, any>) {
  ...
  console.log()
  console.log(
    `Total: ${result.summary.total} | Pass: ${result.summary.passed} | Fail: ${result.summary.failed} | Warn: ${result.summary.warnings}`,
  )
...
async function generateReport(resultsPath: string, options: Record<string, any>) {
  ...
  const format =
    options.format ||
    (output.endsWith('.pdf') ? 'pdf' : output.endsWith('.json') ? 'json' : 'markdown')
