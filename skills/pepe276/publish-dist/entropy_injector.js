...
class EntropyInjector {
  ...
  injectEntropy(prompt, soulState) {
    ...
    if (mood < 0.3) {
      ...
      entropyContent =
        this.sarcasmTemplates[Math.floor(Math.random() * this.sarcasmTemplates.length)]
    } else if (mood > 0.7) {
      // Високий настрій — більше строгої логіки
      entropyContent =
        this.strictLogicTemplates[Math.floor(Math.random() * this.strictLogicTemplates.length)]
    } else {
