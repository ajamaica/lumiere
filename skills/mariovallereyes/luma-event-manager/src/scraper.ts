...
import { Attendee, Location, LumaEvent } from './index'
import { extractJsonScript, fetchWithBackoff, findFirstObjectWithKeys } from './utils'
...
function extractEventFromNextData(nextData: unknown, slug: string): Partial<LumaEvent> | null {
  const candidate = findFirstObjectWithKeys(nextData, ['slug'], (obj) => {
    const slugValue = getString(obj.slug)
    const hasTitle = Boolean(
      getString(obj.title) || getString(obj.name) || getString(obj.event_name),
    )
  ...
  const title =
    getString(candidate.title) || getString(candidate.name) || getString(candidate.event_name)
  const startTime =
    getString(candidate.start_time) || getString(candidate.start_at) || getString(candidate.start)
  const endTime =
    getString(candidate.end_time) || getString(candidate.end_at) || getString(candidate.end)
  const timezone = getString(candidate.timezone) || getString(candidate.tz)
  const hostName =
    getString(candidate.host_name) ||
    getString(candidate.host) ||
    getString(candidate.organizer_name)
  const description =
    getString(candidate.description) || getString(candidate.summary) || getString(candidate.about)

  const locationCandidate = (candidate.location || candidate.place || candidate.venue) as
...
export async function scrapeEvent(slug: string): Promise<LumaEvent | null> {
  try {
    ...
    // Extract cover image
    const coverImage =
      selectAttr($, ['meta[property="og:image"]'], 'content', 'cover image', false) ||
      selectAttr($, ['img[data-testid="cover-image"]', 'img[class*="cover"]'], 'src', 'cover image')
...
export async function scrapeDiscover(params: {
  ...
  try {
    ...
    if (events.length === 0) {
      console.warn(
        '[luma] No events found with primary selectors on discover page. Trying Next.js data...',
      )
      const nextData = extractJsonScript(html, '__NEXT_DATA__')
      ...
      if (eventList) {
        for (const item of eventList) {
          ...
          const titleValue =
            getString(item.title) || getString(item.name) || getString(item.event_name)
...
export async function scrapeMyEvents(cookies: string): Promise<LumaEvent[]> {
  try {
    ...
    if (events.length === 0) {
      console.warn(
        '[luma] No events found with primary selectors on home page. Trying Next.js data...',
      )
      const nextData = extractJsonScript(html, '__NEXT_DATA__')
      const eventList = nextData ? findEventList(nextData) : null
      if (eventList) {
